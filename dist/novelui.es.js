import V, { useRef as ye, useContext as De, useLayoutEffect as Er, useMemo as Lr, useDebugValue as Ir, createElement as ft, createContext as Wn, useState as _e, useEffect as be, useCallback as xe, useReducer as Vn, forwardRef as Cn, isValidElement as pt, cloneElement as vr } from "react";
import { createPortal as Fo } from "react-dom";
var Nt = {}, ko = {
  get exports() {
    return Nt;
  },
  set exports(e) {
    Nt = e;
  }
}, wt = {};
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Jr;
function Mo() {
  if (Jr)
    return wt;
  Jr = 1;
  var e = V, t = Symbol.for("react.element"), r = Symbol.for("react.fragment"), n = Object.prototype.hasOwnProperty, o = e.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, i = { key: !0, ref: !0, __self: !0, __source: !0 };
  function a(s, l, c) {
    var f, d = {}, g = null, h = null;
    c !== void 0 && (g = "" + c), l.key !== void 0 && (g = "" + l.key), l.ref !== void 0 && (h = l.ref);
    for (f in l)
      n.call(l, f) && !i.hasOwnProperty(f) && (d[f] = l[f]);
    if (s && s.defaultProps)
      for (f in l = s.defaultProps, l)
        d[f] === void 0 && (d[f] = l[f]);
    return { $$typeof: t, type: s, key: g, ref: h, props: d, _owner: o.current };
  }
  return wt.Fragment = r, wt.jsx = a, wt.jsxs = a, wt;
}
var Pt = {};
/**
 * @license React
 * react-jsx-runtime.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Kr;
function Ro() {
  return Kr || (Kr = 1, process.env.NODE_ENV !== "production" && function() {
    var e = V, t = Symbol.for("react.element"), r = Symbol.for("react.portal"), n = Symbol.for("react.fragment"), o = Symbol.for("react.strict_mode"), i = Symbol.for("react.profiler"), a = Symbol.for("react.provider"), s = Symbol.for("react.context"), l = Symbol.for("react.forward_ref"), c = Symbol.for("react.suspense"), f = Symbol.for("react.suspense_list"), d = Symbol.for("react.memo"), g = Symbol.for("react.lazy"), h = Symbol.for("react.offscreen"), v = Symbol.iterator, y = "@@iterator";
    function p(u) {
      if (u === null || typeof u != "object")
        return null;
      var O = v && u[v] || u[y];
      return typeof O == "function" ? O : null;
    }
    var A = e.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    function w(u) {
      {
        for (var O = arguments.length, E = new Array(O > 1 ? O - 1 : 0), B = 1; B < O; B++)
          E[B - 1] = arguments[B];
        X("error", u, E);
      }
    }
    function X(u, O, E) {
      {
        var B = A.ReactDebugCurrentFrame, se = B.getStackAddendum();
        se !== "" && (O += "%s", E = E.concat([se]));
        var pe = E.map(function(oe) {
          return String(oe);
        });
        pe.unshift("Warning: " + O), Function.prototype.apply.call(console[u], console, pe);
      }
    }
    var T = !1, m = !1, M = !1, Z = !1, z = !1, W;
    W = Symbol.for("react.module.reference");
    function C(u) {
      return !!(typeof u == "string" || typeof u == "function" || u === n || u === i || z || u === o || u === c || u === f || Z || u === h || T || m || M || typeof u == "object" && u !== null && (u.$$typeof === g || u.$$typeof === d || u.$$typeof === a || u.$$typeof === s || u.$$typeof === l || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      u.$$typeof === W || u.getModuleId !== void 0));
    }
    function Y(u, O, E) {
      var B = u.displayName;
      if (B)
        return B;
      var se = O.displayName || O.name || "";
      return se !== "" ? E + "(" + se + ")" : E;
    }
    function j(u) {
      return u.displayName || "Context";
    }
    function F(u) {
      if (u == null)
        return null;
      if (typeof u.tag == "number" && w("Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."), typeof u == "function")
        return u.displayName || u.name || null;
      if (typeof u == "string")
        return u;
      switch (u) {
        case n:
          return "Fragment";
        case r:
          return "Portal";
        case i:
          return "Profiler";
        case o:
          return "StrictMode";
        case c:
          return "Suspense";
        case f:
          return "SuspenseList";
      }
      if (typeof u == "object")
        switch (u.$$typeof) {
          case s:
            var O = u;
            return j(O) + ".Consumer";
          case a:
            var E = u;
            return j(E._context) + ".Provider";
          case l:
            return Y(u, u.render, "ForwardRef");
          case d:
            var B = u.displayName || null;
            return B !== null ? B : F(u.type) || "Memo";
          case g: {
            var se = u, pe = se._payload, oe = se._init;
            try {
              return F(oe(pe));
            } catch {
              return null;
            }
          }
        }
      return null;
    }
    var H = Object.assign, G = 0, ne, re, I, ve, he, Ne, L;
    function N() {
    }
    N.__reactDisabledLog = !0;
    function q() {
      {
        if (G === 0) {
          ne = console.log, re = console.info, I = console.warn, ve = console.error, he = console.group, Ne = console.groupCollapsed, L = console.groupEnd;
          var u = {
            configurable: !0,
            enumerable: !0,
            value: N,
            writable: !0
          };
          Object.defineProperties(console, {
            info: u,
            log: u,
            warn: u,
            error: u,
            group: u,
            groupCollapsed: u,
            groupEnd: u
          });
        }
        G++;
      }
    }
    function k() {
      {
        if (G--, G === 0) {
          var u = {
            configurable: !0,
            enumerable: !0,
            writable: !0
          };
          Object.defineProperties(console, {
            log: H({}, u, {
              value: ne
            }),
            info: H({}, u, {
              value: re
            }),
            warn: H({}, u, {
              value: I
            }),
            error: H({}, u, {
              value: ve
            }),
            group: H({}, u, {
              value: he
            }),
            groupCollapsed: H({}, u, {
              value: Ne
            }),
            groupEnd: H({}, u, {
              value: L
            })
          });
        }
        G < 0 && w("disabledDepth fell below zero. This is a bug in React. Please file an issue.");
      }
    }
    var b = A.ReactCurrentDispatcher, K;
    function S(u, O, E) {
      {
        if (K === void 0)
          try {
            throw Error();
          } catch (se) {
            var B = se.stack.trim().match(/\n( *(at )?)/);
            K = B && B[1] || "";
          }
        return `
` + K + u;
      }
    }
    var ie = !1, J;
    {
      var Q = typeof WeakMap == "function" ? WeakMap : Map;
      J = new Q();
    }
    function P(u, O) {
      if (!u || ie)
        return "";
      {
        var E = J.get(u);
        if (E !== void 0)
          return E;
      }
      var B;
      ie = !0;
      var se = Error.prepareStackTrace;
      Error.prepareStackTrace = void 0;
      var pe;
      pe = b.current, b.current = null, q();
      try {
        if (O) {
          var oe = function() {
            throw Error();
          };
          if (Object.defineProperty(oe.prototype, "props", {
            set: function() {
              throw Error();
            }
          }), typeof Reflect == "object" && Reflect.construct) {
            try {
              Reflect.construct(oe, []);
            } catch (Ie) {
              B = Ie;
            }
            Reflect.construct(u, [], oe);
          } else {
            try {
              oe.call();
            } catch (Ie) {
              B = Ie;
            }
            u.call(oe.prototype);
          }
        } else {
          try {
            throw Error();
          } catch (Ie) {
            B = Ie;
          }
          u();
        }
      } catch (Ie) {
        if (Ie && B && typeof Ie.stack == "string") {
          for (var $ = Ie.stack.split(`
`), Le = B.stack.split(`
`), Pe = $.length - 1, Oe = Le.length - 1; Pe >= 1 && Oe >= 0 && $[Pe] !== Le[Oe]; )
            Oe--;
          for (; Pe >= 1 && Oe >= 0; Pe--, Oe--)
            if ($[Pe] !== Le[Oe]) {
              if (Pe !== 1 || Oe !== 1)
                do
                  if (Pe--, Oe--, Oe < 0 || $[Pe] !== Le[Oe]) {
                    var Me = `
` + $[Pe].replace(" at new ", " at ");
                    return u.displayName && Me.includes("<anonymous>") && (Me = Me.replace("<anonymous>", u.displayName)), typeof u == "function" && J.set(u, Me), Me;
                  }
                while (Pe >= 1 && Oe >= 0);
              break;
            }
        }
      } finally {
        ie = !1, b.current = pe, k(), Error.prepareStackTrace = se;
      }
      var ut = u ? u.displayName || u.name : "", Gr = ut ? S(ut) : "";
      return typeof u == "function" && J.set(u, Gr), Gr;
    }
    function de(u, O, E) {
      return P(u, !1);
    }
    function x(u) {
      var O = u.prototype;
      return !!(O && O.isReactComponent);
    }
    function D(u, O, E) {
      if (u == null)
        return "";
      if (typeof u == "function")
        return P(u, x(u));
      if (typeof u == "string")
        return S(u);
      switch (u) {
        case c:
          return S("Suspense");
        case f:
          return S("SuspenseList");
      }
      if (typeof u == "object")
        switch (u.$$typeof) {
          case l:
            return de(u.render);
          case d:
            return D(u.type, O, E);
          case g: {
            var B = u, se = B._payload, pe = B._init;
            try {
              return D(pe(se), O, E);
            } catch {
            }
          }
        }
      return "";
    }
    var ae = Object.prototype.hasOwnProperty, _ = {}, Ae = A.ReactDebugCurrentFrame;
    function Ce(u) {
      if (u) {
        var O = u._owner, E = D(u.type, u._source, O ? O.type : null);
        Ae.setExtraStackFrame(E);
      } else
        Ae.setExtraStackFrame(null);
    }
    function we(u, O, E, B, se) {
      {
        var pe = Function.call.bind(ae);
        for (var oe in u)
          if (pe(u, oe)) {
            var $ = void 0;
            try {
              if (typeof u[oe] != "function") {
                var Le = Error((B || "React class") + ": " + E + " type `" + oe + "` is invalid; it must be a function, usually from the `prop-types` package, but received `" + typeof u[oe] + "`.This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.");
                throw Le.name = "Invariant Violation", Le;
              }
              $ = u[oe](O, oe, B, E, null, "SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED");
            } catch (Pe) {
              $ = Pe;
            }
            $ && !($ instanceof Error) && (Ce(se), w("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", B || "React class", E, oe, typeof $), Ce(null)), $ instanceof Error && !($.message in _) && (_[$.message] = !0, Ce(se), w("Failed %s type: %s", E, $.message), Ce(null));
          }
      }
    }
    var Ge = Array.isArray;
    function Be(u) {
      return Ge(u);
    }
    function Ee(u) {
      {
        var O = typeof Symbol == "function" && Symbol.toStringTag, E = O && u[Symbol.toStringTag] || u.constructor.name || "Object";
        return E;
      }
    }
    function R(u) {
      try {
        return me(u), !1;
      } catch {
        return !0;
      }
    }
    function me(u) {
      return "" + u;
    }
    function Ue(u) {
      if (R(u))
        return w("The provided key is an unsupported type %s. This value must be coerced to a string before before using it here.", Ee(u)), me(u);
    }
    var Fe = A.ReactCurrentOwner, ke = {
      key: !0,
      ref: !0,
      __self: !0,
      __source: !0
    }, zt, it, st;
    st = {};
    function wo(u) {
      if (ae.call(u, "ref")) {
        var O = Object.getOwnPropertyDescriptor(u, "ref").get;
        if (O && O.isReactWarning)
          return !1;
      }
      return u.ref !== void 0;
    }
    function Po(u) {
      if (ae.call(u, "key")) {
        var O = Object.getOwnPropertyDescriptor(u, "key").get;
        if (O && O.isReactWarning)
          return !1;
      }
      return u.key !== void 0;
    }
    function To(u, O) {
      if (typeof u.ref == "string" && Fe.current && O && Fe.current.stateNode !== O) {
        var E = F(Fe.current.type);
        st[E] || (w('Component "%s" contains the string ref "%s". Support for string refs will be removed in a future major release. This case cannot be automatically converted to an arrow function. We ask you to manually fix this case by using useRef() or createRef() instead. Learn more about using refs safely here: https://reactjs.org/link/strict-mode-string-ref', F(Fe.current.type), u.ref), st[E] = !0);
      }
    }
    function Oo(u, O) {
      {
        var E = function() {
          zt || (zt = !0, w("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", O));
        };
        E.isReactWarning = !0, Object.defineProperty(u, "key", {
          get: E,
          configurable: !0
        });
      }
    }
    function So(u, O) {
      {
        var E = function() {
          it || (it = !0, w("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", O));
        };
        E.isReactWarning = !0, Object.defineProperty(u, "ref", {
          get: E,
          configurable: !0
        });
      }
    }
    var Xo = function(u, O, E, B, se, pe, oe) {
      var $ = {
        // This tag allows us to uniquely identify this as a React Element
        $$typeof: t,
        // Built-in properties that belong on the element
        type: u,
        key: O,
        ref: E,
        props: oe,
        // Record the component responsible for creating this element.
        _owner: pe
      };
      return $._store = {}, Object.defineProperty($._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: !1
      }), Object.defineProperty($, "_self", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: B
      }), Object.defineProperty($, "_source", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: se
      }), Object.freeze && (Object.freeze($.props), Object.freeze($)), $;
    };
    function No(u, O, E, B, se) {
      {
        var pe, oe = {}, $ = null, Le = null;
        E !== void 0 && (Ue(E), $ = "" + E), Po(O) && (Ue(O.key), $ = "" + O.key), wo(O) && (Le = O.ref, To(O, se));
        for (pe in O)
          ae.call(O, pe) && !ke.hasOwnProperty(pe) && (oe[pe] = O[pe]);
        if (u && u.defaultProps) {
          var Pe = u.defaultProps;
          for (pe in Pe)
            oe[pe] === void 0 && (oe[pe] = Pe[pe]);
        }
        if ($ || Le) {
          var Oe = typeof u == "function" ? u.displayName || u.name || "Unknown" : u;
          $ && Oo(oe, Oe), Le && So(oe, Oe);
        }
        return Xo(u, $, Le, se, B, Fe.current, oe);
      }
    }
    var nr = A.ReactCurrentOwner, Br = A.ReactDebugCurrentFrame;
    function lt(u) {
      if (u) {
        var O = u._owner, E = D(u.type, u._source, O ? O.type : null);
        Br.setExtraStackFrame(E);
      } else
        Br.setExtraStackFrame(null);
    }
    var or;
    or = !1;
    function ar(u) {
      return typeof u == "object" && u !== null && u.$$typeof === t;
    }
    function jr() {
      {
        if (nr.current) {
          var u = F(nr.current.type);
          if (u)
            return `

Check the render method of \`` + u + "`.";
        }
        return "";
      }
    }
    function qo(u) {
      {
        if (u !== void 0) {
          var O = u.fileName.replace(/^.*[\\\/]/, ""), E = u.lineNumber;
          return `

Check your code at ` + O + ":" + E + ".";
        }
        return "";
      }
    }
    var Yr = {};
    function xo(u) {
      {
        var O = jr();
        if (!O) {
          var E = typeof u == "string" ? u : u.displayName || u.name;
          E && (O = `

Check the top-level render call using <` + E + ">.");
        }
        return O;
      }
    }
    function Zr(u, O) {
      {
        if (!u._store || u._store.validated || u.key != null)
          return;
        u._store.validated = !0;
        var E = xo(O);
        if (Yr[E])
          return;
        Yr[E] = !0;
        var B = "";
        u && u._owner && u._owner !== nr.current && (B = " It was passed a child from " + F(u._owner.type) + "."), lt(u), w('Each child in a list should have a unique "key" prop.%s%s See https://reactjs.org/link/warning-keys for more information.', E, B), lt(null);
      }
    }
    function Hr(u, O) {
      {
        if (typeof u != "object")
          return;
        if (Be(u))
          for (var E = 0; E < u.length; E++) {
            var B = u[E];
            ar(B) && Zr(B, O);
          }
        else if (ar(u))
          u._store && (u._store.validated = !0);
        else if (u) {
          var se = p(u);
          if (typeof se == "function" && se !== u.entries)
            for (var pe = se.call(u), oe; !(oe = pe.next()).done; )
              ar(oe.value) && Zr(oe.value, O);
        }
      }
    }
    function Eo(u) {
      {
        var O = u.type;
        if (O == null || typeof O == "string")
          return;
        var E;
        if (typeof O == "function")
          E = O.propTypes;
        else if (typeof O == "object" && (O.$$typeof === l || // Note: Memo only checks outer props here.
        // Inner props are checked in the reconciler.
        O.$$typeof === d))
          E = O.propTypes;
        else
          return;
        if (E) {
          var B = F(O);
          we(E, u.props, "prop", B, u);
        } else if (O.PropTypes !== void 0 && !or) {
          or = !0;
          var se = F(O);
          w("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", se || "Unknown");
        }
        typeof O.getDefaultProps == "function" && !O.getDefaultProps.isReactClassApproved && w("getDefaultProps is only used on classic React.createClass definitions. Use a static property named `defaultProps` instead.");
      }
    }
    function Lo(u) {
      {
        for (var O = Object.keys(u.props), E = 0; E < O.length; E++) {
          var B = O[E];
          if (B !== "children" && B !== "key") {
            lt(u), w("Invalid prop `%s` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.", B), lt(null);
            break;
          }
        }
        u.ref !== null && (lt(u), w("Invalid attribute `ref` supplied to `React.Fragment`."), lt(null));
      }
    }
    function Ur(u, O, E, B, se, pe) {
      {
        var oe = C(u);
        if (!oe) {
          var $ = "";
          (u === void 0 || typeof u == "object" && u !== null && Object.keys(u).length === 0) && ($ += " You likely forgot to export your component from the file it's defined in, or you might have mixed up default and named imports.");
          var Le = qo(se);
          Le ? $ += Le : $ += jr();
          var Pe;
          u === null ? Pe = "null" : Be(u) ? Pe = "array" : u !== void 0 && u.$$typeof === t ? (Pe = "<" + (F(u.type) || "Unknown") + " />", $ = " Did you accidentally export a JSX literal instead of a component?") : Pe = typeof u, w("React.jsx: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: %s.%s", Pe, $);
        }
        var Oe = No(u, O, E, se, pe);
        if (Oe == null)
          return Oe;
        if (oe) {
          var Me = O.children;
          if (Me !== void 0)
            if (B)
              if (Be(Me)) {
                for (var ut = 0; ut < Me.length; ut++)
                  Hr(Me[ut], u);
                Object.freeze && Object.freeze(Me);
              } else
                w("React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead.");
            else
              Hr(Me, u);
        }
        return u === n ? Lo(Oe) : Eo(Oe), Oe;
      }
    }
    function zo(u, O, E) {
      return Ur(u, O, E, !0);
    }
    function Wo(u, O, E) {
      return Ur(u, O, E, !1);
    }
    var Vo = Wo, Co = zo;
    Pt.Fragment = n, Pt.jsx = Vo, Pt.jsxs = Co;
  }()), Pt;
}
(function(e) {
  process.env.NODE_ENV === "production" ? e.exports = Mo() : e.exports = Ro();
})(ko);
const Fn = Nt.Fragment, U = Nt.jsx, ze = Nt.jsxs;
var qt = {}, Do = {
  get exports() {
    return qt;
  },
  set exports(e) {
    qt = e;
  }
}, le = {};
/**
 * @license React
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Qr;
function Bo() {
  if (Qr)
    return le;
  Qr = 1;
  var e = Symbol.for("react.element"), t = Symbol.for("react.portal"), r = Symbol.for("react.fragment"), n = Symbol.for("react.strict_mode"), o = Symbol.for("react.profiler"), i = Symbol.for("react.provider"), a = Symbol.for("react.context"), s = Symbol.for("react.server_context"), l = Symbol.for("react.forward_ref"), c = Symbol.for("react.suspense"), f = Symbol.for("react.suspense_list"), d = Symbol.for("react.memo"), g = Symbol.for("react.lazy"), h = Symbol.for("react.offscreen"), v;
  v = Symbol.for("react.module.reference");
  function y(p) {
    if (typeof p == "object" && p !== null) {
      var A = p.$$typeof;
      switch (A) {
        case e:
          switch (p = p.type, p) {
            case r:
            case o:
            case n:
            case c:
            case f:
              return p;
            default:
              switch (p = p && p.$$typeof, p) {
                case s:
                case a:
                case l:
                case g:
                case d:
                case i:
                  return p;
                default:
                  return A;
              }
          }
        case t:
          return A;
      }
    }
  }
  return le.ContextConsumer = a, le.ContextProvider = i, le.Element = e, le.ForwardRef = l, le.Fragment = r, le.Lazy = g, le.Memo = d, le.Portal = t, le.Profiler = o, le.StrictMode = n, le.Suspense = c, le.SuspenseList = f, le.isAsyncMode = function() {
    return !1;
  }, le.isConcurrentMode = function() {
    return !1;
  }, le.isContextConsumer = function(p) {
    return y(p) === a;
  }, le.isContextProvider = function(p) {
    return y(p) === i;
  }, le.isElement = function(p) {
    return typeof p == "object" && p !== null && p.$$typeof === e;
  }, le.isForwardRef = function(p) {
    return y(p) === l;
  }, le.isFragment = function(p) {
    return y(p) === r;
  }, le.isLazy = function(p) {
    return y(p) === g;
  }, le.isMemo = function(p) {
    return y(p) === d;
  }, le.isPortal = function(p) {
    return y(p) === t;
  }, le.isProfiler = function(p) {
    return y(p) === o;
  }, le.isStrictMode = function(p) {
    return y(p) === n;
  }, le.isSuspense = function(p) {
    return y(p) === c;
  }, le.isSuspenseList = function(p) {
    return y(p) === f;
  }, le.isValidElementType = function(p) {
    return typeof p == "string" || typeof p == "function" || p === r || p === o || p === n || p === c || p === f || p === h || typeof p == "object" && p !== null && (p.$$typeof === g || p.$$typeof === d || p.$$typeof === i || p.$$typeof === a || p.$$typeof === l || p.$$typeof === v || p.getModuleId !== void 0);
  }, le.typeOf = y, le;
}
var ue = {};
/**
 * @license React
 * react-is.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var _r;
function jo() {
  return _r || (_r = 1, process.env.NODE_ENV !== "production" && function() {
    var e = Symbol.for("react.element"), t = Symbol.for("react.portal"), r = Symbol.for("react.fragment"), n = Symbol.for("react.strict_mode"), o = Symbol.for("react.profiler"), i = Symbol.for("react.provider"), a = Symbol.for("react.context"), s = Symbol.for("react.server_context"), l = Symbol.for("react.forward_ref"), c = Symbol.for("react.suspense"), f = Symbol.for("react.suspense_list"), d = Symbol.for("react.memo"), g = Symbol.for("react.lazy"), h = Symbol.for("react.offscreen"), v = !1, y = !1, p = !1, A = !1, w = !1, X;
    X = Symbol.for("react.module.reference");
    function T(x) {
      return !!(typeof x == "string" || typeof x == "function" || x === r || x === o || w || x === n || x === c || x === f || A || x === h || v || y || p || typeof x == "object" && x !== null && (x.$$typeof === g || x.$$typeof === d || x.$$typeof === i || x.$$typeof === a || x.$$typeof === l || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      x.$$typeof === X || x.getModuleId !== void 0));
    }
    function m(x) {
      if (typeof x == "object" && x !== null) {
        var D = x.$$typeof;
        switch (D) {
          case e:
            var ae = x.type;
            switch (ae) {
              case r:
              case o:
              case n:
              case c:
              case f:
                return ae;
              default:
                var _ = ae && ae.$$typeof;
                switch (_) {
                  case s:
                  case a:
                  case l:
                  case g:
                  case d:
                  case i:
                    return _;
                  default:
                    return D;
                }
            }
          case t:
            return D;
        }
      }
    }
    var M = a, Z = i, z = e, W = l, C = r, Y = g, j = d, F = t, H = o, G = n, ne = c, re = f, I = !1, ve = !1;
    function he(x) {
      return I || (I = !0, console.warn("The ReactIs.isAsyncMode() alias has been deprecated, and will be removed in React 18+.")), !1;
    }
    function Ne(x) {
      return ve || (ve = !0, console.warn("The ReactIs.isConcurrentMode() alias has been deprecated, and will be removed in React 18+.")), !1;
    }
    function L(x) {
      return m(x) === a;
    }
    function N(x) {
      return m(x) === i;
    }
    function q(x) {
      return typeof x == "object" && x !== null && x.$$typeof === e;
    }
    function k(x) {
      return m(x) === l;
    }
    function b(x) {
      return m(x) === r;
    }
    function K(x) {
      return m(x) === g;
    }
    function S(x) {
      return m(x) === d;
    }
    function ie(x) {
      return m(x) === t;
    }
    function J(x) {
      return m(x) === o;
    }
    function Q(x) {
      return m(x) === n;
    }
    function P(x) {
      return m(x) === c;
    }
    function de(x) {
      return m(x) === f;
    }
    ue.ContextConsumer = M, ue.ContextProvider = Z, ue.Element = z, ue.ForwardRef = W, ue.Fragment = C, ue.Lazy = Y, ue.Memo = j, ue.Portal = F, ue.Profiler = H, ue.StrictMode = G, ue.Suspense = ne, ue.SuspenseList = re, ue.isAsyncMode = he, ue.isConcurrentMode = Ne, ue.isContextConsumer = L, ue.isContextProvider = N, ue.isElement = q, ue.isForwardRef = k, ue.isFragment = b, ue.isLazy = K, ue.isMemo = S, ue.isPortal = ie, ue.isProfiler = J, ue.isStrictMode = Q, ue.isSuspense = P, ue.isSuspenseList = de, ue.isValidElementType = T, ue.typeOf = m;
  }()), ue;
}
(function(e) {
  process.env.NODE_ENV === "production" ? e.exports = Bo() : e.exports = jo();
})(Do);
function Yo(e) {
  function t(L, N, q, k, b) {
    for (var K = 0, S = 0, ie = 0, J = 0, Q, P, de = 0, x = 0, D, ae = D = Q = 0, _ = 0, Ae = 0, Ce = 0, we = 0, Ge = q.length, Be = Ge - 1, Ee, R = "", me = "", Ue = "", Fe = "", ke; _ < Ge; ) {
      if (P = q.charCodeAt(_), _ === Be && S + J + ie + K !== 0 && (S !== 0 && (P = S === 47 ? 10 : 47), J = ie = K = 0, Ge++, Be++), S + J + ie + K === 0) {
        if (_ === Be && (0 < Ae && (R = R.replace(g, "")), 0 < R.trim().length)) {
          switch (P) {
            case 32:
            case 9:
            case 59:
            case 13:
            case 10:
              break;
            default:
              R += q.charAt(_);
          }
          P = 59;
        }
        switch (P) {
          case 123:
            for (R = R.trim(), Q = R.charCodeAt(0), D = 1, we = ++_; _ < Ge; ) {
              switch (P = q.charCodeAt(_)) {
                case 123:
                  D++;
                  break;
                case 125:
                  D--;
                  break;
                case 47:
                  switch (P = q.charCodeAt(_ + 1)) {
                    case 42:
                    case 47:
                      e: {
                        for (ae = _ + 1; ae < Be; ++ae)
                          switch (q.charCodeAt(ae)) {
                            case 47:
                              if (P === 42 && q.charCodeAt(ae - 1) === 42 && _ + 2 !== ae) {
                                _ = ae + 1;
                                break e;
                              }
                              break;
                            case 10:
                              if (P === 47) {
                                _ = ae + 1;
                                break e;
                              }
                          }
                        _ = ae;
                      }
                  }
                  break;
                case 91:
                  P++;
                case 40:
                  P++;
                case 34:
                case 39:
                  for (; _++ < Be && q.charCodeAt(_) !== P; )
                    ;
              }
              if (D === 0)
                break;
              _++;
            }
            switch (D = q.substring(we, _), Q === 0 && (Q = (R = R.replace(d, "").trim()).charCodeAt(0)), Q) {
              case 64:
                switch (0 < Ae && (R = R.replace(g, "")), P = R.charCodeAt(1), P) {
                  case 100:
                  case 109:
                  case 115:
                  case 45:
                    Ae = N;
                    break;
                  default:
                    Ae = ne;
                }
                if (D = t(N, Ae, D, P, b + 1), we = D.length, 0 < I && (Ae = r(ne, R, Ce), ke = s(3, D, Ae, N, F, j, we, P, b, k), R = Ae.join(""), ke !== void 0 && (we = (D = ke.trim()).length) === 0 && (P = 0, D = "")), 0 < we)
                  switch (P) {
                    case 115:
                      R = R.replace(M, a);
                    case 100:
                    case 109:
                    case 45:
                      D = R + "{" + D + "}";
                      break;
                    case 107:
                      R = R.replace(w, "$1 $2"), D = R + "{" + D + "}", D = G === 1 || G === 2 && i("@" + D, 3) ? "@-webkit-" + D + "@" + D : "@" + D;
                      break;
                    default:
                      D = R + D, k === 112 && (D = (me += D, ""));
                  }
                else
                  D = "";
                break;
              default:
                D = t(N, r(N, R, Ce), D, k, b + 1);
            }
            Ue += D, D = Ce = Ae = ae = Q = 0, R = "", P = q.charCodeAt(++_);
            break;
          case 125:
          case 59:
            if (R = (0 < Ae ? R.replace(g, "") : R).trim(), 1 < (we = R.length))
              switch (ae === 0 && (Q = R.charCodeAt(0), Q === 45 || 96 < Q && 123 > Q) && (we = (R = R.replace(" ", ":")).length), 0 < I && (ke = s(1, R, N, L, F, j, me.length, k, b, k)) !== void 0 && (we = (R = ke.trim()).length) === 0 && (R = "\0\0"), Q = R.charCodeAt(0), P = R.charCodeAt(1), Q) {
                case 0:
                  break;
                case 64:
                  if (P === 105 || P === 99) {
                    Fe += R + q.charAt(_);
                    break;
                  }
                default:
                  R.charCodeAt(we - 1) !== 58 && (me += o(R, Q, P, R.charCodeAt(2)));
              }
            Ce = Ae = ae = Q = 0, R = "", P = q.charCodeAt(++_);
        }
      }
      switch (P) {
        case 13:
        case 10:
          S === 47 ? S = 0 : 1 + Q === 0 && k !== 107 && 0 < R.length && (Ae = 1, R += "\0"), 0 < I * he && s(0, R, N, L, F, j, me.length, k, b, k), j = 1, F++;
          break;
        case 59:
        case 125:
          if (S + J + ie + K === 0) {
            j++;
            break;
          }
        default:
          switch (j++, Ee = q.charAt(_), P) {
            case 9:
            case 32:
              if (J + K + S === 0)
                switch (de) {
                  case 44:
                  case 58:
                  case 9:
                  case 32:
                    Ee = "";
                    break;
                  default:
                    P !== 32 && (Ee = " ");
                }
              break;
            case 0:
              Ee = "\\0";
              break;
            case 12:
              Ee = "\\f";
              break;
            case 11:
              Ee = "\\v";
              break;
            case 38:
              J + S + K === 0 && (Ae = Ce = 1, Ee = "\f" + Ee);
              break;
            case 108:
              if (J + S + K + H === 0 && 0 < ae)
                switch (_ - ae) {
                  case 2:
                    de === 112 && q.charCodeAt(_ - 3) === 58 && (H = de);
                  case 8:
                    x === 111 && (H = x);
                }
              break;
            case 58:
              J + S + K === 0 && (ae = _);
              break;
            case 44:
              S + ie + J + K === 0 && (Ae = 1, Ee += "\r");
              break;
            case 34:
            case 39:
              S === 0 && (J = J === P ? 0 : J === 0 ? P : J);
              break;
            case 91:
              J + S + ie === 0 && K++;
              break;
            case 93:
              J + S + ie === 0 && K--;
              break;
            case 41:
              J + S + K === 0 && ie--;
              break;
            case 40:
              if (J + S + K === 0) {
                if (Q === 0)
                  switch (2 * de + 3 * x) {
                    case 533:
                      break;
                    default:
                      Q = 1;
                  }
                ie++;
              }
              break;
            case 64:
              S + ie + J + K + ae + D === 0 && (D = 1);
              break;
            case 42:
            case 47:
              if (!(0 < J + K + ie))
                switch (S) {
                  case 0:
                    switch (2 * P + 3 * q.charCodeAt(_ + 1)) {
                      case 235:
                        S = 47;
                        break;
                      case 220:
                        we = _, S = 42;
                    }
                    break;
                  case 42:
                    P === 47 && de === 42 && we + 2 !== _ && (q.charCodeAt(we + 2) === 33 && (me += q.substring(we, _ + 1)), Ee = "", S = 0);
                }
          }
          S === 0 && (R += Ee);
      }
      x = de, de = P, _++;
    }
    if (we = me.length, 0 < we) {
      if (Ae = N, 0 < I && (ke = s(2, me, Ae, L, F, j, we, k, b, k), ke !== void 0 && (me = ke).length === 0))
        return Fe + me + Ue;
      if (me = Ae.join(",") + "{" + me + "}", G * H !== 0) {
        switch (G !== 2 || i(me, 2) || (H = 0), H) {
          case 111:
            me = me.replace(T, ":-moz-$1") + me;
            break;
          case 112:
            me = me.replace(X, "::-webkit-input-$1") + me.replace(X, "::-moz-$1") + me.replace(X, ":-ms-input-$1") + me;
        }
        H = 0;
      }
    }
    return Fe + me + Ue;
  }
  function r(L, N, q) {
    var k = N.trim().split(p);
    N = k;
    var b = k.length, K = L.length;
    switch (K) {
      case 0:
      case 1:
        var S = 0;
        for (L = K === 0 ? "" : L[0] + " "; S < b; ++S)
          N[S] = n(L, N[S], q).trim();
        break;
      default:
        var ie = S = 0;
        for (N = []; S < b; ++S)
          for (var J = 0; J < K; ++J)
            N[ie++] = n(L[J] + " ", k[S], q).trim();
    }
    return N;
  }
  function n(L, N, q) {
    var k = N.charCodeAt(0);
    switch (33 > k && (k = (N = N.trim()).charCodeAt(0)), k) {
      case 38:
        return N.replace(A, "$1" + L.trim());
      case 58:
        return L.trim() + N.replace(A, "$1" + L.trim());
      default:
        if (0 < 1 * q && 0 < N.indexOf("\f"))
          return N.replace(A, (L.charCodeAt(0) === 58 ? "" : "$1") + L.trim());
    }
    return L + N;
  }
  function o(L, N, q, k) {
    var b = L + ";", K = 2 * N + 3 * q + 4 * k;
    if (K === 944) {
      L = b.indexOf(":", 9) + 1;
      var S = b.substring(L, b.length - 1).trim();
      return S = b.substring(0, L).trim() + S + ";", G === 1 || G === 2 && i(S, 1) ? "-webkit-" + S + S : S;
    }
    if (G === 0 || G === 2 && !i(b, 1))
      return b;
    switch (K) {
      case 1015:
        return b.charCodeAt(10) === 97 ? "-webkit-" + b + b : b;
      case 951:
        return b.charCodeAt(3) === 116 ? "-webkit-" + b + b : b;
      case 963:
        return b.charCodeAt(5) === 110 ? "-webkit-" + b + b : b;
      case 1009:
        if (b.charCodeAt(4) !== 100)
          break;
      case 969:
      case 942:
        return "-webkit-" + b + b;
      case 978:
        return "-webkit-" + b + "-moz-" + b + b;
      case 1019:
      case 983:
        return "-webkit-" + b + "-moz-" + b + "-ms-" + b + b;
      case 883:
        if (b.charCodeAt(8) === 45)
          return "-webkit-" + b + b;
        if (0 < b.indexOf("image-set(", 11))
          return b.replace(Y, "$1-webkit-$2") + b;
        break;
      case 932:
        if (b.charCodeAt(4) === 45)
          switch (b.charCodeAt(5)) {
            case 103:
              return "-webkit-box-" + b.replace("-grow", "") + "-webkit-" + b + "-ms-" + b.replace("grow", "positive") + b;
            case 115:
              return "-webkit-" + b + "-ms-" + b.replace("shrink", "negative") + b;
            case 98:
              return "-webkit-" + b + "-ms-" + b.replace("basis", "preferred-size") + b;
          }
        return "-webkit-" + b + "-ms-" + b + b;
      case 964:
        return "-webkit-" + b + "-ms-flex-" + b + b;
      case 1023:
        if (b.charCodeAt(8) !== 99)
          break;
        return S = b.substring(b.indexOf(":", 15)).replace("flex-", "").replace("space-between", "justify"), "-webkit-box-pack" + S + "-webkit-" + b + "-ms-flex-pack" + S + b;
      case 1005:
        return v.test(b) ? b.replace(h, ":-webkit-") + b.replace(h, ":-moz-") + b : b;
      case 1e3:
        switch (S = b.substring(13).trim(), N = S.indexOf("-") + 1, S.charCodeAt(0) + S.charCodeAt(N)) {
          case 226:
            S = b.replace(m, "tb");
            break;
          case 232:
            S = b.replace(m, "tb-rl");
            break;
          case 220:
            S = b.replace(m, "lr");
            break;
          default:
            return b;
        }
        return "-webkit-" + b + "-ms-" + S + b;
      case 1017:
        if (b.indexOf("sticky", 9) === -1)
          break;
      case 975:
        switch (N = (b = L).length - 10, S = (b.charCodeAt(N) === 33 ? b.substring(0, N) : b).substring(L.indexOf(":", 7) + 1).trim(), K = S.charCodeAt(0) + (S.charCodeAt(7) | 0)) {
          case 203:
            if (111 > S.charCodeAt(8))
              break;
          case 115:
            b = b.replace(S, "-webkit-" + S) + ";" + b;
            break;
          case 207:
          case 102:
            b = b.replace(S, "-webkit-" + (102 < K ? "inline-" : "") + "box") + ";" + b.replace(S, "-webkit-" + S) + ";" + b.replace(S, "-ms-" + S + "box") + ";" + b;
        }
        return b + ";";
      case 938:
        if (b.charCodeAt(5) === 45)
          switch (b.charCodeAt(6)) {
            case 105:
              return S = b.replace("-items", ""), "-webkit-" + b + "-webkit-box-" + S + "-ms-flex-" + S + b;
            case 115:
              return "-webkit-" + b + "-ms-flex-item-" + b.replace(z, "") + b;
            default:
              return "-webkit-" + b + "-ms-flex-line-pack" + b.replace("align-content", "").replace(z, "") + b;
          }
        break;
      case 973:
      case 989:
        if (b.charCodeAt(3) !== 45 || b.charCodeAt(4) === 122)
          break;
      case 931:
      case 953:
        if (C.test(L) === !0)
          return (S = L.substring(L.indexOf(":") + 1)).charCodeAt(0) === 115 ? o(L.replace("stretch", "fill-available"), N, q, k).replace(":fill-available", ":stretch") : b.replace(S, "-webkit-" + S) + b.replace(S, "-moz-" + S.replace("fill-", "")) + b;
        break;
      case 962:
        if (b = "-webkit-" + b + (b.charCodeAt(5) === 102 ? "-ms-" + b : "") + b, q + k === 211 && b.charCodeAt(13) === 105 && 0 < b.indexOf("transform", 10))
          return b.substring(0, b.indexOf(";", 27) + 1).replace(y, "$1-webkit-$2") + b;
    }
    return b;
  }
  function i(L, N) {
    var q = L.indexOf(N === 1 ? ":" : "{"), k = L.substring(0, N !== 3 ? q : 10);
    return q = L.substring(q + 1, L.length - 1), ve(N !== 2 ? k : k.replace(W, "$1"), q, N);
  }
  function a(L, N) {
    var q = o(N, N.charCodeAt(0), N.charCodeAt(1), N.charCodeAt(2));
    return q !== N + ";" ? q.replace(Z, " or ($1)").substring(4) : "(" + N + ")";
  }
  function s(L, N, q, k, b, K, S, ie, J, Q) {
    for (var P = 0, de = N, x; P < I; ++P)
      switch (x = re[P].call(f, L, de, q, k, b, K, S, ie, J, Q)) {
        case void 0:
        case !1:
        case !0:
        case null:
          break;
        default:
          de = x;
      }
    if (de !== N)
      return de;
  }
  function l(L) {
    switch (L) {
      case void 0:
      case null:
        I = re.length = 0;
        break;
      default:
        if (typeof L == "function")
          re[I++] = L;
        else if (typeof L == "object")
          for (var N = 0, q = L.length; N < q; ++N)
            l(L[N]);
        else
          he = !!L | 0;
    }
    return l;
  }
  function c(L) {
    return L = L.prefix, L !== void 0 && (ve = null, L ? typeof L != "function" ? G = 1 : (G = 2, ve = L) : G = 0), c;
  }
  function f(L, N) {
    var q = L;
    if (33 > q.charCodeAt(0) && (q = q.trim()), Ne = q, q = [Ne], 0 < I) {
      var k = s(-1, N, q, q, F, j, 0, 0, 0, 0);
      k !== void 0 && typeof k == "string" && (N = k);
    }
    var b = t(ne, q, N, 0, 0);
    return 0 < I && (k = s(-2, b, q, q, F, j, b.length, 0, 0, 0), k !== void 0 && (b = k)), Ne = "", H = 0, j = F = 1, b;
  }
  var d = /^\0+/g, g = /[\0\r\f]/g, h = /: */g, v = /zoo|gra/, y = /([,: ])(transform)/g, p = /,\r+?/g, A = /([\t\r\n ])*\f?&/g, w = /@(k\w+)\s*(\S*)\s*/, X = /::(place)/g, T = /:(read-only)/g, m = /[svh]\w+-[tblr]{2}/, M = /\(\s*(.*)\s*\)/g, Z = /([\s\S]*?);/g, z = /-self|flex-/g, W = /[^]*?(:[rp][el]a[\w-]+)[^]*/, C = /stretch|:\s*\w+\-(?:conte|avail)/, Y = /([^-])(image-set\()/, j = 1, F = 1, H = 0, G = 1, ne = [], re = [], I = 0, ve = null, he = 0, Ne = "";
  return f.use = l, f.set = c, e !== void 0 && c(e), f;
}
var Zo = {
  animationIterationCount: 1,
  borderImageOutset: 1,
  borderImageSlice: 1,
  borderImageWidth: 1,
  boxFlex: 1,
  boxFlexGroup: 1,
  boxOrdinalGroup: 1,
  columnCount: 1,
  columns: 1,
  flex: 1,
  flexGrow: 1,
  flexPositive: 1,
  flexShrink: 1,
  flexNegative: 1,
  flexOrder: 1,
  gridRow: 1,
  gridRowEnd: 1,
  gridRowSpan: 1,
  gridRowStart: 1,
  gridColumn: 1,
  gridColumnEnd: 1,
  gridColumnSpan: 1,
  gridColumnStart: 1,
  msGridRow: 1,
  msGridRowSpan: 1,
  msGridColumn: 1,
  msGridColumnSpan: 1,
  fontWeight: 1,
  lineHeight: 1,
  opacity: 1,
  order: 1,
  orphans: 1,
  tabSize: 1,
  widows: 1,
  zIndex: 1,
  zoom: 1,
  WebkitLineClamp: 1,
  // SVG-related properties
  fillOpacity: 1,
  floodOpacity: 1,
  stopOpacity: 1,
  strokeDasharray: 1,
  strokeDashoffset: 1,
  strokeMiterlimit: 1,
  strokeOpacity: 1,
  strokeWidth: 1
};
function Ho(e) {
  var t = /* @__PURE__ */ Object.create(null);
  return function(r) {
    return t[r] === void 0 && (t[r] = e(r)), t[r];
  };
}
var Uo = /^((children|dangerouslySetInnerHTML|key|ref|autoFocus|defaultValue|defaultChecked|innerHTML|suppressContentEditableWarning|suppressHydrationWarning|valueLink|abbr|accept|acceptCharset|accessKey|action|allow|allowUserMedia|allowPaymentRequest|allowFullScreen|allowTransparency|alt|async|autoComplete|autoPlay|capture|cellPadding|cellSpacing|challenge|charSet|checked|cite|classID|className|cols|colSpan|content|contentEditable|contextMenu|controls|controlsList|coords|crossOrigin|data|dateTime|decoding|default|defer|dir|disabled|disablePictureInPicture|download|draggable|encType|enterKeyHint|form|formAction|formEncType|formMethod|formNoValidate|formTarget|frameBorder|headers|height|hidden|high|href|hrefLang|htmlFor|httpEquiv|id|inputMode|integrity|is|keyParams|keyType|kind|label|lang|list|loading|loop|low|marginHeight|marginWidth|max|maxLength|media|mediaGroup|method|min|minLength|multiple|muted|name|nonce|noValidate|open|optimum|pattern|placeholder|playsInline|poster|preload|profile|radioGroup|readOnly|referrerPolicy|rel|required|reversed|role|rows|rowSpan|sandbox|scope|scoped|scrolling|seamless|selected|shape|size|sizes|slot|span|spellCheck|src|srcDoc|srcLang|srcSet|start|step|style|summary|tabIndex|target|title|translate|type|useMap|value|width|wmode|wrap|about|datatype|inlist|prefix|property|resource|typeof|vocab|autoCapitalize|autoCorrect|autoSave|color|incremental|fallback|inert|itemProp|itemScope|itemType|itemID|itemRef|on|option|results|security|unselectable|accentHeight|accumulate|additive|alignmentBaseline|allowReorder|alphabetic|amplitude|arabicForm|ascent|attributeName|attributeType|autoReverse|azimuth|baseFrequency|baselineShift|baseProfile|bbox|begin|bias|by|calcMode|capHeight|clip|clipPathUnits|clipPath|clipRule|colorInterpolation|colorInterpolationFilters|colorProfile|colorRendering|contentScriptType|contentStyleType|cursor|cx|cy|d|decelerate|descent|diffuseConstant|direction|display|divisor|dominantBaseline|dur|dx|dy|edgeMode|elevation|enableBackground|end|exponent|externalResourcesRequired|fill|fillOpacity|fillRule|filter|filterRes|filterUnits|floodColor|floodOpacity|focusable|fontFamily|fontSize|fontSizeAdjust|fontStretch|fontStyle|fontVariant|fontWeight|format|from|fr|fx|fy|g1|g2|glyphName|glyphOrientationHorizontal|glyphOrientationVertical|glyphRef|gradientTransform|gradientUnits|hanging|horizAdvX|horizOriginX|ideographic|imageRendering|in|in2|intercept|k|k1|k2|k3|k4|kernelMatrix|kernelUnitLength|kerning|keyPoints|keySplines|keyTimes|lengthAdjust|letterSpacing|lightingColor|limitingConeAngle|local|markerEnd|markerMid|markerStart|markerHeight|markerUnits|markerWidth|mask|maskContentUnits|maskUnits|mathematical|mode|numOctaves|offset|opacity|operator|order|orient|orientation|origin|overflow|overlinePosition|overlineThickness|panose1|paintOrder|pathLength|patternContentUnits|patternTransform|patternUnits|pointerEvents|points|pointsAtX|pointsAtY|pointsAtZ|preserveAlpha|preserveAspectRatio|primitiveUnits|r|radius|refX|refY|renderingIntent|repeatCount|repeatDur|requiredExtensions|requiredFeatures|restart|result|rotate|rx|ry|scale|seed|shapeRendering|slope|spacing|specularConstant|specularExponent|speed|spreadMethod|startOffset|stdDeviation|stemh|stemv|stitchTiles|stopColor|stopOpacity|strikethroughPosition|strikethroughThickness|string|stroke|strokeDasharray|strokeDashoffset|strokeLinecap|strokeLinejoin|strokeMiterlimit|strokeOpacity|strokeWidth|surfaceScale|systemLanguage|tableValues|targetX|targetY|textAnchor|textDecoration|textRendering|textLength|to|transform|u1|u2|underlinePosition|underlineThickness|unicode|unicodeBidi|unicodeRange|unitsPerEm|vAlphabetic|vHanging|vIdeographic|vMathematical|values|vectorEffect|version|vertAdvY|vertOriginX|vertOriginY|viewBox|viewTarget|visibility|widths|wordSpacing|writingMode|x|xHeight|x1|x2|xChannelSelector|xlinkActuate|xlinkArcrole|xlinkHref|xlinkRole|xlinkShow|xlinkTitle|xlinkType|xmlBase|xmlns|xmlnsXlink|xmlLang|xmlSpace|y|y1|y2|yChannelSelector|z|zoomAndPan|for|class|autofocus)|(([Dd][Aa][Tt][Aa]|[Aa][Rr][Ii][Aa]|x)-.*))$/, $r = /* @__PURE__ */ Ho(
  function(e) {
    return Uo.test(e) || e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && e.charCodeAt(2) < 91;
  }
  /* Z+1 */
), mr = {}, Go = {
  get exports() {
    return mr;
  },
  set exports(e) {
    mr = e;
  }
}, ce = {};
/** @license React v16.13.1
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var en;
function Io() {
  if (en)
    return ce;
  en = 1;
  var e = typeof Symbol == "function" && Symbol.for, t = e ? Symbol.for("react.element") : 60103, r = e ? Symbol.for("react.portal") : 60106, n = e ? Symbol.for("react.fragment") : 60107, o = e ? Symbol.for("react.strict_mode") : 60108, i = e ? Symbol.for("react.profiler") : 60114, a = e ? Symbol.for("react.provider") : 60109, s = e ? Symbol.for("react.context") : 60110, l = e ? Symbol.for("react.async_mode") : 60111, c = e ? Symbol.for("react.concurrent_mode") : 60111, f = e ? Symbol.for("react.forward_ref") : 60112, d = e ? Symbol.for("react.suspense") : 60113, g = e ? Symbol.for("react.suspense_list") : 60120, h = e ? Symbol.for("react.memo") : 60115, v = e ? Symbol.for("react.lazy") : 60116, y = e ? Symbol.for("react.block") : 60121, p = e ? Symbol.for("react.fundamental") : 60117, A = e ? Symbol.for("react.responder") : 60118, w = e ? Symbol.for("react.scope") : 60119;
  function X(m) {
    if (typeof m == "object" && m !== null) {
      var M = m.$$typeof;
      switch (M) {
        case t:
          switch (m = m.type, m) {
            case l:
            case c:
            case n:
            case i:
            case o:
            case d:
              return m;
            default:
              switch (m = m && m.$$typeof, m) {
                case s:
                case f:
                case v:
                case h:
                case a:
                  return m;
                default:
                  return M;
              }
          }
        case r:
          return M;
      }
    }
  }
  function T(m) {
    return X(m) === c;
  }
  return ce.AsyncMode = l, ce.ConcurrentMode = c, ce.ContextConsumer = s, ce.ContextProvider = a, ce.Element = t, ce.ForwardRef = f, ce.Fragment = n, ce.Lazy = v, ce.Memo = h, ce.Portal = r, ce.Profiler = i, ce.StrictMode = o, ce.Suspense = d, ce.isAsyncMode = function(m) {
    return T(m) || X(m) === l;
  }, ce.isConcurrentMode = T, ce.isContextConsumer = function(m) {
    return X(m) === s;
  }, ce.isContextProvider = function(m) {
    return X(m) === a;
  }, ce.isElement = function(m) {
    return typeof m == "object" && m !== null && m.$$typeof === t;
  }, ce.isForwardRef = function(m) {
    return X(m) === f;
  }, ce.isFragment = function(m) {
    return X(m) === n;
  }, ce.isLazy = function(m) {
    return X(m) === v;
  }, ce.isMemo = function(m) {
    return X(m) === h;
  }, ce.isPortal = function(m) {
    return X(m) === r;
  }, ce.isProfiler = function(m) {
    return X(m) === i;
  }, ce.isStrictMode = function(m) {
    return X(m) === o;
  }, ce.isSuspense = function(m) {
    return X(m) === d;
  }, ce.isValidElementType = function(m) {
    return typeof m == "string" || typeof m == "function" || m === n || m === c || m === i || m === o || m === d || m === g || typeof m == "object" && m !== null && (m.$$typeof === v || m.$$typeof === h || m.$$typeof === a || m.$$typeof === s || m.$$typeof === f || m.$$typeof === p || m.$$typeof === A || m.$$typeof === w || m.$$typeof === y);
  }, ce.typeOf = X, ce;
}
var fe = {};
/** @license React v16.13.1
 * react-is.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var tn;
function Jo() {
  return tn || (tn = 1, process.env.NODE_ENV !== "production" && function() {
    var e = typeof Symbol == "function" && Symbol.for, t = e ? Symbol.for("react.element") : 60103, r = e ? Symbol.for("react.portal") : 60106, n = e ? Symbol.for("react.fragment") : 60107, o = e ? Symbol.for("react.strict_mode") : 60108, i = e ? Symbol.for("react.profiler") : 60114, a = e ? Symbol.for("react.provider") : 60109, s = e ? Symbol.for("react.context") : 60110, l = e ? Symbol.for("react.async_mode") : 60111, c = e ? Symbol.for("react.concurrent_mode") : 60111, f = e ? Symbol.for("react.forward_ref") : 60112, d = e ? Symbol.for("react.suspense") : 60113, g = e ? Symbol.for("react.suspense_list") : 60120, h = e ? Symbol.for("react.memo") : 60115, v = e ? Symbol.for("react.lazy") : 60116, y = e ? Symbol.for("react.block") : 60121, p = e ? Symbol.for("react.fundamental") : 60117, A = e ? Symbol.for("react.responder") : 60118, w = e ? Symbol.for("react.scope") : 60119;
    function X(P) {
      return typeof P == "string" || typeof P == "function" || // Note: its typeof might be other than 'symbol' or 'number' if it's a polyfill.
      P === n || P === c || P === i || P === o || P === d || P === g || typeof P == "object" && P !== null && (P.$$typeof === v || P.$$typeof === h || P.$$typeof === a || P.$$typeof === s || P.$$typeof === f || P.$$typeof === p || P.$$typeof === A || P.$$typeof === w || P.$$typeof === y);
    }
    function T(P) {
      if (typeof P == "object" && P !== null) {
        var de = P.$$typeof;
        switch (de) {
          case t:
            var x = P.type;
            switch (x) {
              case l:
              case c:
              case n:
              case i:
              case o:
              case d:
                return x;
              default:
                var D = x && x.$$typeof;
                switch (D) {
                  case s:
                  case f:
                  case v:
                  case h:
                  case a:
                    return D;
                  default:
                    return de;
                }
            }
          case r:
            return de;
        }
      }
    }
    var m = l, M = c, Z = s, z = a, W = t, C = f, Y = n, j = v, F = h, H = r, G = i, ne = o, re = d, I = !1;
    function ve(P) {
      return I || (I = !0, console.warn("The ReactIs.isAsyncMode() alias has been deprecated, and will be removed in React 17+. Update your code to use ReactIs.isConcurrentMode() instead. It has the exact same API.")), he(P) || T(P) === l;
    }
    function he(P) {
      return T(P) === c;
    }
    function Ne(P) {
      return T(P) === s;
    }
    function L(P) {
      return T(P) === a;
    }
    function N(P) {
      return typeof P == "object" && P !== null && P.$$typeof === t;
    }
    function q(P) {
      return T(P) === f;
    }
    function k(P) {
      return T(P) === n;
    }
    function b(P) {
      return T(P) === v;
    }
    function K(P) {
      return T(P) === h;
    }
    function S(P) {
      return T(P) === r;
    }
    function ie(P) {
      return T(P) === i;
    }
    function J(P) {
      return T(P) === o;
    }
    function Q(P) {
      return T(P) === d;
    }
    fe.AsyncMode = m, fe.ConcurrentMode = M, fe.ContextConsumer = Z, fe.ContextProvider = z, fe.Element = W, fe.ForwardRef = C, fe.Fragment = Y, fe.Lazy = j, fe.Memo = F, fe.Portal = H, fe.Profiler = G, fe.StrictMode = ne, fe.Suspense = re, fe.isAsyncMode = ve, fe.isConcurrentMode = he, fe.isContextConsumer = Ne, fe.isContextProvider = L, fe.isElement = N, fe.isForwardRef = q, fe.isFragment = k, fe.isLazy = b, fe.isMemo = K, fe.isPortal = S, fe.isProfiler = ie, fe.isStrictMode = J, fe.isSuspense = Q, fe.isValidElementType = X, fe.typeOf = T;
  }()), fe;
}
(function(e) {
  process.env.NODE_ENV === "production" ? e.exports = Io() : e.exports = Jo();
})(Go);
var zr = mr, Ko = {
  childContextTypes: !0,
  contextType: !0,
  contextTypes: !0,
  defaultProps: !0,
  displayName: !0,
  getDefaultProps: !0,
  getDerivedStateFromError: !0,
  getDerivedStateFromProps: !0,
  mixins: !0,
  propTypes: !0,
  type: !0
}, Qo = {
  name: !0,
  length: !0,
  prototype: !0,
  caller: !0,
  callee: !0,
  arguments: !0,
  arity: !0
}, _o = {
  $$typeof: !0,
  render: !0,
  defaultProps: !0,
  displayName: !0,
  propTypes: !0
}, kn = {
  $$typeof: !0,
  compare: !0,
  defaultProps: !0,
  displayName: !0,
  propTypes: !0,
  type: !0
}, Wr = {};
Wr[zr.ForwardRef] = _o;
Wr[zr.Memo] = kn;
function rn(e) {
  return zr.isMemo(e) ? kn : Wr[e.$$typeof] || Ko;
}
var $o = Object.defineProperty, ea = Object.getOwnPropertyNames, nn = Object.getOwnPropertySymbols, ta = Object.getOwnPropertyDescriptor, ra = Object.getPrototypeOf, on = Object.prototype;
function Mn(e, t, r) {
  if (typeof t != "string") {
    if (on) {
      var n = ra(t);
      n && n !== on && Mn(e, n, r);
    }
    var o = ea(t);
    nn && (o = o.concat(nn(t)));
    for (var i = rn(e), a = rn(t), s = 0; s < o.length; ++s) {
      var l = o[s];
      if (!Qo[l] && !(r && r[l]) && !(a && a[l]) && !(i && i[l])) {
        var c = ta(t, l);
        try {
          $o(e, l, c);
        } catch {
        }
      }
    }
  }
  return e;
}
var na = Mn;
function He() {
  return (He = Object.assign || function(e) {
    for (var t = 1; t < arguments.length; t++) {
      var r = arguments[t];
      for (var n in r)
        Object.prototype.hasOwnProperty.call(r, n) && (e[n] = r[n]);
    }
    return e;
  }).apply(this, arguments);
}
var an = function(e, t) {
  for (var r = [e[0]], n = 0, o = t.length; n < o; n += 1)
    r.push(t[n], e[n + 1]);
  return r;
}, hr = function(e) {
  return e !== null && typeof e == "object" && (e.toString ? e.toString() : Object.prototype.toString.call(e)) === "[object Object]" && !qt.typeOf(e);
}, Gt = Object.freeze([]), et = Object.freeze({});
function vt(e) {
  return typeof e == "function";
}
function yr(e) {
  return process.env.NODE_ENV !== "production" && typeof e == "string" && e || e.displayName || e.name || "Component";
}
function Vr(e) {
  return e && typeof e.styledComponentId == "string";
}
var mt = typeof process < "u" && (process.env.REACT_APP_SC_ATTR || process.env.SC_ATTR) || "data-styled", Cr = typeof window < "u" && "HTMLElement" in window, oa = Boolean(typeof SC_DISABLE_SPEEDY == "boolean" ? SC_DISABLE_SPEEDY : typeof process < "u" && process.env.REACT_APP_SC_DISABLE_SPEEDY !== void 0 && process.env.REACT_APP_SC_DISABLE_SPEEDY !== "" ? process.env.REACT_APP_SC_DISABLE_SPEEDY !== "false" && process.env.REACT_APP_SC_DISABLE_SPEEDY : typeof process < "u" && process.env.SC_DISABLE_SPEEDY !== void 0 && process.env.SC_DISABLE_SPEEDY !== "" ? process.env.SC_DISABLE_SPEEDY !== "false" && process.env.SC_DISABLE_SPEEDY : process.env.NODE_ENV !== "production"), aa = {}, ia = process.env.NODE_ENV !== "production" ? { 1: `Cannot create styled-component for component: %s.

`, 2: `Can't collect styles once you've consumed a \`ServerStyleSheet\`'s styles! \`ServerStyleSheet\` is a one off instance for each server-side render cycle.

- Are you trying to reuse it across renders?
- Are you accidentally calling collectStyles twice?

`, 3: `Streaming SSR is only supported in a Node.js environment; Please do not try to call this method in the browser.

`, 4: `The \`StyleSheetManager\` expects a valid target or sheet prop!

- Does this error occur on the client and is your target falsy?
- Does this error occur on the server and is the sheet falsy?

`, 5: `The clone method cannot be used on the client!

- Are you running in a client-like environment on the server?
- Are you trying to run SSR on the client?

`, 6: `Trying to insert a new style tag, but the given Node is unmounted!

- Are you using a custom target that isn't mounted?
- Does your document not have a valid head element?
- Have you accidentally removed a style tag manually?

`, 7: 'ThemeProvider: Please return an object from your "theme" prop function, e.g.\n\n```js\ntheme={() => ({})}\n```\n\n', 8: `ThemeProvider: Please make your "theme" prop an object.

`, 9: "Missing document `<head>`\n\n", 10: `Cannot find a StyleSheet instance. Usually this happens if there are multiple copies of styled-components loaded at once. Check out this issue for how to troubleshoot and fix the common cases where this situation can happen: https://github.com/styled-components/styled-components/issues/1941#issuecomment-417862021

`, 11: `_This error was replaced with a dev-time warning, it will be deleted for v4 final._ [createGlobalStyle] received children which will not be rendered. Please use the component without passing children elements.

`, 12: "It seems you are interpolating a keyframe declaration (%s) into an untagged string. This was supported in styled-components v3, but is not longer supported in v4 as keyframes are now injected on-demand. Please wrap your string in the css\\`\\` helper which ensures the styles are injected correctly. See https://www.styled-components.com/docs/api#css\n\n", 13: `%s is not a styled component and cannot be referred to via component selector. See https://www.styled-components.com/docs/advanced#referring-to-other-components for more details.

`, 14: `ThemeProvider: "theme" prop is required.

`, 15: "A stylis plugin has been supplied that is not named. We need a name for each plugin to be able to prevent styling collisions between different stylis configurations within the same app. Before you pass your plugin to `<StyleSheetManager stylisPlugins={[]}>`, please make sure each plugin is uniquely-named, e.g.\n\n```js\nObject.defineProperty(importedPlugin, 'name', { value: 'some-unique-name' });\n```\n\n", 16: `Reached the limit of how many styled components may be created at group %s.
You may only create up to 1,073,741,824 components. If you're creating components dynamically,
as for instance in your render method then you may be running into this limitation.

`, 17: `CSSStyleSheet could not be found on HTMLStyleElement.
Has styled-components' style tag been unmounted or altered by another script?
` } : {};
function sa() {
  for (var e = arguments.length <= 0 ? void 0 : arguments[0], t = [], r = 1, n = arguments.length; r < n; r += 1)
    t.push(r < 0 || arguments.length <= r ? void 0 : arguments[r]);
  return t.forEach(function(o) {
    e = e.replace(/%[a-z]/, o);
  }), e;
}
function Ke(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  throw process.env.NODE_ENV === "production" ? new Error("An error occurred. See https://git.io/JUIaE#" + e + " for more information." + (r.length > 0 ? " Args: " + r.join(", ") : "")) : new Error(sa.apply(void 0, [ia[e]].concat(r)).trim());
}
var la = function() {
  function e(r) {
    this.groupSizes = new Uint32Array(512), this.length = 512, this.tag = r;
  }
  var t = e.prototype;
  return t.indexOfGroup = function(r) {
    for (var n = 0, o = 0; o < r; o++)
      n += this.groupSizes[o];
    return n;
  }, t.insertRules = function(r, n) {
    if (r >= this.groupSizes.length) {
      for (var o = this.groupSizes, i = o.length, a = i; r >= a; )
        (a <<= 1) < 0 && Ke(16, "" + r);
      this.groupSizes = new Uint32Array(a), this.groupSizes.set(o), this.length = a;
      for (var s = i; s < a; s++)
        this.groupSizes[s] = 0;
    }
    for (var l = this.indexOfGroup(r + 1), c = 0, f = n.length; c < f; c++)
      this.tag.insertRule(l, n[c]) && (this.groupSizes[r]++, l++);
  }, t.clearGroup = function(r) {
    if (r < this.length) {
      var n = this.groupSizes[r], o = this.indexOfGroup(r), i = o + n;
      this.groupSizes[r] = 0;
      for (var a = o; a < i; a++)
        this.tag.deleteRule(o);
    }
  }, t.getGroup = function(r) {
    var n = "";
    if (r >= this.length || this.groupSizes[r] === 0)
      return n;
    for (var o = this.groupSizes[r], i = this.indexOfGroup(r), a = i + o, s = i; s < a; s++)
      n += this.tag.getRule(s) + `/*!sc*/
`;
    return n;
  }, e;
}(), Dt = /* @__PURE__ */ new Map(), It = /* @__PURE__ */ new Map(), Ot = 1, Wt = function(e) {
  if (Dt.has(e))
    return Dt.get(e);
  for (; It.has(Ot); )
    Ot++;
  var t = Ot++;
  return process.env.NODE_ENV !== "production" && ((0 | t) < 0 || t > 1 << 30) && Ke(16, "" + t), Dt.set(e, t), It.set(t, e), t;
}, ua = function(e) {
  return It.get(e);
}, ca = function(e, t) {
  t >= Ot && (Ot = t + 1), Dt.set(e, t), It.set(t, e);
}, fa = "style[" + mt + '][data-styled-version="5.3.6"]', da = new RegExp("^" + mt + '\\.g(\\d+)\\[id="([\\w\\d-]+)"\\].*?"([^"]*)'), pa = function(e, t, r) {
  for (var n, o = r.split(","), i = 0, a = o.length; i < a; i++)
    (n = o[i]) && e.registerName(t, n);
}, va = function(e, t) {
  for (var r = (t.textContent || "").split(`/*!sc*/
`), n = [], o = 0, i = r.length; o < i; o++) {
    var a = r[o].trim();
    if (a) {
      var s = a.match(da);
      if (s) {
        var l = 0 | parseInt(s[1], 10), c = s[2];
        l !== 0 && (ca(c, l), pa(e, c, s[3]), e.getTag().insertRules(l, n)), n.length = 0;
      } else
        n.push(a);
    }
  }
}, ma = function() {
  return typeof __webpack_nonce__ < "u" ? __webpack_nonce__ : null;
}, Rn = function(e) {
  var t = document.head, r = e || t, n = document.createElement("style"), o = function(s) {
    for (var l = s.childNodes, c = l.length; c >= 0; c--) {
      var f = l[c];
      if (f && f.nodeType === 1 && f.hasAttribute(mt))
        return f;
    }
  }(r), i = o !== void 0 ? o.nextSibling : null;
  n.setAttribute(mt, "active"), n.setAttribute("data-styled-version", "5.3.6");
  var a = ma();
  return a && n.setAttribute("nonce", a), r.insertBefore(n, i), n;
}, ha = function() {
  function e(r) {
    var n = this.element = Rn(r);
    n.appendChild(document.createTextNode("")), this.sheet = function(o) {
      if (o.sheet)
        return o.sheet;
      for (var i = document.styleSheets, a = 0, s = i.length; a < s; a++) {
        var l = i[a];
        if (l.ownerNode === o)
          return l;
      }
      Ke(17);
    }(n), this.length = 0;
  }
  var t = e.prototype;
  return t.insertRule = function(r, n) {
    try {
      return this.sheet.insertRule(n, r), this.length++, !0;
    } catch {
      return !1;
    }
  }, t.deleteRule = function(r) {
    this.sheet.deleteRule(r), this.length--;
  }, t.getRule = function(r) {
    var n = this.sheet.cssRules[r];
    return n !== void 0 && typeof n.cssText == "string" ? n.cssText : "";
  }, e;
}(), ya = function() {
  function e(r) {
    var n = this.element = Rn(r);
    this.nodes = n.childNodes, this.length = 0;
  }
  var t = e.prototype;
  return t.insertRule = function(r, n) {
    if (r <= this.length && r >= 0) {
      var o = document.createTextNode(n), i = this.nodes[r];
      return this.element.insertBefore(o, i || null), this.length++, !0;
    }
    return !1;
  }, t.deleteRule = function(r) {
    this.element.removeChild(this.nodes[r]), this.length--;
  }, t.getRule = function(r) {
    return r < this.length ? this.nodes[r].textContent : "";
  }, e;
}(), ga = function() {
  function e(r) {
    this.rules = [], this.length = 0;
  }
  var t = e.prototype;
  return t.insertRule = function(r, n) {
    return r <= this.length && (this.rules.splice(r, 0, n), this.length++, !0);
  }, t.deleteRule = function(r) {
    this.rules.splice(r, 1), this.length--;
  }, t.getRule = function(r) {
    return r < this.length ? this.rules[r] : "";
  }, e;
}(), sn = Cr, ba = { isServer: !Cr, useCSSOMInjection: !oa }, Jt = function() {
  function e(r, n, o) {
    r === void 0 && (r = et), n === void 0 && (n = {}), this.options = He({}, ba, {}, r), this.gs = n, this.names = new Map(o), this.server = !!r.isServer, !this.server && Cr && sn && (sn = !1, function(i) {
      for (var a = document.querySelectorAll(fa), s = 0, l = a.length; s < l; s++) {
        var c = a[s];
        c && c.getAttribute(mt) !== "active" && (va(i, c), c.parentNode && c.parentNode.removeChild(c));
      }
    }(this));
  }
  e.registerId = function(r) {
    return Wt(r);
  };
  var t = e.prototype;
  return t.reconstructWithOptions = function(r, n) {
    return n === void 0 && (n = !0), new e(He({}, this.options, {}, r), this.gs, n && this.names || void 0);
  }, t.allocateGSInstance = function(r) {
    return this.gs[r] = (this.gs[r] || 0) + 1;
  }, t.getTag = function() {
    return this.tag || (this.tag = (o = (n = this.options).isServer, i = n.useCSSOMInjection, a = n.target, r = o ? new ga(a) : i ? new ha(a) : new ya(a), new la(r)));
    var r, n, o, i, a;
  }, t.hasNameForId = function(r, n) {
    return this.names.has(r) && this.names.get(r).has(n);
  }, t.registerName = function(r, n) {
    if (Wt(r), this.names.has(r))
      this.names.get(r).add(n);
    else {
      var o = /* @__PURE__ */ new Set();
      o.add(n), this.names.set(r, o);
    }
  }, t.insertRules = function(r, n, o) {
    this.registerName(r, n), this.getTag().insertRules(Wt(r), o);
  }, t.clearNames = function(r) {
    this.names.has(r) && this.names.get(r).clear();
  }, t.clearRules = function(r) {
    this.getTag().clearGroup(Wt(r)), this.clearNames(r);
  }, t.clearTag = function() {
    this.tag = void 0;
  }, t.toString = function() {
    return function(r) {
      for (var n = r.getTag(), o = n.length, i = "", a = 0; a < o; a++) {
        var s = ua(a);
        if (s !== void 0) {
          var l = r.names.get(s), c = n.getGroup(a);
          if (l && c && l.size) {
            var f = mt + ".g" + a + '[id="' + s + '"]', d = "";
            l !== void 0 && l.forEach(function(g) {
              g.length > 0 && (d += g + ",");
            }), i += "" + c + f + '{content:"' + d + `"}/*!sc*/
`;
          }
        }
      }
      return i;
    }(this);
  }, e;
}(), Aa = /(a)(d)/gi, ln = function(e) {
  return String.fromCharCode(e + (e > 25 ? 39 : 97));
};
function gr(e) {
  var t, r = "";
  for (t = Math.abs(e); t > 52; t = t / 52 | 0)
    r = ln(t % 52) + r;
  return (ln(t % 52) + r).replace(Aa, "$1-$2");
}
var nt = function(e, t) {
  for (var r = t.length; r; )
    e = 33 * e ^ t.charCodeAt(--r);
  return e;
}, Dn = function(e) {
  return nt(5381, e);
};
function Bn(e) {
  for (var t = 0; t < e.length; t += 1) {
    var r = e[t];
    if (vt(r) && !Vr(r))
      return !1;
  }
  return !0;
}
var wa = Dn("5.3.6"), Pa = function() {
  function e(t, r, n) {
    this.rules = t, this.staticRulesId = "", this.isStatic = process.env.NODE_ENV === "production" && (n === void 0 || n.isStatic) && Bn(t), this.componentId = r, this.baseHash = nt(wa, r), this.baseStyle = n, Jt.registerId(r);
  }
  return e.prototype.generateAndInjectStyles = function(t, r, n) {
    var o = this.componentId, i = [];
    if (this.baseStyle && i.push(this.baseStyle.generateAndInjectStyles(t, r, n)), this.isStatic && !n.hash)
      if (this.staticRulesId && r.hasNameForId(o, this.staticRulesId))
        i.push(this.staticRulesId);
      else {
        var a = ot(this.rules, t, r, n).join(""), s = gr(nt(this.baseHash, a) >>> 0);
        if (!r.hasNameForId(o, s)) {
          var l = n(a, "." + s, void 0, o);
          r.insertRules(o, s, l);
        }
        i.push(s), this.staticRulesId = s;
      }
    else {
      for (var c = this.rules.length, f = nt(this.baseHash, n.hash), d = "", g = 0; g < c; g++) {
        var h = this.rules[g];
        if (typeof h == "string")
          d += h, process.env.NODE_ENV !== "production" && (f = nt(f, h + g));
        else if (h) {
          var v = ot(h, t, r, n), y = Array.isArray(v) ? v.join("") : v;
          f = nt(f, y + g), d += y;
        }
      }
      if (d) {
        var p = gr(f >>> 0);
        if (!r.hasNameForId(o, p)) {
          var A = n(d, "." + p, void 0, o);
          r.insertRules(o, p, A);
        }
        i.push(p);
      }
    }
    return i.join(" ");
  }, e;
}(), Ta = /^\s*\/\/.*$/gm, Oa = [":", "[", ".", "#"];
function Sa(e) {
  var t, r, n, o, i = e === void 0 ? et : e, a = i.options, s = a === void 0 ? et : a, l = i.plugins, c = l === void 0 ? Gt : l, f = new Yo(s), d = [], g = function(y) {
    function p(A) {
      if (A)
        try {
          y(A + "}");
        } catch {
        }
    }
    return function(A, w, X, T, m, M, Z, z, W, C) {
      switch (A) {
        case 1:
          if (W === 0 && w.charCodeAt(0) === 64)
            return y(w + ";"), "";
          break;
        case 2:
          if (z === 0)
            return w + "/*|*/";
          break;
        case 3:
          switch (z) {
            case 102:
            case 112:
              return y(X[0] + w), "";
            default:
              return w + (C === 0 ? "/*|*/" : "");
          }
        case -2:
          w.split("/*|*/}").forEach(p);
      }
    };
  }(function(y) {
    d.push(y);
  }), h = function(y, p, A) {
    return p === 0 && Oa.indexOf(A[r.length]) !== -1 || A.match(o) ? y : "." + t;
  };
  function v(y, p, A, w) {
    w === void 0 && (w = "&");
    var X = y.replace(Ta, ""), T = p && A ? A + " " + p + " { " + X + " }" : X;
    return t = w, r = p, n = new RegExp("\\" + r + "\\b", "g"), o = new RegExp("(\\" + r + "\\b){2,}"), f(A || !p ? "" : p, T);
  }
  return f.use([].concat(c, [function(y, p, A) {
    y === 2 && A.length && A[0].lastIndexOf(r) > 0 && (A[0] = A[0].replace(n, h));
  }, g, function(y) {
    if (y === -2) {
      var p = d;
      return d = [], p;
    }
  }])), v.hash = c.length ? c.reduce(function(y, p) {
    return p.name || Ke(15), nt(y, p.name);
  }, 5381).toString() : "", v;
}
var jn = V.createContext();
jn.Consumer;
var Yn = V.createContext(), Xa = (Yn.Consumer, new Jt()), br = Sa();
function Zn() {
  return De(jn) || Xa;
}
function Hn() {
  return De(Yn) || br;
}
var Na = function() {
  function e(t, r) {
    var n = this;
    this.inject = function(o, i) {
      i === void 0 && (i = br);
      var a = n.name + i.hash;
      o.hasNameForId(n.id, a) || o.insertRules(n.id, a, i(n.rules, a, "@keyframes"));
    }, this.toString = function() {
      return Ke(12, String(n.name));
    }, this.name = t, this.id = "sc-keyframes-" + t, this.rules = r;
  }
  return e.prototype.getName = function(t) {
    return t === void 0 && (t = br), this.name + t.hash;
  }, e;
}(), qa = /([A-Z])/, xa = /([A-Z])/g, Ea = /^ms-/, La = function(e) {
  return "-" + e.toLowerCase();
};
function un(e) {
  return qa.test(e) ? e.replace(xa, La).replace(Ea, "-ms-") : e;
}
var cn = function(e) {
  return e == null || e === !1 || e === "";
};
function ot(e, t, r, n) {
  if (Array.isArray(e)) {
    for (var o, i = [], a = 0, s = e.length; a < s; a += 1)
      (o = ot(e[a], t, r, n)) !== "" && (Array.isArray(o) ? i.push.apply(i, o) : i.push(o));
    return i;
  }
  if (cn(e))
    return "";
  if (Vr(e))
    return "." + e.styledComponentId;
  if (vt(e)) {
    if (typeof (c = e) != "function" || c.prototype && c.prototype.isReactComponent || !t)
      return e;
    var l = e(t);
    return process.env.NODE_ENV !== "production" && qt.isElement(l) && console.warn(yr(e) + " is not a styled component and cannot be referred to via component selector. See https://www.styled-components.com/docs/advanced#referring-to-other-components for more details."), ot(l, t, r, n);
  }
  var c;
  return e instanceof Na ? r ? (e.inject(r, n), e.getName(n)) : e : hr(e) ? function f(d, g) {
    var h, v, y = [];
    for (var p in d)
      d.hasOwnProperty(p) && !cn(d[p]) && (Array.isArray(d[p]) && d[p].isCss || vt(d[p]) ? y.push(un(p) + ":", d[p], ";") : hr(d[p]) ? y.push.apply(y, f(d[p], p)) : y.push(un(p) + ": " + (h = p, (v = d[p]) == null || typeof v == "boolean" || v === "" ? "" : typeof v != "number" || v === 0 || h in Zo ? String(v).trim() : v + "px") + ";"));
    return g ? [g + " {"].concat(y, ["}"]) : y;
  }(e) : e.toString();
}
var fn = function(e) {
  return Array.isArray(e) && (e.isCss = !0), e;
};
function te(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  return vt(e) || hr(e) ? fn(ot(an(Gt, [e].concat(r)))) : r.length === 0 && e.length === 1 && typeof e[0] == "string" ? e : fn(ot(an(e, r)));
}
var dn = /invalid hook call/i, Vt = /* @__PURE__ */ new Set(), Un = function(e, t) {
  if (process.env.NODE_ENV !== "production") {
    var r = "The component " + e + (t ? ' with the id of "' + t + '"' : "") + ` has been created dynamically.
You may see this warning because you've called styled inside another component.
To resolve this only create new StyledComponents outside of any render method and function component.`, n = console.error;
    try {
      var o = !0;
      console.error = function(i) {
        if (dn.test(i))
          o = !1, Vt.delete(r);
        else {
          for (var a = arguments.length, s = new Array(a > 1 ? a - 1 : 0), l = 1; l < a; l++)
            s[l - 1] = arguments[l];
          n.apply(void 0, [i].concat(s));
        }
      }, ye(), o && !Vt.has(r) && (console.warn(r), Vt.add(r));
    } catch (i) {
      dn.test(i.message) && Vt.delete(r);
    } finally {
      console.error = n;
    }
  }
}, Gn = function(e, t, r) {
  return r === void 0 && (r = et), e.theme !== r.theme && e.theme || t || r.theme;
}, za = /[!"#$%&'()*+,./:;<=>?@[\\\]^`{|}~-]+/g, Wa = /(^-|-$)/g;
function ir(e) {
  return e.replace(za, "-").replace(Wa, "");
}
var In = function(e) {
  return gr(Dn(e) >>> 0);
};
function Ct(e) {
  return typeof e == "string" && (process.env.NODE_ENV === "production" || e.charAt(0) === e.charAt(0).toLowerCase());
}
var Ar = function(e) {
  return typeof e == "function" || typeof e == "object" && e !== null && !Array.isArray(e);
}, Va = function(e) {
  return e !== "__proto__" && e !== "constructor" && e !== "prototype";
};
function Ca(e, t, r) {
  var n = e[r];
  Ar(t) && Ar(n) ? Jn(n, t) : e[r] = t;
}
function Jn(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  for (var o = 0, i = r; o < i.length; o++) {
    var a = i[o];
    if (Ar(a))
      for (var s in a)
        Va(s) && Ca(e, a[s], s);
  }
  return e;
}
var ht = V.createContext();
ht.Consumer;
function Fa(e) {
  var t = De(ht), r = Lr(function() {
    return function(n, o) {
      if (!n)
        return Ke(14);
      if (vt(n)) {
        var i = n(o);
        return process.env.NODE_ENV === "production" || i !== null && !Array.isArray(i) && typeof i == "object" ? i : Ke(7);
      }
      return Array.isArray(n) || typeof n != "object" ? Ke(8) : o ? He({}, o, {}, n) : n;
    }(e.theme, t);
  }, [e.theme, t]);
  return e.children ? V.createElement(ht.Provider, { value: r }, e.children) : null;
}
var sr = {};
function Kn(e, t, r) {
  var n = Vr(e), o = !Ct(e), i = t.attrs, a = i === void 0 ? Gt : i, s = t.componentId, l = s === void 0 ? function(w, X) {
    var T = typeof w != "string" ? "sc" : ir(w);
    sr[T] = (sr[T] || 0) + 1;
    var m = T + "-" + In("5.3.6" + T + sr[T]);
    return X ? X + "-" + m : m;
  }(t.displayName, t.parentComponentId) : s, c = t.displayName, f = c === void 0 ? function(w) {
    return Ct(w) ? "styled." + w : "Styled(" + yr(w) + ")";
  }(e) : c, d = t.displayName && t.componentId ? ir(t.displayName) + "-" + t.componentId : t.componentId || l, g = n && e.attrs ? Array.prototype.concat(e.attrs, a).filter(Boolean) : a, h = t.shouldForwardProp;
  n && e.shouldForwardProp && (h = t.shouldForwardProp ? function(w, X, T) {
    return e.shouldForwardProp(w, X, T) && t.shouldForwardProp(w, X, T);
  } : e.shouldForwardProp);
  var v, y = new Pa(r, d, n ? e.componentStyle : void 0), p = y.isStatic && a.length === 0, A = function(w, X) {
    return function(T, m, M, Z) {
      var z = T.attrs, W = T.componentStyle, C = T.defaultProps, Y = T.foldedComponentIds, j = T.shouldForwardProp, F = T.styledComponentId, H = T.target;
      process.env.NODE_ENV !== "production" && Ir(F);
      var G = function(k, b, K) {
        k === void 0 && (k = et);
        var S = He({}, b, { theme: k }), ie = {};
        return K.forEach(function(J) {
          var Q, P, de, x = J;
          for (Q in vt(x) && (x = x(S)), x)
            S[Q] = ie[Q] = Q === "className" ? (P = ie[Q], de = x[Q], P && de ? P + " " + de : P || de) : x[Q];
        }), [S, ie];
      }(Gn(m, De(ht), C) || et, m, z), ne = G[0], re = G[1], I = function(k, b, K, S) {
        var ie = Zn(), J = Hn(), Q = b ? k.generateAndInjectStyles(et, ie, J) : k.generateAndInjectStyles(K, ie, J);
        return process.env.NODE_ENV !== "production" && Ir(Q), process.env.NODE_ENV !== "production" && !b && S && S(Q), Q;
      }(W, Z, ne, process.env.NODE_ENV !== "production" ? T.warnTooManyClasses : void 0), ve = M, he = re.$as || m.$as || re.as || m.as || H, Ne = Ct(he), L = re !== m ? He({}, m, {}, re) : m, N = {};
      for (var q in L)
        q[0] !== "$" && q !== "as" && (q === "forwardedAs" ? N.as = L[q] : (j ? j(q, $r, he) : !Ne || $r(q)) && (N[q] = L[q]));
      return m.style && re.style !== m.style && (N.style = He({}, m.style, {}, re.style)), N.className = Array.prototype.concat(Y, F, I !== F ? I : null, m.className, re.className).filter(Boolean).join(" "), N.ref = ve, ft(he, N);
    }(v, w, X, p);
  };
  return A.displayName = f, (v = V.forwardRef(A)).attrs = g, v.componentStyle = y, v.displayName = f, v.shouldForwardProp = h, v.foldedComponentIds = n ? Array.prototype.concat(e.foldedComponentIds, e.styledComponentId) : Gt, v.styledComponentId = d, v.target = n ? e.target : e, v.withComponent = function(w) {
    var X = t.componentId, T = function(M, Z) {
      if (M == null)
        return {};
      var z, W, C = {}, Y = Object.keys(M);
      for (W = 0; W < Y.length; W++)
        z = Y[W], Z.indexOf(z) >= 0 || (C[z] = M[z]);
      return C;
    }(t, ["componentId"]), m = X && X + "-" + (Ct(w) ? w : ir(yr(w)));
    return Kn(w, He({}, T, { attrs: g, componentId: m }), r);
  }, Object.defineProperty(v, "defaultProps", { get: function() {
    return this._foldedDefaultProps;
  }, set: function(w) {
    this._foldedDefaultProps = n ? Jn({}, e.defaultProps, w) : w;
  } }), process.env.NODE_ENV !== "production" && (Un(f, d), v.warnTooManyClasses = function(w, X) {
    var T = {}, m = !1;
    return function(M) {
      if (!m && (T[M] = !0, Object.keys(T).length >= 200)) {
        var Z = X ? ' with the id of "' + X + '"' : "";
        console.warn("Over 200 classes were generated for component " + w + Z + `.
Consider using the attrs method, together with a style object for frequently changed styles.
Example:
  const Component = styled.div.attrs(props => ({
    style: {
      background: props.background,
    },
  }))\`width: 100%;\`

  <Component />`), m = !0, T = {};
      }
    };
  }(f, d)), v.toString = function() {
    return "." + v.styledComponentId;
  }, o && na(v, e, { attrs: !0, componentStyle: !0, displayName: !0, foldedComponentIds: !0, shouldForwardProp: !0, styledComponentId: !0, target: !0, withComponent: !0 }), v;
}
var wr = function(e) {
  return function t(r, n, o) {
    if (o === void 0 && (o = et), !qt.isValidElementType(n))
      return Ke(1, String(n));
    var i = function() {
      return r(n, o, te.apply(void 0, arguments));
    };
    return i.withConfig = function(a) {
      return t(r, n, He({}, o, {}, a));
    }, i.attrs = function(a) {
      return t(r, n, He({}, o, { attrs: Array.prototype.concat(o.attrs, a).filter(Boolean) }));
    }, i;
  }(Kn, e);
};
["a", "abbr", "address", "area", "article", "aside", "audio", "b", "base", "bdi", "bdo", "big", "blockquote", "body", "br", "button", "canvas", "caption", "cite", "code", "col", "colgroup", "data", "datalist", "dd", "del", "details", "dfn", "dialog", "div", "dl", "dt", "em", "embed", "fieldset", "figcaption", "figure", "footer", "form", "h1", "h2", "h3", "h4", "h5", "h6", "head", "header", "hgroup", "hr", "html", "i", "iframe", "img", "input", "ins", "kbd", "keygen", "label", "legend", "li", "link", "main", "map", "mark", "marquee", "menu", "menuitem", "meta", "meter", "nav", "noscript", "object", "ol", "optgroup", "option", "output", "p", "param", "picture", "pre", "progress", "q", "rp", "rt", "ruby", "s", "samp", "script", "section", "select", "small", "source", "span", "strong", "style", "sub", "summary", "sup", "table", "tbody", "td", "textarea", "tfoot", "th", "thead", "time", "title", "tr", "track", "u", "ul", "var", "video", "wbr", "circle", "clipPath", "defs", "ellipse", "foreignObject", "g", "image", "line", "linearGradient", "marker", "mask", "path", "pattern", "polygon", "polyline", "radialGradient", "rect", "stop", "svg", "text", "textPath", "tspan"].forEach(function(e) {
  wr[e] = wr(e);
});
var ka = function() {
  function e(r, n) {
    this.rules = r, this.componentId = n, this.isStatic = Bn(r), Jt.registerId(this.componentId + 1);
  }
  var t = e.prototype;
  return t.createStyles = function(r, n, o, i) {
    var a = i(ot(this.rules, n, o, i).join(""), ""), s = this.componentId + r;
    o.insertRules(s, s, a);
  }, t.removeStyles = function(r, n) {
    n.clearRules(this.componentId + r);
  }, t.renderStyles = function(r, n, o, i) {
    r > 2 && Jt.registerId(this.componentId + r), this.removeStyles(r, o), this.createStyles(r, n, o, i);
  }, e;
}();
function Qn(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  var o = te.apply(void 0, [e].concat(r)), i = "sc-global-" + In(JSON.stringify(o)), a = new ka(o, i);
  function s(c) {
    var f = Zn(), d = Hn(), g = De(ht), h = ye(f.allocateGSInstance(i)).current;
    return process.env.NODE_ENV !== "production" && V.Children.count(c.children) && console.warn("The global style component " + i + " was given child JSX. createGlobalStyle does not render children."), process.env.NODE_ENV !== "production" && o.some(function(v) {
      return typeof v == "string" && v.indexOf("@import") !== -1;
    }) && console.warn("Please do not use @import CSS syntax in createGlobalStyle at this time, as the CSSOM APIs we use in production do not handle it well. Instead, we recommend using a library such as react-helmet to inject a typical <link> meta tag to the stylesheet, or simply embedding it manually in your index.html <head> section for a simpler app."), f.server && l(h, c, f, g, d), Er(function() {
      if (!f.server)
        return l(h, c, f, g, d), function() {
          return a.removeStyles(h, f);
        };
    }, [h, c, f, g, d]), null;
  }
  function l(c, f, d, g, h) {
    if (a.isStatic)
      a.renderStyles(c, aa, d, h);
    else {
      var v = He({}, f, { theme: Gn(f, g, s.defaultProps) });
      a.renderStyles(c, v, d, h);
    }
  }
  return process.env.NODE_ENV !== "production" && Un(i), V.memo(s);
}
var Ma = function() {
  return De(ht);
};
process.env.NODE_ENV !== "production" && typeof navigator < "u" && navigator.product === "ReactNative" && console.warn(`It looks like you've imported 'styled-components' on React Native.
Perhaps you're looking to import 'styled-components/native'?
Read more about this at https://www.styled-components.com/docs/basics#react-native`), process.env.NODE_ENV !== "production" && process.env.NODE_ENV !== "test" && typeof window < "u" && (window["__styled-components-init__"] = window["__styled-components-init__"] || 0, window["__styled-components-init__"] === 1 && console.warn(`It looks like there are several instances of 'styled-components' initialized in this application. This may cause dynamic styles to not render properly, errors during the rehydration process, a missing theme prop, and makes your application bigger without good reason.

See https://s-c.sh/2BAXzed for more info.`), window["__styled-components-init__"] += 1);
const Te = wr, Ra = Te.hr`
	width: 100%;

	${({ theme: { colors: e }, width: t }) => te`
			width: ${t} || 100%;
			border: 0.5px solid ${e.color5};
		`}
`, rl = ({ width: e = "", "data-testid": t = "break" }) => /* @__PURE__ */ U(
  Ra,
  {
    width: e,
    "data-testid": t
  }
), Da = Te.div`
	text-align: center;
	display: flex;

	> div {
		width: 10px;
		height: 10px;
		background-color: ${({ theme: e }) => e.colors.main};
		border-radius: 100%;
		display: inline-block;
		animation: animation-loader-button 1.4s infinite ease-in-out both;
	}

	.load1 {
		animation-delay: -0.32s;
	}

	.load2 {
		animation-delay: -0.16s;
	}

	@keyframes animation-loader-button {
		0%,
		80%,
		100% {
			-webkit-transform: scale(0);
			transform: scale(0);
		}
		40% {
			-webkit-transform: scale(1);
			transform: scale(1);
		}
	}
`, Ba = Te.div`
	display: inline-block;
	width: 100%;
	height: 100vh;
	display: flex;
	justify-content: center;
	align-items: center;

	&:after {
		content: ' ';
		display: block;
		width: 64px;
		height: 64px;
		border-radius: 50%;
		border: ${({ theme: e }) => `6px solid ${e.colors.secondary}`};
		border-color: ${({ theme: e }) => `${e.colors.secondary} transparent ${e.colors.secondary} transparent`};
		animation: animation-loader 1.2s linear infinite;
	}

	@keyframes animation-loader {
		0% {
			transform: rotate(0deg);
		}
		100% {
			transform: rotate(360deg);
		}
	}
`, _n = ({
  type: e = "button",
  "data-testid": t = "loader"
}) => e === "button" ? /* @__PURE__ */ ze(Da, { "data-testid": t, children: [
  /* @__PURE__ */ U("div", { className: "load1" }),
  /* @__PURE__ */ U("div", { className: "load2" }),
  /* @__PURE__ */ U("div", { className: "load3" })
] }) : /* @__PURE__ */ U(Ba, { "data-testid": t });
_n.displayName = "Loader";
const ja = ({ colors: e }, t = "primary") => ({
  primary: te`
			color: ${e.main};
			background: ${e.secondary};

			&:hover {
				background: ${e.color1};
			}

			&:disabled,
			&[disabled] {
				background: ${e.color5};
				color: ${e.main};
				cursor: not-allowed;
			}
		`,
  secondary: te`
			color: ${e.secondary};
			background: ${e.main};
			border: 2px solid ${e.secondary};

			&:hover {
				background: ${e.secondary};
				color: ${e.main};
			}
		`,
  alert: te`
			color: ${e.secondary};
			background: ${e.error};

			&:hover {
				background: ${e.color5};
			}
		`
})[t], Ya = (e = "center") => ({
  center: te`
			justify-content: center;
		`,
  left: te`
			justify-content: flex-start;
		`
})[e], Za = Te.button`
	cursor: pointer;
	min-width: fit-content;
	min-height: 44px;
	max-height: 44px;
	border: 0;
	border-radius: 15px;
	display: flex;
	align-items: center;
	gap: 8px;
	transition: 0.2s;

	${({ theme: { spaces: e, font: t }, hasOnlyIcon: r, hasFullWidth: n, isDropdown: o }) => te`
			width: ${n && "100%"};
			font-weight: ${t.weights.bold};
			padding: ${r ? `${e.tiny} ${e.small}` : `${e.tiny} ${e.big}`};
			border-radius: ${o && "15px 15px 0 0"};
		`}

	${({ theme: e, variant: t }) => ja(e, t)}
	${({ align: e }) => Ya(e)}
`, Pr = ({
  variant: e = "primary",
  type: t = "button",
  children: r,
  action: n,
  hasFullWidth: o = !1,
  hasOnlyIcon: i = !1,
  align: a = "center",
  isLoading: s = !1,
  isDisabled: l = !1,
  isDropdown: c = !1,
  "data-testid": f = "button"
}) => /* @__PURE__ */ U(
  Za,
  {
    variant: e,
    type: t,
    onClick: n,
    hasFullWidth: o,
    hasOnlyIcon: i,
    align: a,
    disabled: l,
    isDropdown: c,
    "data-testid": f,
    children: s ? /* @__PURE__ */ U(_n, { "data-testid": "button-loader" }) : r
  }
);
Pr.displayName = "Button";
const Ha = ({ font: e, colors: t }, r = "h1") => ({
  h1: te`
			font-weight: ${e.weights.medium};
			font-size: ${e.sizes.h1};
			line-height: ${e.lineHeight.h1};
			letter-spacing: ${e.letterSpacing.h1};
			color: ${t.color1};
			text-transform: uppercase;
		`,
  h2: te`
			font-weight: ${e.weights.regular};
			font-size: ${e.sizes.h2};
			line-height: ${e.lineHeight.h2};
			letter-spacing: ${e.letterSpacing.h2};
			color: ${t.color2};
		`,
  h3: te`
			font-weight: ${e.weights.regular};
			font-size: ${e.sizes.h1};
			line-height: ${e.lineHeight.h2};
			letter-spacing: ${e.letterSpacing.h2};
			color: ${t.color2};
		`
})[r], Ua = Te.h1`
	text-align: ${({ align: e }) => e};

	${({ theme: e, level: t }) => Ha(e, `h${t}`)};
`, $n = ({
  level: e = 1,
  children: t,
  align: r = "center",
  "data-testid": n = "heading"
}) => /* @__PURE__ */ U(
  Ua,
  {
    as: `h${e}`,
    level: e,
    align: r,
    "data-testid": n,
    children: t
  }
);
$n.displayName = "Heading";
const Ga = ({ colors: e }, t = "avatar") => ({
  avatar: te`
			background: ${e.main};
			border: 2px solid ${e.secondary};
			border-radius: 100%;
			object-fit: cover;
		`
})[t], Ia = Te.img`
	${({ onClick: e }) => te`
			cursor: ${e && "pointer"};
		`}

	${({ theme: e, variant: t }) => Ga(e, t)}
`, Ja = ({
  src: e,
  width: t,
  height: r,
  alt: n,
  variant: o = "default",
  onClick: i,
  "data-testid": a = "image"
}) => /* @__PURE__ */ U(
  Ia,
  {
    src: e,
    width: t,
    height: r,
    alt: n,
    variant: o,
    onClick: i,
    "data-testid": a
  }
);
Ja.displayName = "Image";
function yt(e) {
  return yt = typeof Symbol == "function" && typeof Symbol.iterator == "symbol" ? function(t) {
    return typeof t;
  } : function(t) {
    return t && typeof Symbol == "function" && t.constructor === Symbol && t !== Symbol.prototype ? "symbol" : typeof t;
  }, yt(e);
}
function Ka(e, t) {
  if (yt(e) !== "object" || e === null)
    return e;
  var r = e[Symbol.toPrimitive];
  if (r !== void 0) {
    var n = r.call(e, t || "default");
    if (yt(n) !== "object")
      return n;
    throw new TypeError("@@toPrimitive must return a primitive value.");
  }
  return (t === "string" ? String : Number)(e);
}
function eo(e) {
  var t = Ka(e, "string");
  return yt(t) === "symbol" ? t : String(t);
}
function Qa(e, t, r) {
  return t = eo(t), t in e ? Object.defineProperty(e, t, {
    value: r,
    enumerable: !0,
    configurable: !0,
    writable: !0
  }) : e[t] = r, e;
}
function _a() {
  if (console && console.warn) {
    for (var e, t = arguments.length, r = new Array(t), n = 0; n < t; n++)
      r[n] = arguments[n];
    typeof r[0] == "string" && (r[0] = "react-i18next:: ".concat(r[0])), (e = console).warn.apply(e, r);
  }
}
var pn = {};
function Tr() {
  for (var e = arguments.length, t = new Array(e), r = 0; r < e; r++)
    t[r] = arguments[r];
  typeof t[0] == "string" && pn[t[0]] || (typeof t[0] == "string" && (pn[t[0]] = new Date()), _a.apply(void 0, t));
}
function vn(e, t, r) {
  e.loadNamespaces(t, function() {
    if (e.isInitialized)
      r();
    else {
      var n = function o() {
        setTimeout(function() {
          e.off("initialized", o);
        }, 0), r();
      };
      e.on("initialized", n);
    }
  });
}
function $a(e, t) {
  var r = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {}, n = t.languages[0], o = t.options ? t.options.fallbackLng : !1, i = t.languages[t.languages.length - 1];
  if (n.toLowerCase() === "cimode")
    return !0;
  var a = function(l, c) {
    var f = t.services.backendConnector.state["".concat(l, "|").concat(c)];
    return f === -1 || f === 2;
  };
  return r.bindI18n && r.bindI18n.indexOf("languageChanging") > -1 && t.services.backendConnector.backend && t.isLanguageChangingTo && !a(t.isLanguageChangingTo, e) ? !1 : !!(t.hasResourceBundle(n, e) || !t.services.backendConnector.backend || t.options.resources && !t.options.partialBundledLanguages || a(n, e) && (!o || a(i, e)));
}
function ei(e, t) {
  var r = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
  if (!t.languages || !t.languages.length)
    return Tr("i18n.languages were undefined or empty", t.languages), !0;
  var n = t.options.ignoreJSONStructure !== void 0;
  return n ? t.hasLoadedNamespace(e, {
    precheck: function(i, a) {
      if (r.bindI18n && r.bindI18n.indexOf("languageChanging") > -1 && i.services.backendConnector.backend && i.isLanguageChangingTo && !a(i.isLanguageChangingTo, e))
        return !1;
    }
  }) : $a(e, t, r);
}
var ti = /&(?:amp|#38|lt|#60|gt|#62|apos|#39|quot|#34|nbsp|#160|copy|#169|reg|#174|hellip|#8230|#x2F|#47);/g, ri = {
  "&amp;": "&",
  "&#38;": "&",
  "&lt;": "<",
  "&#60;": "<",
  "&gt;": ">",
  "&#62;": ">",
  "&apos;": "'",
  "&#39;": "'",
  "&quot;": '"',
  "&#34;": '"',
  "&nbsp;": " ",
  "&#160;": " ",
  "&copy;": "©",
  "&#169;": "©",
  "&reg;": "®",
  "&#174;": "®",
  "&hellip;": "…",
  "&#8230;": "…",
  "&#x2F;": "/",
  "&#47;": "/"
}, ni = function(t) {
  return ri[t];
}, oi = function(t) {
  return t.replace(ti, ni);
}, ai = {
  bindI18n: "languageChanged",
  bindI18nStore: "",
  transEmptyNodeValue: "",
  transSupportBasicHtmlNodes: !0,
  transWrapTextNodes: "",
  transKeepBasicHtmlNodesFor: ["br", "strong", "i", "p"],
  useSuspense: !0,
  unescape: oi
};
function ii() {
  return ai;
}
var si;
function li() {
  return si;
}
function ui(e, t) {
  if (!(e instanceof t))
    throw new TypeError("Cannot call a class as a function");
}
function mn(e, t) {
  for (var r = 0; r < t.length; r++) {
    var n = t[r];
    n.enumerable = n.enumerable || !1, n.configurable = !0, "value" in n && (n.writable = !0), Object.defineProperty(e, eo(n.key), n);
  }
}
function ci(e, t, r) {
  return t && mn(e.prototype, t), r && mn(e, r), Object.defineProperty(e, "prototype", {
    writable: !1
  }), e;
}
var fi = Wn(), di = function() {
  function e() {
    ui(this, e), this.usedNamespaces = {};
  }
  return ci(e, [{
    key: "addUsedNamespaces",
    value: function(r) {
      var n = this;
      r.forEach(function(o) {
        n.usedNamespaces[o] || (n.usedNamespaces[o] = !0);
      });
    }
  }, {
    key: "getUsedNamespaces",
    value: function() {
      return Object.keys(this.usedNamespaces);
    }
  }]), e;
}();
function pi(e) {
  if (Array.isArray(e))
    return e;
}
function vi(e, t) {
  var r = e == null ? null : typeof Symbol < "u" && e[Symbol.iterator] || e["@@iterator"];
  if (r != null) {
    var n, o, i, a, s = [], l = !0, c = !1;
    try {
      if (i = (r = r.call(e)).next, t === 0) {
        if (Object(r) !== r)
          return;
        l = !1;
      } else
        for (; !(l = (n = i.call(r)).done) && (s.push(n.value), s.length !== t); l = !0)
          ;
    } catch (f) {
      c = !0, o = f;
    } finally {
      try {
        if (!l && r.return != null && (a = r.return(), Object(a) !== a))
          return;
      } finally {
        if (c)
          throw o;
      }
    }
    return s;
  }
}
function hn(e, t) {
  (t == null || t > e.length) && (t = e.length);
  for (var r = 0, n = new Array(t); r < t; r++)
    n[r] = e[r];
  return n;
}
function mi(e, t) {
  if (e) {
    if (typeof e == "string")
      return hn(e, t);
    var r = Object.prototype.toString.call(e).slice(8, -1);
    if (r === "Object" && e.constructor && (r = e.constructor.name), r === "Map" || r === "Set")
      return Array.from(e);
    if (r === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r))
      return hn(e, t);
  }
}
function hi() {
  throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
}
function yi(e, t) {
  return pi(e) || vi(e, t) || mi(e, t) || hi();
}
function yn(e, t) {
  var r = Object.keys(e);
  if (Object.getOwnPropertySymbols) {
    var n = Object.getOwnPropertySymbols(e);
    t && (n = n.filter(function(o) {
      return Object.getOwnPropertyDescriptor(e, o).enumerable;
    })), r.push.apply(r, n);
  }
  return r;
}
function lr(e) {
  for (var t = 1; t < arguments.length; t++) {
    var r = arguments[t] != null ? arguments[t] : {};
    t % 2 ? yn(Object(r), !0).forEach(function(n) {
      Qa(e, n, r[n]);
    }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(r)) : yn(Object(r)).forEach(function(n) {
      Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(r, n));
    });
  }
  return e;
}
var gi = function(t, r) {
  var n = ye();
  return be(function() {
    n.current = r ? n.current : t;
  }, [t, r]), n.current;
};
function to(e) {
  var t = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {}, r = t.i18n, n = De(fi) || {}, o = n.i18n, i = n.defaultNS, a = r || o || li();
  if (a && !a.reportNamespaces && (a.reportNamespaces = new di()), !a) {
    Tr("You will need to pass in an i18next instance by using initReactI18next");
    var s = function(W, C) {
      return typeof C == "string" ? C : C && yt(C) === "object" && typeof C.defaultValue == "string" ? C.defaultValue : Array.isArray(W) ? W[W.length - 1] : W;
    }, l = [s, {}, !1];
    return l.t = s, l.i18n = {}, l.ready = !1, l;
  }
  a.options.react && a.options.react.wait !== void 0 && Tr("It seems you are still using the old wait option, you may migrate to the new useSuspense behaviour.");
  var c = lr(lr(lr({}, ii()), a.options.react), t), f = c.useSuspense, d = c.keyPrefix, g = e || i || a.options && a.options.defaultNS;
  g = typeof g == "string" ? [g] : g || ["translation"], a.reportNamespaces.addUsedNamespaces && a.reportNamespaces.addUsedNamespaces(g);
  var h = (a.isInitialized || a.initializedStoreOnce) && g.every(function(z) {
    return ei(z, a, c);
  });
  function v() {
    return a.getFixedT(null, c.nsMode === "fallback" ? g : g[0], d);
  }
  var y = _e(v), p = yi(y, 2), A = p[0], w = p[1], X = g.join(), T = gi(X), m = ye(!0);
  be(function() {
    var z = c.bindI18n, W = c.bindI18nStore;
    m.current = !0, !h && !f && vn(a, g, function() {
      m.current && w(v);
    }), h && T && T !== X && m.current && w(v);
    function C() {
      m.current && w(v);
    }
    return z && a && a.on(z, C), W && a && a.store.on(W, C), function() {
      m.current = !1, z && a && z.split(" ").forEach(function(Y) {
        return a.off(Y, C);
      }), W && a && W.split(" ").forEach(function(Y) {
        return a.store.off(Y, C);
      });
    };
  }, [a, X]);
  var M = ye(!0);
  be(function() {
    m.current && !M.current && w(v), M.current = !1;
  }, [a, d]);
  var Z = [A, a, h];
  if (Z.t = A, Z.i18n = a, Z.ready = h, h || !h && !f)
    return Z;
  throw new Promise(function(z) {
    vn(a, g, function() {
      z();
    });
  });
}
var ro = {
  color: void 0,
  size: void 0,
  className: void 0,
  style: void 0,
  attr: void 0
}, gn = V.createContext && V.createContext(ro), tt = globalThis && globalThis.__assign || function() {
  return tt = Object.assign || function(e) {
    for (var t, r = 1, n = arguments.length; r < n; r++) {
      t = arguments[r];
      for (var o in t)
        Object.prototype.hasOwnProperty.call(t, o) && (e[o] = t[o]);
    }
    return e;
  }, tt.apply(this, arguments);
}, bi = globalThis && globalThis.__rest || function(e, t) {
  var r = {};
  for (var n in e)
    Object.prototype.hasOwnProperty.call(e, n) && t.indexOf(n) < 0 && (r[n] = e[n]);
  if (e != null && typeof Object.getOwnPropertySymbols == "function")
    for (var o = 0, n = Object.getOwnPropertySymbols(e); o < n.length; o++)
      t.indexOf(n[o]) < 0 && Object.prototype.propertyIsEnumerable.call(e, n[o]) && (r[n[o]] = e[n[o]]);
  return r;
};
function no(e) {
  return e && e.map(function(t, r) {
    return V.createElement(t.tag, tt({
      key: r
    }, t.attr), no(t.child));
  });
}
function _t(e) {
  return function(t) {
    return V.createElement(Ai, tt({
      attr: tt({}, e.attr)
    }, t), no(e.child));
  };
}
function Ai(e) {
  var t = function(r) {
    var n = e.attr, o = e.size, i = e.title, a = bi(e, ["attr", "size", "title"]), s = o || r.size || "1em", l;
    return r.className && (l = r.className), e.className && (l = (l ? l + " " : "") + e.className), V.createElement("svg", tt({
      stroke: "currentColor",
      fill: "currentColor",
      strokeWidth: "0"
    }, r.attr, n, a, {
      className: l,
      style: tt(tt({
        color: e.color || r.color
      }, r.style), e.style),
      height: s,
      width: s,
      xmlns: "http://www.w3.org/2000/svg"
    }), i && V.createElement("title", null, i), e.children);
  };
  return gn !== void 0 ? V.createElement(gn.Consumer, null, function(r) {
    return t(r);
  }) : t(ro);
}
function wi(e) {
  return _t({ tag: "svg", attr: { viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true" }, child: [{ tag: "path", attr: { d: "M3.53 2.47a.75.75 0 00-1.06 1.06l18 18a.75.75 0 101.06-1.06l-18-18zM22.676 12.553a11.249 11.249 0 01-2.631 4.31l-3.099-3.099a5.25 5.25 0 00-6.71-6.71L7.759 4.577a11.217 11.217 0 014.242-.827c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113z" } }, { tag: "path", attr: { d: "M15.75 12c0 .18-.013.357-.037.53l-4.244-4.243A3.75 3.75 0 0115.75 12zM12.53 15.713l-4.243-4.244a3.75 3.75 0 004.243 4.243z" } }, { tag: "path", attr: { d: "M6.75 12c0-.619.107-1.213.304-1.764l-3.1-3.1a11.25 11.25 0 00-2.63 4.31c-.12.362-.12.752 0 1.114 1.489 4.467 5.704 7.69 10.675 7.69 1.5 0 2.933-.294 4.242-.827l-2.477-2.477A5.25 5.25 0 016.75 12z" } }] })(e);
}
function Pi(e) {
  return _t({ tag: "svg", attr: { viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true" }, child: [{ tag: "path", attr: { d: "M12 15a3 3 0 100-6 3 3 0 000 6z" } }, { tag: "path", attr: { fillRule: "evenodd", d: "M1.323 11.447C2.811 6.976 7.028 3.75 12.001 3.75c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113-1.487 4.471-5.705 7.697-10.677 7.697-4.97 0-9.186-3.223-10.675-7.69a1.762 1.762 0 010-1.113zM17.25 12a5.25 5.25 0 11-10.5 0 5.25 5.25 0 0110.5 0z", clipRule: "evenodd" } }] })(e);
}
function Ti(e) {
  return _t({ tag: "svg", attr: { viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true" }, child: [{ tag: "path", attr: { fillRule: "evenodd", d: "M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm8.706-1.442c1.146-.573 2.437.463 2.126 1.706l-.709 2.836.042-.02a.75.75 0 01.67 1.34l-.04.022c-1.147.573-2.438-.463-2.127-1.706l.71-2.836-.042.02a.75.75 0 11-.671-1.34l.041-.022zM12 9a.75.75 0 100-1.5.75.75 0 000 1.5z", clipRule: "evenodd" } }] })(e);
}
function Oi(e) {
  return _t({ tag: "svg", attr: { viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true" }, child: [{ tag: "path", attr: { fillRule: "evenodd", d: "M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zm-1.72 6.97a.75.75 0 10-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 101.06 1.06L12 13.06l1.72 1.72a.75.75 0 101.06-1.06L13.06 12l1.72-1.72a.75.75 0 10-1.06-1.06L12 10.94l-1.72-1.72z", clipRule: "evenodd" } }] })(e);
}
const Si = ({ colors: e }, t = "label") => ({
  default: te`
			color: ${e.secondary};
		`,
  label: te`
			color: ${e.color5};
		`,
  error: te`
			color: ${e.error};
		`
})[t], Xi = ({ font: e }, t = "medium") => ({
  small: te`
			font-size: ${e.sizes.error};
		`,
  medium: te`
			font-size: ${e.sizes.default};
		`,
  big: te`
			font-size: ${e.sizes.info};
		`,
  large: te`
			font-size: ${e.sizes.icon};
		`,
  huge: te`
			font-size: ${e.sizes.notFound};
		`
})[t], Ni = Te.p`
	font-size: ${({ size: e }) => e};
	text-align: ${({ align: e }) => e};
	min-height: 24px;
	display: flex;
	align-items: center;
	gap: 5px;

	${({ theme: e, type: t }) => Si(e, t)};
	${({ theme: e, size: t }) => Xi(e, t)};
`, xt = ({
  type: e = "default",
  children: t,
  size: r = "medium",
  align: n = "center",
  "data-testid": o = "paragraph"
}) => /* @__PURE__ */ U(
  Ni,
  {
    type: e,
    size: r,
    align: n,
    "data-testid": o,
    children: t
  }
);
xt.displayName = "Paragraph";
const qi = Te.label`
	${({ theme: { colors: e, spaces: t }, hasFullWidth: r }) => te`
			width: ${!!r && "100%"};
			color: ${e.color5};
			display: flex;
			justify-content: space-between;
			align-items: center;
			margin-bottom: ${t.mini};
		`}
`, xi = Te.div`
	position: relative;
`, Ei = Te.input`
	min-height: 44px;
	max-height: 44px;
	border-radius: 15px;

	${({ theme: { colors: e, spaces: t }, hasFullWidth: r, isError: n, isPassword: o, isDropdown: i }) => te`
			width: ${r && "100%"};
			background: ${e.color3};
			border: solid 2px ${e.color4};
			padding: ${t.small} ${t.medium};
			color: ${e.secondary};

			&:focus {
				outline: none;
				border: solid 2px ${e.secondary};
			}

			border: ${n && `solid 2px ${e.error}`};
			padding-right: ${o && `calc(${t.big} + ${t.tiny})`};
			border-radius: ${i && "15px 15px 0 0"};
		`}
`, Li = Te.span`
	cursor: pointer;
	top: 12.5px;
	right: 20px;
	position: absolute;
	font-size: 18px;
`, zi = Te.div`
	width: 100%;
	height: 24px;
	left: 0;
	bottom: -24px;
	position: absolute;
	border-radius: 2px;
	display: flex;
	justify-content: space-between;
	align-items: center;

	.strength {
		width: calc(100% - 120px);
		display: flex;
		justify-content: flex-start;
		align-items: center;
	}

	${({ theme: { colors: e }, strength: t = 0, color: r = "error" }) => te`
			.very-weak,
			.weak,
			.medium,
			.good,
			.very-good {
				width: 35px;
				height: 3px;
				margin: 0 2px;
				height: 3px;
				background: ${e[r]};
			}

			.very-weak {
				display: ${t < 20 && "none"};
			}

			.weak {
				display: ${t < 40 && "none"};
			}

			.medium {
				display: ${t < 60 && "none"};
			}

			.good {
				display: ${t < 80 && "none"};
			}

			.very-good {
				display: ${t !== 100 && "none"};
			}
		`}
`, oo = ({ value: e = "" }) => {
  const { t } = to(), [r, n] = _e(0), o = {
    20: "word.very.weak",
    40: "word.weak",
    60: "word.medium",
    80: "word.good",
    100: "word.very.good"
  }, i = {
    20: "error",
    40: "weak",
    60: "warning",
    80: "good",
    100: "success"
  };
  return be(() => {
    const a = {
      length: 0,
      hasUpperCase: !1,
      hasLowerCase: !1,
      hasDigit: !1,
      hasSpecialChar: !1
    };
    a.length = e.length >= 8, a.hasUpperCase = /[A-Z]+/.test(e), a.hasLowerCase = /[a-z]+/.test(e), a.hasDigit = /[0-9]+/.test(e), a.hasSpecialChar = /[^A-Za-z0-9]+/.test(e);
    const s = Object.values(a).filter((l) => l);
    n(s.length / 5 * 100);
  }, [e]), /* @__PURE__ */ ze(
    zi,
    {
      strength: r,
      color: i[r],
      children: [
        /* @__PURE__ */ ze("div", { className: "strength", children: [
          /* @__PURE__ */ U("div", { className: "very-weak" }),
          /* @__PURE__ */ U("div", { className: "weak" }),
          /* @__PURE__ */ U("div", { className: "medium" }),
          /* @__PURE__ */ U("div", { className: "good" }),
          /* @__PURE__ */ U("div", { className: "very-good" })
        ] }),
        !!o[r] && /* @__PURE__ */ ze(
          xt,
          {
            size: "small",
            "data-testid": "input-error",
            children: [
              t(o[r]),
              /* @__PURE__ */ U(Ti, {})
            ]
          }
        )
      ]
    }
  );
};
oo.displayName = "PasswordMeter";
const Wi = ({
  type: e = "text",
  label: t,
  name: r,
  placeholder: n = "",
  id: o,
  value: i = "",
  touched: a = {},
  errors: s = {},
  onChange: l,
  onBlur: c,
  autoFocus: f = !1,
  hasFullWidth: d = !1,
  hasPasswordMeter: g = !1,
  isDropdown: h = !1,
  hideError: v = !1,
  onClick: y,
  "data-testid": p = "input"
}) => {
  const { t: A } = to(), [w, X] = _e(!1), T = s[r] && a[r], m = e === "password";
  return /* @__PURE__ */ ze("div", { children: [
    t && /* @__PURE__ */ U(
      qi,
      {
        htmlFor: o,
        hasFullWidth: d,
        children: /* @__PURE__ */ U(
          xt,
          {
            type: "label",
            "data-testid": "input-label",
            children: t
          }
        )
      }
    ),
    /* @__PURE__ */ ze(xi, { children: [
      /* @__PURE__ */ U(
        Ei,
        {
          id: o,
          name: r,
          type: w ? "text" : e,
          value: i,
          placeholder: n,
          onChange: l,
          onBlur: c,
          hasFullWidth: d,
          autoFocus: f,
          isError: T,
          isPassword: m,
          "data-testid": p,
          isDropdown: h,
          onClick: y
        }
      ),
      m && /* @__PURE__ */ U(
        Li,
        {
          onClick: () => X(!w),
          "data-testid": "input-password",
          children: w ? /* @__PURE__ */ U(wi, {}) : /* @__PURE__ */ U(Pi, {})
        }
      ),
      m && !T && g && /* @__PURE__ */ U(oo, { value: i })
    ] }),
    !v && /* @__PURE__ */ U(
      xt,
      {
        type: "error",
        size: "small",
        align: "left",
        "data-testid": "input-error",
        children: T ? A(`${s[r]}`) : null
      }
    )
  ] });
};
Wi.displayName = "Input";
const Vi = Te.div`
	${({ theme: { breakpoints: e } }) => te`
			width: 100%;
			display: flex;
			justify-content: center;
			align-items: flex-start;
			overflow-y: auto;

			@media (max-width: ${e.mobile}px) {
				width: 100%;
				height: calc(100vh - 112px);
			}
		`}
`, Ci = Te.div`
	${({ theme: { spaces: e, colors: t, breakpoints: r }, image: n }) => te`
			width: calc(100% - 600px);
			height: calc(100vh - 84px);
			padding: calc(${e.huge} + ${e.huge}) ${e.huge};
			background: ${t.main};
			background-image: url(${n});
			background-position: center;
			background-repeat: no-repeat;
			background-size: cover;
			opacity: 0.5;
			display: ${!n && "none"};

			@media (max-width: ${r.mobile}px) {
				display: none;
			}

			@media (max-width: ${r.tablet}px) {
				display: none;
			}
		`}
`, Fi = Te.div`
	${({ theme: { spaces: e, breakpoints: t }, hasFullWidth: r }) => te`
			width: ${r ? "100%" : "400px"};
			margin: 0 auto;
			padding: ${e.huge} ${e.big};
			display: flex;
			flex-direction: column;
			align-items: center;

			@media (max-width: ${t.mobile}px) {
				width: 100%;
			}
		`}
`, ki = ({
  children: e,
  image: t,
  hasFullWidth: r,
  "data-testid": n = "layout"
}) => /* @__PURE__ */ ze(Vi, { "data-testid": n, children: [
  /* @__PURE__ */ U(Ci, { image: t }),
  /* @__PURE__ */ U(Fi, { hasFullWidth: r, children: e })
] });
ki.displayName = "Layout";
const Mi = Te.div`
	${({ theme: { spaces: e }, type: t, space: r = "medium" }) => te`
			width: ${t === "horizontal" && e[r]};
			height: ${t === "vertical" && e[r]};
		`}
`, Bt = ({
  type: e = "horizontal",
  space: t = "medium",
  "data-testid": r = "spacer"
}) => /* @__PURE__ */ U(
  Mi,
  {
    type: e,
    space: t,
    "data-testid": r
  }
);
Bt.displayName = "Spacer";
var qe = globalThis && globalThis.__assign || function() {
  return qe = Object.assign || function(e) {
    for (var t, r = 1, n = arguments.length; r < n; r++) {
      t = arguments[r];
      for (var o in t)
        Object.prototype.hasOwnProperty.call(t, o) && (e[o] = t[o]);
    }
    return e;
  }, qe.apply(this, arguments);
}, Ri = globalThis && globalThis.__rest || function(e, t) {
  var r = {};
  for (var n in e)
    Object.prototype.hasOwnProperty.call(e, n) && t.indexOf(n) < 0 && (r[n] = e[n]);
  if (e != null && typeof Object.getOwnPropertySymbols == "function")
    for (var o = 0, n = Object.getOwnPropertySymbols(e); o < n.length; o++)
      t.indexOf(n[o]) < 0 && Object.prototype.propertyIsEnumerable.call(e, n[o]) && (r[n[o]] = e[n[o]]);
  return r;
}, ur = Symbol("NiceModalId"), Fr = {}, bt = V.createContext(Fr), ao = V.createContext(null), Qe = {}, Et = {}, Di = 0, At = function() {
  throw new Error("No dispatch method detected, did you embed your app with NiceModal.Provider?");
}, Bi = function() {
  return "_nice_modal_" + Di++;
}, io = function(e, t) {
  var r, n, o;
  switch (e === void 0 && (e = Fr), t.type) {
    case "nice-modal/show": {
      var i = t.payload, a = i.modalId, s = i.args;
      return qe(qe({}, e), (r = {}, r[a] = qe(qe({}, e[a]), {
        id: a,
        args: s,
        // If modal is not mounted, mount it first then make it visible.
        // There is logic inside HOC wrapper to make it visible after its first mount.
        // This mechanism ensures the entering transition.
        visible: !!Et[a],
        delayVisible: !Et[a]
      }), r));
    }
    case "nice-modal/hide": {
      var a = t.payload.modalId;
      return e[a] ? qe(qe({}, e), (n = {}, n[a] = qe(qe({}, e[a]), { visible: !1 }), n)) : e;
    }
    case "nice-modal/remove": {
      var a = t.payload.modalId, l = qe({}, e);
      return delete l[a], l;
    }
    case "nice-modal/set-flags": {
      var c = t.payload, a = c.modalId, f = c.flags;
      return qe(qe({}, e), (o = {}, o[a] = qe(qe({}, e[a]), f), o));
    }
    default:
      return e;
  }
};
function ji(e, t) {
  return {
    type: "nice-modal/show",
    payload: {
      modalId: e,
      args: t
    }
  };
}
function Yi(e, t) {
  return {
    type: "nice-modal/set-flags",
    payload: {
      modalId: e,
      flags: t
    }
  };
}
function Zi(e) {
  return {
    type: "nice-modal/hide",
    payload: {
      modalId: e
    }
  };
}
function Hi(e) {
  return {
    type: "nice-modal/remove",
    payload: {
      modalId: e
    }
  };
}
var Je = {}, dt = {}, kr = function(e) {
  return typeof e == "string" ? e : (e[ur] || (e[ur] = Bi()), e[ur]);
};
function so(e, t) {
  var r = kr(e);
  if (typeof e != "string" && !Qe[r] && $t(r, e), At(ji(r, t)), !Je[r]) {
    var n, o, i = new Promise(function(a, s) {
      n = a, o = s;
    });
    Je[r] = {
      resolve: n,
      reject: o,
      promise: i
    };
  }
  return Je[r].promise;
}
function lo(e) {
  var t = kr(e);
  if (At(Zi(t)), delete Je[t], !dt[t]) {
    var r, n, o = new Promise(function(i, a) {
      r = i, n = a;
    });
    dt[t] = {
      resolve: r,
      reject: n,
      promise: o
    };
  }
  return dt[t].promise;
}
var uo = function(e) {
  At(Hi(e)), delete Je[e], delete dt[e];
}, Ui = function(e, t) {
  At(Yi(e, t));
};
function Mr(e, t) {
  var r = De(bt), n = De(ao), o = null, i = e && typeof e != "string";
  if (e ? o = kr(e) : o = n, !o)
    throw new Error("No modal id found in NiceModal.useModal.");
  var a = o;
  be(function() {
    i && !Qe[a] && $t(a, e, t);
  }, [i, a, e, t]);
  var s = r[a], l = xe(function(v) {
    return so(a, v);
  }, [a]), c = xe(function() {
    return lo(a);
  }, [a]), f = xe(function() {
    return uo(a);
  }, [a]), d = xe(function(v) {
    var y;
    (y = Je[a]) === null || y === void 0 || y.resolve(v), delete Je[a];
  }, [a]), g = xe(function(v) {
    var y;
    (y = Je[a]) === null || y === void 0 || y.reject(v), delete Je[a];
  }, [a]), h = xe(function(v) {
    var y;
    (y = dt[a]) === null || y === void 0 || y.resolve(v), delete dt[a];
  }, [a]);
  return {
    id: a,
    args: s == null ? void 0 : s.args,
    visible: !!(s != null && s.visible),
    keepMounted: !!(s != null && s.keepMounted),
    show: l,
    hide: c,
    remove: f,
    resolve: d,
    reject: g,
    resolveHide: h
  };
}
var Gi = function(e) {
  return function(t) {
    var r, n = t.defaultVisible, o = t.keepMounted, i = t.id, a = Ri(t, ["defaultVisible", "keepMounted", "id"]), s = Mr(i), l = s.args, c = s.show, f = De(bt), d = !!f[i];
    be(function() {
      return n && c(), Et[i] = !0, function() {
        delete Et[i];
      };
    }, [i, c, n]), be(function() {
      o && Ui(i, { keepMounted: !0 });
    }, [i, o]);
    var g = (r = f[i]) === null || r === void 0 ? void 0 : r.delayVisible;
    return be(function() {
      g && c(l);
    }, [g, l, c]), d ? V.createElement(
      ao.Provider,
      { value: i },
      V.createElement(e, qe({}, a, l))
    ) : null;
  };
}, $t = function(e, t, r) {
  Qe[e] ? Qe[e].props = r : Qe[e] = { comp: t, props: r };
}, Ii = function(e) {
  delete Qe[e];
}, co = function() {
  var e = De(bt), t = Object.keys(e).filter(function(n) {
    return !!e[n];
  });
  t.forEach(function(n) {
    if (!Qe[n] && !Et[n]) {
      console.warn("No modal found for id: " + n + ". Please check the id or if it is registered or declared via JSX.");
      return;
    }
  });
  var r = t.filter(function(n) {
    return Qe[n];
  }).map(function(n) {
    return qe({ id: n }, Qe[n]);
  });
  return V.createElement(V.Fragment, null, r.map(function(n) {
    return V.createElement(n.comp, qe({ key: n.id, id: n.id }, n.props));
  }));
}, Ji = function(e) {
  var t = e.children, r = Vn(io, Fr), n = r[0];
  return At = r[1], V.createElement(
    bt.Provider,
    { value: n },
    t,
    V.createElement(co, null)
  );
}, Ki = function(e) {
  var t = e.children, r = e.dispatch, n = e.modals;
  return !r || !n ? V.createElement(Ji, null, t) : (At = r, V.createElement(
    bt.Provider,
    { value: n },
    t,
    V.createElement(co, null)
  ));
}, Qi = function(e) {
  var t = e.id, r = e.component;
  return be(function() {
    return $t(t, r), function() {
      Ii(t);
    };
  }, [t, r]), null;
}, _i = function(e) {
  return {
    visible: e.visible,
    onOk: function() {
      return e.hide();
    },
    onCancel: function() {
      return e.hide();
    },
    afterClose: function() {
      e.resolveHide(), e.keepMounted || e.remove();
    }
  };
}, $i = function(e) {
  return {
    visible: e.visible,
    onClose: function() {
      return e.hide();
    },
    afterVisibleChange: function(t) {
      t || e.resolveHide(), !t && !e.keepMounted && e.remove();
    }
  };
}, es = function(e) {
  return {
    open: e.visible,
    onClose: function() {
      return e.hide();
    },
    onExited: function() {
      e.resolveHide(), !e.keepMounted && e.remove();
    }
  };
}, ts = function(e) {
  return {
    show: e.visible,
    onHide: function() {
      return e.hide();
    },
    onExited: function() {
      e.resolveHide(), !e.keepMounted && e.remove();
    }
  };
}, Rr = {
  Provider: Ki,
  ModalDef: Qi,
  NiceModalContext: bt,
  create: Gi,
  register: $t,
  show: so,
  hide: lo,
  remove: uo,
  useModal: Mr,
  reducer: io,
  antdModal: _i,
  antdDrawer: $i,
  muiDialog: es,
  bootstrapDialog: ts
};
function fo(e) {
  var t, r, n = "";
  if (typeof e == "string" || typeof e == "number")
    n += e;
  else if (typeof e == "object")
    if (Array.isArray(e))
      for (t = 0; t < e.length; t++)
        e[t] && (r = fo(e[t])) && (n && (n += " "), n += r);
    else
      for (t in e)
        e[t] && (n && (n += " "), n += t);
  return n;
}
function $e() {
  for (var e, t, r = 0, n = ""; r < arguments.length; )
    (e = arguments[r++]) && (t = fo(e)) && (n && (n += " "), n += t);
  return n;
}
const St = (e) => typeof e == "number" && !isNaN(e), at = (e) => typeof e == "string", Ve = (e) => typeof e == "function", jt = (e) => at(e) || Ve(e) ? e : null, cr = (e) => pt(e) || at(e) || Ve(e) || St(e);
function rs(e, t, r) {
  r === void 0 && (r = 300);
  const { scrollHeight: n, style: o } = e;
  requestAnimationFrame(() => {
    o.minHeight = "initial", o.height = n + "px", o.transition = `all ${r}ms`, requestAnimationFrame(() => {
      o.height = "0", o.padding = "0", o.margin = "0", setTimeout(t, r);
    });
  });
}
function er(e) {
  let { enter: t, exit: r, appendPosition: n = !1, collapse: o = !0, collapseDuration: i = 300 } = e;
  return function(a) {
    let { children: s, position: l, preventExitTransition: c, done: f, nodeRef: d, isIn: g } = a;
    const h = n ? `${t}--${l}` : t, v = n ? `${r}--${l}` : r, y = ye(0);
    return Er(() => {
      const p = d.current, A = h.split(" "), w = (X) => {
        X.target === d.current && (p.dispatchEvent(new Event("d")), p.removeEventListener("animationend", w), p.removeEventListener("animationcancel", w), y.current === 0 && X.type !== "animationcancel" && p.classList.remove(...A));
      };
      p.classList.add(...A), p.addEventListener("animationend", w), p.addEventListener("animationcancel", w);
    }, []), be(() => {
      const p = d.current, A = () => {
        p.removeEventListener("animationend", A), o ? rs(p, f, i) : f();
      };
      g || (c ? A() : (y.current = 1, p.className += ` ${v}`, p.addEventListener("animationend", A)));
    }, [g]), V.createElement(V.Fragment, null, s);
  };
}
function bn(e, t) {
  return { content: e.content, containerId: e.props.containerId, id: e.props.toastId, theme: e.props.theme, type: e.props.type, data: e.props.data || {}, isLoading: e.props.isLoading, icon: e.props.icon, status: t };
}
const Re = { list: /* @__PURE__ */ new Map(), emitQueue: /* @__PURE__ */ new Map(), on(e, t) {
  return this.list.has(e) || this.list.set(e, []), this.list.get(e).push(t), this;
}, off(e, t) {
  if (t) {
    const r = this.list.get(e).filter((n) => n !== t);
    return this.list.set(e, r), this;
  }
  return this.list.delete(e), this;
}, cancelEmit(e) {
  const t = this.emitQueue.get(e);
  return t && (t.forEach(clearTimeout), this.emitQueue.delete(e)), this;
}, emit(e) {
  this.list.has(e) && this.list.get(e).forEach((t) => {
    const r = setTimeout(() => {
      t(...[].slice.call(arguments, 1));
    }, 0);
    this.emitQueue.has(e) || this.emitQueue.set(e, []), this.emitQueue.get(e).push(r);
  });
} }, Ft = (e) => {
  let { theme: t, type: r, ...n } = e;
  return V.createElement("svg", { viewBox: "0 0 24 24", width: "100%", height: "100%", fill: t === "colored" ? "currentColor" : `var(--toastify-icon-color-${r})`, ...n });
}, fr = { info: function(e) {
  return V.createElement(Ft, { ...e }, V.createElement("path", { d: "M12 0a12 12 0 1012 12A12.013 12.013 0 0012 0zm.25 5a1.5 1.5 0 11-1.5 1.5 1.5 1.5 0 011.5-1.5zm2.25 13.5h-4a1 1 0 010-2h.75a.25.25 0 00.25-.25v-4.5a.25.25 0 00-.25-.25h-.75a1 1 0 010-2h1a2 2 0 012 2v4.75a.25.25 0 00.25.25h.75a1 1 0 110 2z" }));
}, warning: function(e) {
  return V.createElement(Ft, { ...e }, V.createElement("path", { d: "M23.32 17.191L15.438 2.184C14.728.833 13.416 0 11.996 0c-1.42 0-2.733.833-3.443 2.184L.533 17.448a4.744 4.744 0 000 4.368C1.243 23.167 2.555 24 3.975 24h16.05C22.22 24 24 22.044 24 19.632c0-.904-.251-1.746-.68-2.44zm-9.622 1.46c0 1.033-.724 1.823-1.698 1.823s-1.698-.79-1.698-1.822v-.043c0-1.028.724-1.822 1.698-1.822s1.698.79 1.698 1.822v.043zm.039-12.285l-.84 8.06c-.057.581-.408.943-.897.943-.49 0-.84-.367-.896-.942l-.84-8.065c-.057-.624.25-1.095.779-1.095h1.91c.528.005.84.476.784 1.1z" }));
}, success: function(e) {
  return V.createElement(Ft, { ...e }, V.createElement("path", { d: "M12 0a12 12 0 1012 12A12.014 12.014 0 0012 0zm6.927 8.2l-6.845 9.289a1.011 1.011 0 01-1.43.188l-4.888-3.908a1 1 0 111.25-1.562l4.076 3.261 6.227-8.451a1 1 0 111.61 1.183z" }));
}, error: function(e) {
  return V.createElement(Ft, { ...e }, V.createElement("path", { d: "M11.983 0a12.206 12.206 0 00-8.51 3.653A11.8 11.8 0 000 12.207 11.779 11.779 0 0011.8 24h.214A12.111 12.111 0 0024 11.791 11.766 11.766 0 0011.983 0zM10.5 16.542a1.476 1.476 0 011.449-1.53h.027a1.527 1.527 0 011.523 1.47 1.475 1.475 0 01-1.449 1.53h-.027a1.529 1.529 0 01-1.523-1.47zM11 12.5v-6a1 1 0 012 0v6a1 1 0 11-2 0z" }));
}, spinner: function() {
  return V.createElement("div", { className: "Toastify__spinner" });
} };
function ns(e) {
  const [, t] = Vn((h) => h + 1, 0), [r, n] = _e([]), o = ye(null), i = ye(/* @__PURE__ */ new Map()).current, a = (h) => r.indexOf(h) !== -1, s = ye({ toastKey: 1, displayedToast: 0, count: 0, queue: [], props: e, containerId: null, isToastActive: a, getToast: (h) => i.get(h) }).current;
  function l(h) {
    let { containerId: v } = h;
    const { limit: y } = s.props;
    !y || v && s.containerId !== v || (s.count -= s.queue.length, s.queue = []);
  }
  function c(h) {
    n((v) => h == null ? [] : v.filter((y) => y !== h));
  }
  function f() {
    const { toastContent: h, toastProps: v, staleId: y } = s.queue.shift();
    g(h, v, y);
  }
  function d(h, v) {
    let { delay: y, staleId: p, ...A } = v;
    if (!cr(h) || function(j) {
      return !o.current || s.props.enableMultiContainer && j.containerId !== s.props.containerId || i.has(j.toastId) && j.updateId == null;
    }(A))
      return;
    const { toastId: w, updateId: X, data: T } = A, { props: m } = s, M = () => c(w), Z = X == null;
    Z && s.count++;
    const z = { ...m, style: m.toastStyle, key: s.toastKey++, ...A, toastId: w, updateId: X, data: T, closeToast: M, isIn: !1, className: jt(A.className || m.toastClassName), bodyClassName: jt(A.bodyClassName || m.bodyClassName), progressClassName: jt(A.progressClassName || m.progressClassName), autoClose: !A.isLoading && (W = A.autoClose, C = m.autoClose, W === !1 || St(W) && W > 0 ? W : C), deleteToast() {
      const j = bn(i.get(w), "removed");
      i.delete(w), Re.emit(4, j);
      const F = s.queue.length;
      if (s.count = w == null ? s.count - s.displayedToast : s.count - 1, s.count < 0 && (s.count = 0), F > 0) {
        const H = w == null ? s.props.limit : 1;
        if (F === 1 || H === 1)
          s.displayedToast++, f();
        else {
          const G = H > F ? F : H;
          s.displayedToast = G;
          for (let ne = 0; ne < G; ne++)
            f();
        }
      } else
        t();
    } };
    var W, C;
    z.iconOut = function(j) {
      let { theme: F, type: H, isLoading: G, icon: ne } = j, re = null;
      const I = { theme: F, type: H };
      return ne === !1 || (Ve(ne) ? re = ne(I) : pt(ne) ? re = vr(ne, I) : at(ne) || St(ne) ? re = ne : G ? re = fr.spinner() : ((ve) => ve in fr)(H) && (re = fr[H](I))), re;
    }(z), Ve(A.onOpen) && (z.onOpen = A.onOpen), Ve(A.onClose) && (z.onClose = A.onClose), z.closeButton = m.closeButton, A.closeButton === !1 || cr(A.closeButton) ? z.closeButton = A.closeButton : A.closeButton === !0 && (z.closeButton = !cr(m.closeButton) || m.closeButton);
    let Y = h;
    pt(h) && !at(h.type) ? Y = vr(h, { closeToast: M, toastProps: z, data: T }) : Ve(h) && (Y = h({ closeToast: M, toastProps: z, data: T })), m.limit && m.limit > 0 && s.count > m.limit && Z ? s.queue.push({ toastContent: Y, toastProps: z, staleId: p }) : St(y) ? setTimeout(() => {
      g(Y, z, p);
    }, y) : g(Y, z, p);
  }
  function g(h, v, y) {
    const { toastId: p } = v;
    y && i.delete(y);
    const A = { content: h, props: v };
    i.set(p, A), n((w) => [...w, p].filter((X) => X !== y)), Re.emit(4, bn(A, A.props.updateId == null ? "added" : "updated"));
  }
  return be(() => (s.containerId = e.containerId, Re.cancelEmit(3).on(0, d).on(1, (h) => o.current && c(h)).on(5, l).emit(2, s), () => {
    i.clear(), Re.emit(3, s);
  }), []), be(() => {
    s.props = e, s.isToastActive = a, s.displayedToast = r.length;
  }), { getToastToRender: function(h) {
    const v = /* @__PURE__ */ new Map(), y = Array.from(i.values());
    return e.newestOnTop && y.reverse(), y.forEach((p) => {
      const { position: A } = p.props;
      v.has(A) || v.set(A, []), v.get(A).push(p);
    }), Array.from(v, (p) => h(p[0], p[1]));
  }, containerRef: o, isToastActive: a };
}
function An(e) {
  return e.targetTouches && e.targetTouches.length >= 1 ? e.targetTouches[0].clientX : e.clientX;
}
function wn(e) {
  return e.targetTouches && e.targetTouches.length >= 1 ? e.targetTouches[0].clientY : e.clientY;
}
function os(e) {
  const [t, r] = _e(!1), [n, o] = _e(!1), i = ye(null), a = ye({ start: 0, x: 0, y: 0, delta: 0, removalDistance: 0, canCloseOnClick: !0, canDrag: !1, boundingRect: null, didMove: !1 }).current, s = ye(e), { autoClose: l, pauseOnHover: c, closeToast: f, onClick: d, closeOnClick: g } = e;
  function h(T) {
    if (e.draggable) {
      T.nativeEvent.type === "touchstart" && T.nativeEvent.preventDefault(), a.didMove = !1, document.addEventListener("mousemove", A), document.addEventListener("mouseup", w), document.addEventListener("touchmove", A), document.addEventListener("touchend", w);
      const m = i.current;
      a.canCloseOnClick = !0, a.canDrag = !0, a.boundingRect = m.getBoundingClientRect(), m.style.transition = "", a.x = An(T.nativeEvent), a.y = wn(T.nativeEvent), e.draggableDirection === "x" ? (a.start = a.x, a.removalDistance = m.offsetWidth * (e.draggablePercent / 100)) : (a.start = a.y, a.removalDistance = m.offsetHeight * (e.draggablePercent === 80 ? 1.5 * e.draggablePercent : e.draggablePercent / 100));
    }
  }
  function v(T) {
    if (a.boundingRect) {
      const { top: m, bottom: M, left: Z, right: z } = a.boundingRect;
      T.nativeEvent.type !== "touchend" && e.pauseOnHover && a.x >= Z && a.x <= z && a.y >= m && a.y <= M ? p() : y();
    }
  }
  function y() {
    r(!0);
  }
  function p() {
    r(!1);
  }
  function A(T) {
    const m = i.current;
    a.canDrag && m && (a.didMove = !0, t && p(), a.x = An(T), a.y = wn(T), a.delta = e.draggableDirection === "x" ? a.x - a.start : a.y - a.start, a.start !== a.x && (a.canCloseOnClick = !1), m.style.transform = `translate${e.draggableDirection}(${a.delta}px)`, m.style.opacity = "" + (1 - Math.abs(a.delta / a.removalDistance)));
  }
  function w() {
    document.removeEventListener("mousemove", A), document.removeEventListener("mouseup", w), document.removeEventListener("touchmove", A), document.removeEventListener("touchend", w);
    const T = i.current;
    if (a.canDrag && a.didMove && T) {
      if (a.canDrag = !1, Math.abs(a.delta) > a.removalDistance)
        return o(!0), void e.closeToast();
      T.style.transition = "transform 0.2s, opacity 0.2s", T.style.transform = `translate${e.draggableDirection}(0)`, T.style.opacity = "1";
    }
  }
  be(() => {
    s.current = e;
  }), be(() => (i.current && i.current.addEventListener("d", y, { once: !0 }), Ve(e.onOpen) && e.onOpen(pt(e.children) && e.children.props), () => {
    const T = s.current;
    Ve(T.onClose) && T.onClose(pt(T.children) && T.children.props);
  }), []), be(() => (e.pauseOnFocusLoss && (document.hasFocus() || p(), window.addEventListener("focus", y), window.addEventListener("blur", p)), () => {
    e.pauseOnFocusLoss && (window.removeEventListener("focus", y), window.removeEventListener("blur", p));
  }), [e.pauseOnFocusLoss]);
  const X = { onMouseDown: h, onTouchStart: h, onMouseUp: v, onTouchEnd: v };
  return l && c && (X.onMouseEnter = p, X.onMouseLeave = y), g && (X.onClick = (T) => {
    d && d(T), a.canCloseOnClick && f();
  }), { playToast: y, pauseToast: p, isRunning: t, preventExitTransition: n, toastRef: i, eventHandlers: X };
}
function po(e) {
  let { closeToast: t, theme: r, ariaLabel: n = "close" } = e;
  return V.createElement("button", { className: `Toastify__close-button Toastify__close-button--${r}`, type: "button", onClick: (o) => {
    o.stopPropagation(), t(o);
  }, "aria-label": n }, V.createElement("svg", { "aria-hidden": "true", viewBox: "0 0 14 16" }, V.createElement("path", { fillRule: "evenodd", d: "M7.71 8.23l3.75 3.75-1.48 1.48-3.75-3.75-3.75 3.75L1 11.98l3.75-3.75L1 4.48 2.48 3l3.75 3.75L9.98 3l1.48 1.48-3.75 3.75z" })));
}
function as(e) {
  let { delay: t, isRunning: r, closeToast: n, type: o = "default", hide: i, className: a, style: s, controlledProgress: l, progress: c, rtl: f, isIn: d, theme: g } = e;
  const h = i || l && c === 0, v = { ...s, animationDuration: `${t}ms`, animationPlayState: r ? "running" : "paused", opacity: h ? 0 : 1 };
  l && (v.transform = `scaleX(${c})`);
  const y = $e("Toastify__progress-bar", l ? "Toastify__progress-bar--controlled" : "Toastify__progress-bar--animated", `Toastify__progress-bar-theme--${g}`, `Toastify__progress-bar--${o}`, { "Toastify__progress-bar--rtl": f }), p = Ve(a) ? a({ rtl: f, type: o, defaultClassName: y }) : $e(y, a);
  return V.createElement("div", { role: "progressbar", "aria-hidden": h ? "true" : "false", "aria-label": "notification timer", className: p, style: v, [l && c >= 1 ? "onTransitionEnd" : "onAnimationEnd"]: l && c < 1 ? null : () => {
    d && n();
  } });
}
const is = (e) => {
  const { isRunning: t, preventExitTransition: r, toastRef: n, eventHandlers: o } = os(e), { closeButton: i, children: a, autoClose: s, onClick: l, type: c, hideProgressBar: f, closeToast: d, transition: g, position: h, className: v, style: y, bodyClassName: p, bodyStyle: A, progressClassName: w, progressStyle: X, updateId: T, role: m, progress: M, rtl: Z, toastId: z, deleteToast: W, isIn: C, isLoading: Y, iconOut: j, closeOnClick: F, theme: H } = e, G = $e("Toastify__toast", `Toastify__toast-theme--${H}`, `Toastify__toast--${c}`, { "Toastify__toast--rtl": Z }, { "Toastify__toast--close-on-click": F }), ne = Ve(v) ? v({ rtl: Z, position: h, type: c, defaultClassName: G }) : $e(G, v), re = !!M || !s, I = { closeToast: d, type: c, theme: H };
  let ve = null;
  return i === !1 || (ve = Ve(i) ? i(I) : pt(i) ? vr(i, I) : po(I)), V.createElement(g, { isIn: C, done: W, position: h, preventExitTransition: r, nodeRef: n }, V.createElement("div", { id: z, onClick: l, className: ne, ...o, style: y, ref: n }, V.createElement("div", { ...C && { role: m }, className: Ve(p) ? p({ type: c }) : $e("Toastify__toast-body", p), style: A }, j != null && V.createElement("div", { className: $e("Toastify__toast-icon", { "Toastify--animate-icon Toastify__zoom-enter": !Y }) }, j), V.createElement("div", null, a)), ve, V.createElement(as, { ...T && !re ? { key: `pb-${T}` } : {}, rtl: Z, theme: H, delay: s, isRunning: t, isIn: C, closeToast: d, hide: f, type: c, style: X, className: w, controlledProgress: re, progress: M || 0 })));
}, tr = function(e, t) {
  return t === void 0 && (t = !1), { enter: `Toastify--animate Toastify__${e}-enter`, exit: `Toastify--animate Toastify__${e}-exit`, appendPosition: t };
}, ss = er(tr("bounce", !0));
er(tr("slide", !0));
er(tr("zoom"));
er(tr("flip"));
const Or = Cn((e, t) => {
  const { getToastToRender: r, containerRef: n, isToastActive: o } = ns(e), { className: i, style: a, rtl: s, containerId: l } = e;
  function c(f) {
    const d = $e("Toastify__toast-container", `Toastify__toast-container--${f}`, { "Toastify__toast-container--rtl": s });
    return Ve(i) ? i({ position: f, rtl: s, defaultClassName: d }) : $e(d, jt(i));
  }
  return be(() => {
    t && (t.current = n.current);
  }, []), V.createElement("div", { ref: n, className: "Toastify", id: l }, r((f, d) => {
    const g = d.length ? { ...a } : { ...a, pointerEvents: "none" };
    return V.createElement("div", { className: c(f), style: g, key: `container-${f}` }, d.map((h, v) => {
      let { content: y, props: p } = h;
      return V.createElement(is, { ...p, isIn: o(p.toastId), style: { ...p.style, "--nth": v + 1, "--len": d.length }, key: `toast-${p.key}` }, y);
    }));
  }));
});
Or.displayName = "ToastContainer", Or.defaultProps = { position: "top-right", transition: ss, autoClose: 5e3, closeButton: po, pauseOnHover: !0, pauseOnFocusLoss: !0, closeOnClick: !0, draggable: !0, draggablePercent: 80, draggableDirection: "x", role: "alert", theme: "light" };
let dr, rt = /* @__PURE__ */ new Map(), Tt = [], ls = 1;
function vo() {
  return "" + ls++;
}
function us(e) {
  return e && (at(e.toastId) || St(e.toastId)) ? e.toastId : vo();
}
function Xt(e, t) {
  return rt.size > 0 ? Re.emit(0, e, t) : Tt.push({ content: e, options: t }), t.toastId;
}
function Kt(e, t) {
  return { ...t, type: t && t.type || e, toastId: us(t) };
}
function kt(e) {
  return (t, r) => Xt(t, Kt(e, r));
}
function ge(e, t) {
  return Xt(e, Kt("default", t));
}
ge.loading = (e, t) => Xt(e, Kt("default", { isLoading: !0, autoClose: !1, closeOnClick: !1, closeButton: !1, draggable: !1, ...t })), ge.promise = function(e, t, r) {
  let n, { pending: o, error: i, success: a } = t;
  o && (n = at(o) ? ge.loading(o, r) : ge.loading(o.render, { ...r, ...o }));
  const s = { isLoading: null, autoClose: null, closeOnClick: null, closeButton: null, draggable: null, delay: 100 }, l = (f, d, g) => {
    if (d == null)
      return void ge.dismiss(n);
    const h = { type: f, ...s, ...r, data: g }, v = at(d) ? { render: d } : d;
    return n ? ge.update(n, { ...h, ...v }) : ge(v.render, { ...h, ...v }), g;
  }, c = Ve(e) ? e() : e;
  return c.then((f) => l("success", a, f)).catch((f) => l("error", i, f)), c;
}, ge.success = kt("success"), ge.info = kt("info"), ge.error = kt("error"), ge.warning = kt("warning"), ge.warn = ge.warning, ge.dark = (e, t) => Xt(e, Kt("default", { theme: "dark", ...t })), ge.dismiss = (e) => {
  rt.size > 0 ? Re.emit(1, e) : Tt = Tt.filter((t) => e != null && t.options.toastId !== e);
}, ge.clearWaitingQueue = function(e) {
  return e === void 0 && (e = {}), Re.emit(5, e);
}, ge.isActive = (e) => {
  let t = !1;
  return rt.forEach((r) => {
    r.isToastActive && r.isToastActive(e) && (t = !0);
  }), t;
}, ge.update = function(e, t) {
  t === void 0 && (t = {}), setTimeout(() => {
    const r = function(n, o) {
      let { containerId: i } = o;
      const a = rt.get(i || dr);
      return a && a.getToast(n);
    }(e, t);
    if (r) {
      const { props: n, content: o } = r, i = { ...n, ...t, toastId: t.toastId || e, updateId: vo() };
      i.toastId !== e && (i.staleId = e);
      const a = i.render || o;
      delete i.render, Xt(a, i);
    }
  }, 0);
}, ge.done = (e) => {
  ge.update(e, { progress: 1 });
}, ge.onChange = (e) => (Re.on(4, e), () => {
  Re.off(4, e);
}), ge.POSITION = { TOP_LEFT: "top-left", TOP_RIGHT: "top-right", TOP_CENTER: "top-center", BOTTOM_LEFT: "bottom-left", BOTTOM_RIGHT: "bottom-right", BOTTOM_CENTER: "bottom-center" }, ge.TYPE = { INFO: "info", SUCCESS: "success", WARNING: "warning", ERROR: "error", DEFAULT: "default" }, Re.on(2, (e) => {
  dr = e.containerId || e, rt.set(dr, e), Tt.forEach((t) => {
    Re.emit(0, t.content, t.options);
  }), Tt = [];
}).on(3, (e) => {
  rt.delete(e.containerId || e), rt.size === 0 && Re.off(0).off(1).off(5);
});
const cs = "data:font/woff;base64,", fs = "data:font/woff2;base64,", ds = "data:font/woff;base64,", ps = "data:font/woff2;base64,", vs = "data:font/woff;base64,", ms = "data:font/woff2;base64,", hs = Qn`
	${({ theme: { font: e, colors: t } }) => te`
		@font-face {
			font-family: Poppins;
			src: url(${ps}) format('woff2'),
				url(${ds}) format('woff');
			font-weight: 400;
		}
	
		@font-face {
			font-family: Poppins;
			src: url(${fs}) format('woff2'),
				url(${cs}) format('woff');
			font-weight: 500;
		}
	
		@font-face {
			font-family: Poppins;
			src: url(${ms}) format('woff2'),
				url(${vs}) format('woff');
			font-weight: 600;
		}
	
		:root {
			font-family: 'Poppins', sans-serif;
			font-weight: ${e.weights.regular};
		}
	
		*,
		*::before,
		*::after {
			box-sizing: border-box;
		}
	
		* {
			margin: 0;
		}
	
		main {
			display: block;
		}
	
		a {
			background-color: transparent;
		}
	
		img {
			border-style: none;
		}
	
		html,
		body {
			height: 100vh;
			font-size: ${e.sizes.default}
			line-height: ${e.lineHeight.default};
			color: ${t.secondary};
			background-color: ${({ theme: r }) => r.colors.main};
		}

		body {
			transition: background .5s, color .7s;
		}

		img,
		picture,
		video,
		canvas,
		svg {
			display: block;
			max-width: 100%;
		}

		input,
		button,
		textarea,
		select {
			font: inherit;
		}

		p,
		h1,
		h2,
		h3,
		h4,
		h5,
		h6 {
			overflow-wrap: break-word;
		}

		ul {
			list-style: none;
			padding: 0;
		}
	`}	
`, ys = Qn`
	${({ theme: { colors: e } }) => te`
			:root {
				--toastify-color-light: ${e.secondary};
				--toastify-color-dark: ${e.secondary};
				--toastify-color-info: ${e.toastInfo};
				--toastify-color-success: ${e.toastSuccess};
				--toastify-color-warning: ${e.toastWarning};
				--toastify-color-error: ${e.toastError};

				--toastify-icon-color-info: var(--toastify-color-info);
				--toastify-icon-color-success: var(--toastify-color-success);
				--toastify-icon-color-warning: var(--toastify-color-warning);
				--toastify-icon-color-error: var(--toastify-color-error);

				--toastify-toast-width: 320px;
				--toastify-toast-min-height: 64px;
				--toastify-toast-max-height: 800px;
				--toastify-font-family: 'Poppins', sans-serif;
				--toastify-z-index: 9999;

				--toastify-text-color-light: ${e.main};
				--toastify-text-color-dark: ${e.main};

				--toastify-spinner-color: #616161;
				--toastify-spinner-color-empty-area: #e0e0e0;

				--toastify-color-progress-info: var(--toastify-color-info);
				--toastify-color-progress-success: var(--toastify-color-success);
				--toastify-color-progress-warning: var(--toastify-color-warning);
				--toastify-color-progress-error: var(--toastify-color-error);
			}

			.Toastify__toast-container {
				z-index: var(--toastify-z-index);
				-webkit-transform: translate3d(0, 0, var(--toastify-z-index) px);
				position: fixed;
				padding: 4px;
				width: var(--toastify-toast-width);
				box-sizing: border-box;
				color: #fff;
			}
			.Toastify__toast-container--top-left {
				top: 1em;
				left: 1em;
			}
			.Toastify__toast-container--top-center {
				top: 1em;
				left: 50%;
				transform: translateX(-50%);
			}
			.Toastify__toast-container--top-right {
				top: 1em;
				right: 1em;
			}
			.Toastify__toast-container--bottom-left {
				bottom: 1em;
				left: 1em;
			}
			.Toastify__toast-container--bottom-center {
				bottom: 1em;
				left: 50%;
				transform: translateX(-50%);
			}
			.Toastify__toast-container--bottom-right {
				bottom: 1em;
				right: 1em;
			}

			@media only screen and (max-width: 480px) {
				.Toastify__toast-container {
					width: 100vw;
					padding: 0;
					left: 0;
					margin: 0;
				}
				.Toastify__toast-container--top-left,
				.Toastify__toast-container--top-center,
				.Toastify__toast-container--top-right {
					top: 0;
					transform: translateX(0);
				}
				.Toastify__toast-container--bottom-left,
				.Toastify__toast-container--bottom-center,
				.Toastify__toast-container--bottom-right {
					bottom: 0;
					transform: translateX(0);
				}
				.Toastify__toast-container--rtl {
					right: 0;
					left: initial;
				}
			}
			.Toastify__toast {
				position: relative;
				min-height: var(--toastify-toast-min-height);
				box-sizing: border-box;
				margin-bottom: 1rem;
				padding: 8px;
				border-radius: 4px;
				box-shadow: 0 1px 10px 0 rgba(0, 0, 0, 0.1), 0 2px 15px 0 rgba(0, 0, 0, 0.05);
				display: -ms-flexbox;
				display: flex;
				-ms-flex-pack: justify;
				justify-content: space-between;
				max-height: var(--toastify-toast-max-height);
				overflow: hidden;
				font-family: var(--toastify-font-family);
				cursor: default;
				direction: ltr;
				/* webkit only issue #791 */
				z-index: 0;
			}
			.Toastify__toast--rtl {
				direction: rtl;
			}
			.Toastify__toast--close-on-click {
				cursor: pointer;
			}
			.Toastify__toast-body {
				margin: auto 0;
				-ms-flex: 1 1 auto;
				flex: 1 1 auto;
				padding: 6px;
				display: -ms-flexbox;
				display: flex;
				-ms-flex-align: center;
				align-items: center;
			}
			.Toastify__toast-body > div:last-child {
				word-break: break-word;
				-ms-flex: 1;
				flex: 1;
			}
			.Toastify__toast-icon {
				-webkit-margin-end: 10px;
				margin-inline-end: 10px;
				width: 20px;
				-ms-flex-negative: 0;
				flex-shrink: 0;
				display: -ms-flexbox;
				display: flex;
			}

			.Toastify--animate {
				animation-fill-mode: both;
				animation-duration: 0.7s;
			}

			.Toastify--animate-icon {
				animation-fill-mode: both;
				animation-duration: 0.3s;
			}

			@media only screen and (max-width: 480px) {
				.Toastify__toast {
					margin-bottom: 0;
					border-radius: 0;
				}
			}
			.Toastify__toast-theme--dark {
				background: var(--toastify-color-dark);
				color: var(--toastify-text-color-dark);
			}
			.Toastify__toast-theme--light {
				background: var(--toastify-color-light);
				color: var(--toastify-text-color-light);
			}
			.Toastify__toast-theme--colored.Toastify__toast--default {
				background: var(--toastify-color-light);
				color: var(--toastify-text-color-light);
			}
			.Toastify__toast-theme--colored.Toastify__toast--info {
				color: var(--toastify-text-color-info);
				background: var(--toastify-color-info);
			}
			.Toastify__toast-theme--colored.Toastify__toast--success {
				color: var(--toastify-text-color-success);
				background: var(--toastify-color-success);
			}
			.Toastify__toast-theme--colored.Toastify__toast--warning {
				color: var(--toastify-text-color-warning);
				background: var(--toastify-color-warning);
			}
			.Toastify__toast-theme--colored.Toastify__toast--error {
				color: var(--toastify-text-color-error);
				background: var(--toastify-color-error);
			}

			.Toastify__progress-bar-theme--light {
				background: var(--toastify-color-progress-light);
			}
			.Toastify__progress-bar-theme--dark {
				background: var(--toastify-color-progress-dark);
			}
			.Toastify__progress-bar--info {
				background: var(--toastify-color-progress-info);
			}
			.Toastify__progress-bar--success {
				background: var(--toastify-color-progress-success);
			}
			.Toastify__progress-bar--warning {
				background: var(--toastify-color-progress-warning);
			}
			.Toastify__progress-bar--error {
				background: var(--toastify-color-progress-error);
			}
			.Toastify__progress-bar-theme--colored.Toastify__progress-bar--info,
			.Toastify__progress-bar-theme--colored.Toastify__progress-bar--success,
			.Toastify__progress-bar-theme--colored.Toastify__progress-bar--warning,
			.Toastify__progress-bar-theme--colored.Toastify__progress-bar--error {
				background: var(--toastify-color-transparent);
			}

			.Toastify__close-button {
				color: #fff;
				background: transparent;
				outline: none;
				border: none;
				padding: 0;
				cursor: pointer;
				opacity: 0.7;
				transition: 0.3s ease;
				-ms-flex-item-align: start;
				align-self: flex-start;
			}
			.Toastify__close-button--light {
				color: #000;
				opacity: 0.3;
			}
			.Toastify__close-button > svg {
				fill: currentColor;
				height: 16px;
				width: 14px;
			}
			.Toastify__close-button:hover,
			.Toastify__close-button:focus {
				opacity: 1;
			}

			@keyframes Toastify__trackProgress {
				0% {
					transform: scaleX(1);
				}
				100% {
					transform: scaleX(0);
				}
			}
			.Toastify__progress-bar {
				position: absolute;
				bottom: 0;
				left: 0;
				width: 100%;
				height: 5px;
				z-index: var(--toastify-z-index);
				opacity: 0.7;
				transform-origin: left;
			}
			.Toastify__progress-bar--animated {
				animation: Toastify__trackProgress linear 1 forwards;
			}
			.Toastify__progress-bar--controlled {
				transition: transform 0.2s;
			}
			.Toastify__progress-bar--rtl {
				right: 0;
				left: initial;
				transform-origin: right;
			}

			.Toastify__spinner {
				width: 20px;
				height: 20px;
				box-sizing: border-box;
				border: 2px solid;
				border-radius: 100%;
				border-color: var(--toastify-spinner-color-empty-area);
				border-right-color: var(--toastify-spinner-color);
				animation: Toastify__spin 0.65s linear infinite;
			}

			@keyframes Toastify__bounceInRight {
				from,
				60%,
				75%,
				90%,
				to {
					animation-timing-function: cubic-bezier(0.215, 0.61, 0.355, 1);
				}
				from {
					opacity: 0;
					transform: translate3d(3000px, 0, 0);
				}
				60% {
					opacity: 1;
					transform: translate3d(-25px, 0, 0);
				}
				75% {
					transform: translate3d(10px, 0, 0);
				}
				90% {
					transform: translate3d(-5px, 0, 0);
				}
				to {
					transform: none;
				}
			}
			@keyframes Toastify__bounceOutRight {
				20% {
					opacity: 1;
					transform: translate3d(-20px, 0, 0);
				}
				to {
					opacity: 0;
					transform: translate3d(2000px, 0, 0);
				}
			}
			@keyframes Toastify__bounceInLeft {
				from,
				60%,
				75%,
				90%,
				to {
					animation-timing-function: cubic-bezier(0.215, 0.61, 0.355, 1);
				}
				0% {
					opacity: 0;
					transform: translate3d(-3000px, 0, 0);
				}
				60% {
					opacity: 1;
					transform: translate3d(25px, 0, 0);
				}
				75% {
					transform: translate3d(-10px, 0, 0);
				}
				90% {
					transform: translate3d(5px, 0, 0);
				}
				to {
					transform: none;
				}
			}
			@keyframes Toastify__bounceOutLeft {
				20% {
					opacity: 1;
					transform: translate3d(20px, 0, 0);
				}
				to {
					opacity: 0;
					transform: translate3d(-2000px, 0, 0);
				}
			}
			@keyframes Toastify__bounceInUp {
				from,
				60%,
				75%,
				90%,
				to {
					animation-timing-function: cubic-bezier(0.215, 0.61, 0.355, 1);
				}
				from {
					opacity: 0;
					transform: translate3d(0, 3000px, 0);
				}
				60% {
					opacity: 1;
					transform: translate3d(0, -20px, 0);
				}
				75% {
					transform: translate3d(0, 10px, 0);
				}
				90% {
					transform: translate3d(0, -5px, 0);
				}
				to {
					transform: translate3d(0, 0, 0);
				}
			}
			@keyframes Toastify__bounceOutUp {
				20% {
					transform: translate3d(0, -10px, 0);
				}
				40%,
				45% {
					opacity: 1;
					transform: translate3d(0, 20px, 0);
				}
				to {
					opacity: 0;
					transform: translate3d(0, -2000px, 0);
				}
			}
			@keyframes Toastify__bounceInDown {
				from,
				60%,
				75%,
				90%,
				to {
					animation-timing-function: cubic-bezier(0.215, 0.61, 0.355, 1);
				}
				0% {
					opacity: 0;
					transform: translate3d(0, -3000px, 0);
				}
				60% {
					opacity: 1;
					transform: translate3d(0, 25px, 0);
				}
				75% {
					transform: translate3d(0, -10px, 0);
				}
				90% {
					transform: translate3d(0, 5px, 0);
				}
				to {
					transform: none;
				}
			}
			@keyframes Toastify__bounceOutDown {
				20% {
					transform: translate3d(0, 10px, 0);
				}
				40%,
				45% {
					opacity: 1;
					transform: translate3d(0, -20px, 0);
				}
				to {
					opacity: 0;
					transform: translate3d(0, 2000px, 0);
				}
			}
			.Toastify__bounce-enter--top-left,
			.Toastify__bounce-enter--bottom-left {
				animation-name: Toastify__bounceInLeft;
			}
			.Toastify__bounce-enter--top-right,
			.Toastify__bounce-enter--bottom-right {
				animation-name: Toastify__bounceInRight;
			}
			.Toastify__bounce-enter--top-center {
				animation-name: Toastify__bounceInDown;
			}
			.Toastify__bounce-enter--bottom-center {
				animation-name: Toastify__bounceInUp;
			}

			.Toastify__bounce-exit--top-left,
			.Toastify__bounce-exit--bottom-left {
				animation-name: Toastify__bounceOutLeft;
			}
			.Toastify__bounce-exit--top-right,
			.Toastify__bounce-exit--bottom-right {
				animation-name: Toastify__bounceOutRight;
			}
			.Toastify__bounce-exit--top-center {
				animation-name: Toastify__bounceOutUp;
			}
			.Toastify__bounce-exit--bottom-center {
				animation-name: Toastify__bounceOutDown;
			}

			@keyframes Toastify__zoomIn {
				from {
					opacity: 0;
					transform: scale3d(0.3, 0.3, 0.3);
				}
				50% {
					opacity: 1;
				}
			}
			@keyframes Toastify__zoomOut {
				from {
					opacity: 1;
				}
				50% {
					opacity: 0;
					transform: scale3d(0.3, 0.3, 0.3);
				}
				to {
					opacity: 0;
				}
			}
			.Toastify__zoom-enter {
				animation-name: Toastify__zoomIn;
			}

			.Toastify__zoom-exit {
				animation-name: Toastify__zoomOut;
			}

			@keyframes Toastify__flipIn {
				from {
					transform: perspective(400px) rotate3d(1, 0, 0, 90deg);
					animation-timing-function: ease-in;
					opacity: 0;
				}
				40% {
					transform: perspective(400px) rotate3d(1, 0, 0, -20deg);
					animation-timing-function: ease-in;
				}
				60% {
					transform: perspective(400px) rotate3d(1, 0, 0, 10deg);
					opacity: 1;
				}
				80% {
					transform: perspective(400px) rotate3d(1, 0, 0, -5deg);
				}
				to {
					transform: perspective(400px);
				}
			}
			@keyframes Toastify__flipOut {
				from {
					transform: perspective(400px);
				}
				30% {
					transform: perspective(400px) rotate3d(1, 0, 0, -20deg);
					opacity: 1;
				}
				to {
					transform: perspective(400px) rotate3d(1, 0, 0, 90deg);
					opacity: 0;
				}
			}
			.Toastify__flip-enter {
				animation-name: Toastify__flipIn;
			}

			.Toastify__flip-exit {
				animation-name: Toastify__flipOut;
			}

			@keyframes Toastify__slideInRight {
				from {
					transform: translate3d(110%, 0, 0);
					visibility: visible;
				}
				to {
					transform: translate3d(0, 0, 0);
				}
			}
			@keyframes Toastify__slideInLeft {
				from {
					transform: translate3d(-110%, 0, 0);
					visibility: visible;
				}
				to {
					transform: translate3d(0, 0, 0);
				}
			}
			@keyframes Toastify__slideInUp {
				from {
					transform: translate3d(0, 110%, 0);
					visibility: visible;
				}
				to {
					transform: translate3d(0, 0, 0);
				}
			}
			@keyframes Toastify__slideInDown {
				from {
					transform: translate3d(0, -110%, 0);
					visibility: visible;
				}
				to {
					transform: translate3d(0, 0, 0);
				}
			}
			@keyframes Toastify__slideOutRight {
				from {
					transform: translate3d(0, 0, 0);
				}
				to {
					visibility: hidden;
					transform: translate3d(110%, 0, 0);
				}
			}
			@keyframes Toastify__slideOutLeft {
				from {
					transform: translate3d(0, 0, 0);
				}
				to {
					visibility: hidden;
					transform: translate3d(-110%, 0, 0);
				}
			}
			@keyframes Toastify__slideOutDown {
				from {
					transform: translate3d(0, 0, 0);
				}
				to {
					visibility: hidden;
					transform: translate3d(0, 500px, 0);
				}
			}
			@keyframes Toastify__slideOutUp {
				from {
					transform: translate3d(0, 0, 0);
				}
				to {
					visibility: hidden;
					transform: translate3d(0, -500px, 0);
				}
			}
			.Toastify__slide-enter--top-left,
			.Toastify__slide-enter--bottom-left {
				animation-name: Toastify__slideInLeft;
			}
			.Toastify__slide-enter--top-right,
			.Toastify__slide-enter--bottom-right {
				animation-name: Toastify__slideInRight;
			}
			.Toastify__slide-enter--top-center {
				animation-name: Toastify__slideInDown;
			}
			.Toastify__slide-enter--bottom-center {
				animation-name: Toastify__slideInUp;
			}

			.Toastify__slide-exit--top-left,
			.Toastify__slide-exit--bottom-left {
				animation-name: Toastify__slideOutLeft;
			}
			.Toastify__slide-exit--top-right,
			.Toastify__slide-exit--bottom-right {
				animation-name: Toastify__slideOutRight;
			}
			.Toastify__slide-exit--top-center {
				animation-name: Toastify__slideOutUp;
			}
			.Toastify__slide-exit--bottom-center {
				animation-name: Toastify__slideOutDown;
			}

			@keyframes Toastify__spin {
				from {
					transform: rotate(0deg);
				}
				to {
					transform: rotate(360deg);
				}
			}
		`}
`, Sr = {
  MOBILE: 428,
  TABLET: 1200
}, ee = {
  Grey100: "#090909",
  Grey100_50: "#09090980",
  Grey200: "#111111",
  Grey300: "#3C3C3C",
  Grey400: "#797979",
  White100: "#EAEAEA",
  White200: "#EEEEEE",
  White300: "#DDDDDD",
  White300_50: "#DDDDDD80",
  Red100: "#F97777",
  Red200: "#F86565",
  Orange100: "#EFA360",
  Orange200: "#F18F5D",
  Yellow100: "#DECC56",
  Yellow200: "#E4CF49",
  Green100: "#D7E66D",
  Green200: "#D6E560",
  Green300: "#8EE296",
  Green400: "#83DF8C",
  Blue100: "#83CDE8",
  Blue200: "#77C8E6"
}, mo = {
  mini: "4px",
  tiny: "8px",
  small: "12px",
  medium: "20px",
  big: "40px",
  large: "60px",
  huge: "80px"
}, ho = {
  weights: {
    regular: 400,
    medium: 500,
    bold: 600
  },
  sizes: {
    error: "12px",
    default: "16px",
    info: "18px",
    h1: "28px",
    h2: "18px",
    icon: "64px",
    notFound: "128px"
  },
  lineHeight: {
    default: "24px",
    h1: "44px",
    h2: "28px"
  },
  letterSpacing: {
    default: "normal",
    h1: "0.1em",
    h2: "0.02em"
  }
}, yo = {
  mobile: Sr.MOBILE,
  tablet: Sr.TABLET
}, gs = {
  colors: {
    main: ee.White100,
    secondary: ee.Grey100,
    secondary_opacity: ee.Grey100_50,
    color1: ee.Grey300,
    color2: ee.Grey200,
    color3: ee.White200,
    color4: ee.Grey300,
    color5: ee.Grey400,
    weak: ee.Orange100,
    good: ee.Green100,
    error: ee.Red100,
    warning: ee.Yellow100,
    success: ee.Green300,
    info: ee.Blue100,
    toastError: ee.Red200,
    toastWarning: ee.Yellow200,
    toastSuccess: ee.Green400,
    toastInfo: ee.Blue200
  },
  font: ho,
  spaces: mo,
  breakpoints: yo
}, bs = {
  colors: {
    main: ee.Grey100,
    secondary: ee.White300,
    secondary_opacity: ee.White300_50,
    color1: ee.White100,
    color2: ee.White200,
    color3: ee.Grey200,
    color4: ee.Grey300,
    color5: ee.Grey400,
    weak: ee.Orange200,
    good: ee.Green200,
    error: ee.Red200,
    warning: ee.Yellow200,
    success: ee.Green400,
    info: ee.Blue200,
    toastError: ee.Red100,
    toastWarning: ee.Yellow100,
    toastSuccess: ee.Green300,
    toastInfo: ee.Blue100
  },
  font: ho,
  spaces: mo,
  breakpoints: yo
}, nl = ({ theme: e = "dark", children: t }) => /* @__PURE__ */ ze(Fa, { theme: e === "dark" ? bs : gs, children: [
  /* @__PURE__ */ U(hs, {}),
  /* @__PURE__ */ U(Or, { theme: e }),
  /* @__PURE__ */ U(ys, {}),
  /* @__PURE__ */ U(Rr.Provider, { children: t })
] });
var As = process.env.NODE_ENV === "production";
function Xr(e, t) {
  if (!As) {
    if (e)
      return;
    var r = "Warning: " + t;
    typeof console < "u" && console.warn(r);
    try {
      throw Error(r);
    } catch {
    }
  }
}
function Pn(e, t) {
  for (var r = 0; r < t.length; r++) {
    var n = t[r];
    n.enumerable = n.enumerable || !1, n.configurable = !0, "value" in n && (n.writable = !0), Object.defineProperty(e, n.key, n);
  }
}
function rr(e, t, r) {
  return t && Pn(e.prototype, t), r && Pn(e, r), e;
}
function We() {
  return We = Object.assign || function(e) {
    for (var t = 1; t < arguments.length; t++) {
      var r = arguments[t];
      for (var n in r)
        Object.prototype.hasOwnProperty.call(r, n) && (e[n] = r[n]);
    }
    return e;
  }, We.apply(this, arguments);
}
function ws(e, t) {
  e.prototype = Object.create(t.prototype), e.prototype.constructor = e, Nr(e, t);
}
function Nr(e, t) {
  return Nr = Object.setPrototypeOf || function(n, o) {
    return n.__proto__ = o, n;
  }, Nr(e, t);
}
function Ps(e, t) {
  if (e == null)
    return {};
  var r = {}, n = Object.keys(e), o, i;
  for (i = 0; i < n.length; i++)
    o = n[i], !(t.indexOf(o) >= 0) && (r[o] = e[o]);
  return r;
}
function Ts(e, t) {
  if (e) {
    if (typeof e == "string")
      return Tn(e, t);
    var r = Object.prototype.toString.call(e).slice(8, -1);
    if (r === "Object" && e.constructor && (r = e.constructor.name), r === "Map" || r === "Set")
      return Array.from(e);
    if (r === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r))
      return Tn(e, t);
  }
}
function Tn(e, t) {
  (t == null || t > e.length) && (t = e.length);
  for (var r = 0, n = new Array(t); r < t; r++)
    n[r] = e[r];
  return n;
}
function gt(e, t) {
  var r = typeof Symbol < "u" && e[Symbol.iterator] || e["@@iterator"];
  if (r)
    return (r = r.call(e)).next.bind(r);
  if (Array.isArray(e) || (r = Ts(e)) || t && e && typeof e.length == "number") {
    r && (e = r);
    var n = 0;
    return function() {
      return n >= e.length ? {
        done: !0
      } : {
        done: !1,
        value: e[n++]
      };
    };
  }
  throw new TypeError(`Invalid attempt to iterate non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
}
function On(e) {
  var t = ye(null);
  function r(n) {
    !n || n === t.current || (t.current = n, e(n));
  }
  return r;
}
function Os(e) {
  var t = ye(e);
  return Lr(function() {
    function r(o) {
      typeof o == "function" ? t.current = o(t.current) : t.current = o;
    }
    function n() {
      return t.current;
    }
    return [n, r];
  }, []);
}
function Ss() {
  var e = ye([]);
  return Lr(function() {
    function t() {
      return e.current.length > 0;
    }
    function r() {
      for (var o = gt(e.current), i; !(i = o()).done; ) {
        var a = i.value;
        a();
      }
      e.current = [];
    }
    function n(o) {
      e.current.push(o);
    }
    return {
      hasEventSubscriptions: t,
      removeAllEventSubscriptions: r,
      addEventSubscription: n
    };
  }, []);
}
var pr = typeof window < "u" ? Er : be;
function Xs(e, t) {
  var r = ye(e);
  return t ? (r.current = e, r) : (r.current = null, r);
}
function Lt(e) {
  return parseFloat(e.replace("px", ""));
}
function qr(e, t, r) {
  return e < t ? t : e > r ? r : e;
}
function Sn(e) {
  return e != null;
}
function Xn() {
  for (var e = arguments.length, t = new Array(e), r = 0; r < e; r++)
    t[r] = arguments[r];
  return function(n) {
    for (var o = gt(t), i; !(i = o()).done; ) {
      var a = i.value;
      a && (typeof a == "function" ? a(n) : a.current = n);
    }
  };
}
function Ns(e, t) {
  if (!(typeof e > "u"))
    return t || e.ResizeObserver;
}
function go(e, t) {
  var r = [];
  if (!e || !t || e === document.body)
    return r;
  var n = t.getComputedStyle(e), o = n.overflow, i = n.overflowX, a = n.overflowY;
  return [o, i, a].some(function(s) {
    return ["auto", "scroll"].includes(s);
  }) && r.push(e), [].concat(r, go(e.parentElement, t));
}
function Mt(e) {
  return "react-laag: Could not find a valid reference for the " + e + ` element. There might be 2 causes:
   - Make sure that the 'ref' is set correctly on the ` + e + ` element when isOpen: true. Also make sure your component forwards the ref with "forwardRef()".
   - Make sure that you are actually rendering the ` + e + " when the isOpen prop is set to true";
}
function qs(e) {
  var t = e.enabled, r = e.onChange, n = e.environment, o = e.ResizeObserverPolyfill, i = e.overflowContainer, a = e.triggerOption, s = Ns(n, o);
  be(function() {
    process.env.NODE_ENV !== "production" && Xr(s, "This browser does not support ResizeObserver out of the box. We recommend to add a polyfill in order to utilize the full capabilities of react-laag. See: https://github.com/everweij/react-laag#resize-observer");
  }, [s]);
  var l = ye(null), c = Boolean(a), f = Os({
    scrollContainers: [],
    trigger: null,
    layer: null
  }), d = f[0], g = f[1], h = Ss(), v = h.hasEventSubscriptions, y = h.addEventSubscription, p = h.removeAllEventSubscriptions, A = xe(function() {
    var W = d(), C = W.layer, Y = W.trigger, j = W.scrollContainers, F = j[0];
    if (!C)
      throw new Error(Mt("layer"));
    if (!Y && !c)
      throw new Error(Mt("trigger"));
    var H = {
      top: 0,
      left: 0
    };
    if (F) {
      var G = F.scrollLeft, ne = F.scrollTop;
      H = {
        top: ne,
        left: G
      };
    } else {
      var re = n.scrollX, I = n.scrollY;
      H = {
        top: I,
        left: re
      };
    }
    var ve = {
      left: 0,
      top: 0
    };
    if (F) {
      var he = n.getComputedStyle(F), Ne = he.borderLeftWidth, L = he.borderTopWidth;
      ve = {
        left: Lt(Ne) || 0,
        top: Lt(L) || 0
      };
    }
    r({
      layer: C,
      trigger: Y,
      scrollContainers: j,
      arrow: l.current
    }, H, ve);
  }, [d, r, n, l, c]), w = xe(function() {
    var W = d(), C = W.trigger, Y = W.layer, j = W.scrollContainers;
    if (!Y)
      throw new Error(Mt("layer"));
    if (!C && !c)
      throw new Error(Mt("trigger"));
    if (s) {
      for (var F = !1, H = function() {
        if (!F) {
          F = !0;
          return;
        }
        A();
      }, G = new s(H), ne = 0, re = [C, Y, document.body]; ne < re.length; ne++) {
        var I = re[ne];
        I && G.observe(I);
      }
      y(function() {
        for (var N = 0, q = [C, Y, document.body]; N < q.length; N++) {
          var k = q[N];
          k && G.unobserve(k);
        }
        G.disconnect();
      });
    }
    for (var ve = [n].concat(j), he = function() {
      var q = L.value;
      q.addEventListener("scroll", A), y(function() {
        return q.removeEventListener("scroll", A);
      });
    }, Ne = gt(ve), L; !(L = Ne()).done; )
      he();
  }, [d, y, A, n, s, c]), X = xe(function(z, W) {
    t && z && z !== W && (p(), w(), A());
  }, [p, w, A, t]), T = On(xe(function(z) {
    var W = d(), C = W.layer;
    g(function(Y) {
      return We({}, Y, {
        layer: z
      });
    }), X(C, z);
  }, [d, g, X])), m = xe(function(W) {
    var C = go(W, n), Y = C[0];
    if (Y) {
      var j = n.getComputedStyle(Y).position, F = ["relative", "absolute", "fixed"].includes(j) || i;
      F || (Y.style.position = "relative"), process.env.NODE_ENV !== "production" && Xr(F, `react-laag: Set the 'position' style of the nearest scroll-container to 'relative', 'absolute' or 'fixed', or set the 'overflowContainer' prop to true. This is needed in order to position the layer properly. Currently the scroll-container is positioned: "` + j + '". For now, "position: relative;" is added for you, but this behavior might be removed in the future. Visit https://react-laag.com/docs/#position-relative for more info.');
    }
    return C;
  }, [n, i]), M = On(xe(function(z) {
    var W = m(z), C = d(), Y = C.trigger;
    g(function(j) {
      return We({}, j, {
        trigger: z,
        scrollContainers: W
      });
    }), X(Y, z);
  }, [d, g, X, m])), Z = a == null || a.getParent == null ? void 0 : a.getParent();
  return pr(function() {
    Z && g(function(z) {
      return We({}, z, {
        scrollContainers: m(Z)
      });
    });
  }, [Z, g, m]), pr(function() {
    return t && (v() || w()), function() {
      v() && p();
    };
  }, [t, v, w, p]), pr(function() {
    t && A();
  }), {
    triggerRef: M,
    layerRef: T,
    arrowRef: l,
    closestScrollContainer: d().scrollContainers[0] || null
  };
}
var bo = /* @__PURE__ */ Wn({});
function xs(e) {
  var t = e.children, r = e.registrations, n = xe(function(i) {
    return r.current.add(i), function() {
      return r.current.delete(i);
    };
  }, [r]);
  return ft(bo.Provider, {
    value: n
  }, t);
}
function Es(e, t) {
  for (var r = gt(e), n; !(n = r()).done; ) {
    var o = n.value.shouldCloseWhenClickedOutside;
    if (!o(t))
      return !1;
  }
  return !0;
}
function Ls(e) {
  var t = e.isOpen, r = e.onOutsideClick, n = e.onParentClose, o = ye(null), i = ye(null), a = ye(/* @__PURE__ */ new Set()), s = De(bo), l = xe(function(f) {
    var d = f.target, g = o.current && o.current.contains(d), h = i.current && i.current.contains(d), v = Es(a.current, f);
    return h && v && a.current.forEach(function(y) {
      var p = y.closeChild;
      return p();
    }), !g && !h && v;
  }, [o, i, a]);
  return be(function() {
    if (typeof s == "function")
      return s({
        shouldCloseWhenClickedOutside: l,
        closeChild: function() {
          process.env.NODE_ENV !== "production" && Xr(n, "react-laag: You are using useLayer() in a nested setting but forgot to set the 'onParentClose()' callback in the options. This could lead to unexpected behavior."), n && n();
        }
      });
  }, [s, l, n, a]), be(function() {
    var c = typeof s == "function", f = !t || !r || c;
    if (f)
      return;
    function d(g) {
      l(g) && r();
    }
    return document.addEventListener("click", d, !0), function() {
      return document.removeEventListener("click", d, !0);
    };
  }, [t, r, l, s]), be(function() {
    t || a.current.forEach(function(c) {
      var f = c.closeChild;
      return f();
    });
  }, [t]), {
    closeOnOutsideClickRefs: {
      trigger: o,
      layer: i
    },
    registrations: a
  };
}
var zs = ["bottom-start", "bottom-end", "bottom-center", "top-start", "top-center", "top-end", "left-end", "left-center", "left-start", "right-end", "right-center", "right-start", "center"], Ws = {
  top: "bottom",
  left: "right",
  bottom: "top",
  right: "left",
  center: "center"
}, Vs = /* @__PURE__ */ function() {
  function e(r, n, o, i, a, s, l, c, f) {
    this.prop = void 0, this.opposite = void 0, this.isHorizontal = void 0, this.sizeProp = void 0, this.oppositeSizeProp = void 0, this.cssProp = void 0, this.oppositeCssProp = void 0, this.isCenter = void 0, this.isPush = void 0, this.prop = r, this.opposite = n, this.isHorizontal = o, this.sizeProp = i, this.oppositeSizeProp = a, this.cssProp = s, this.oppositeCssProp = l, this.isCenter = c, this.isPush = f;
  }
  var t = e.prototype;
  return t.factor = function(n) {
    return n * (this.isPush ? 1 : -1);
  }, t.isOppositeDirection = function(n) {
    return this.isHorizontal !== n.isHorizontal;
  }, e;
}();
function ct(e, t) {
  t === void 0 && (t = !0);
  var r = ["left", "right"].includes(e);
  return new Vs(e, t ? ct(Ws[e], !1) : null, r, r ? "width" : "height", r ? "height" : "width", r ? "left" : "top", r ? "top" : "left", e === "center", !["right", "bottom"].includes(e));
}
var Xe = {
  top: /* @__PURE__ */ ct("top"),
  bottom: /* @__PURE__ */ ct("bottom"),
  left: /* @__PURE__ */ ct("left"),
  right: /* @__PURE__ */ ct("right")
}, Se = /* @__PURE__ */ We({}, Xe, {
  center: /* @__PURE__ */ ct("center")
}), Rt = ["top", "left", "bottom", "right"], Dr = /* @__PURE__ */ function() {
  function e(t) {
    return this.top = void 0, this.left = void 0, this.right = void 0, this.bottom = void 0, Object.assign(this, t);
  }
  return e.mergeSmallestSides = function(r) {
    var n = r[0], o = r.slice(1);
    if (!n)
      throw new Error("Please provide at least 1 bounds objects in order to merge");
    for (var i = Object.fromEntries(Rt.map(function(g) {
      return [g, n[g]];
    })), a = gt(o), s; !(s = a()).done; )
      for (var l = s.value, c = gt(Rt), f; !(f = c()).done; ) {
        var d = f.value;
        i[d] = Math.min(i[d], l[d]);
      }
    return new e(i);
  }, rr(e, [{
    key: "allSidesArePositive",
    get: function() {
      var r = this;
      return Rt.every(function(n) {
        return r[n] >= 0;
      });
    }
    /**
     * Returns a partial IBoundsOffsets with sides that are negative, meaning sides aren't entirely
     * visible in respect to a parent Bounds instance
     */
  }, {
    key: "negativeSides",
    get: function() {
      var r = this;
      return Object.fromEntries(Rt.filter(function(n) {
        return r[n] < 0;
      }).map(function(n) {
        return [n, r[n]];
      }));
    }
  }]), e;
}();
function Nn() {
  for (var e = arguments.length, t = new Array(e), r = 0; r < e; r++)
    t[r] = arguments[r];
  return t.reduce(function(n, o) {
    return n + (o ? Lt(o) : 0);
  }, 0);
}
function xr(e) {
  var t = e.top, r = e.left, n = e.right, o = e.bottom, i = e.width, a = e.height;
  return {
    top: t,
    left: r,
    right: n,
    bottom: o,
    width: i,
    height: a
  };
}
var Cs = {
  top: 0,
  left: 0,
  right: 0,
  bottom: 0,
  width: 0,
  height: 0
}, Ze = /* @__PURE__ */ function() {
  e.create = function(n) {
    return new e(n);
  }, e.fromElement = function(n, o) {
    o === void 0 && (o = {});
    var i = o, a = i.withTransform, s = a === void 0 ? !0 : a, l = i.environment, c = l === void 0 ? window : l, f = i.withScrollbars, d = f === void 0 ? !0 : f, g = xr(n.getBoundingClientRect()), h = new e(g);
    if (!s) {
      var v = c.getComputedStyle(n), y = v.width, p = v.height, A = v.boxSizing, w = v.borderLeft, X = v.borderRight, T = v.borderTop, m = v.borderBottom, M = v.paddingLeft, Z = v.paddingRight, z = v.paddingTop, W = v.paddingBottom, C = A === "border-box" ? Lt(y) : Nn(y, w, X, M, Z), Y = A === "border-box" ? Lt(p) : Nn(p, T, m, z, W);
      h = new e(We({}, h, {
        width: C,
        height: Y
      }));
    }
    if (!d) {
      var j = h.width - n.clientWidth, F = h.height - n.clientHeight;
      return h.substract({
        right: j,
        bottom: F
      });
    }
    return h;
  }, e.empty = function() {
    return new e();
  }, e.fromWindow = function(n) {
    var o, i = (o = n == null ? void 0 : n.document.scrollingElement) != null ? o : n == null ? void 0 : n.document.documentElement, a = i ?? {}, s = a.clientWidth, l = s === void 0 ? 0 : s, c = a.clientHeight, f = c === void 0 ? 0 : c;
    return new e({
      width: l,
      height: f,
      right: l,
      bottom: f
    });
  };
  function e(r) {
    return r === void 0 && (r = {}), this.top = void 0, this.left = void 0, this.right = void 0, this.bottom = void 0, this.width = void 0, this.height = void 0, Object.assign(this, Cs, r);
  }
  var t = e.prototype;
  return t.toObject = function() {
    return xr(this);
  }, t.merge = function(n) {
    var o = this.toObject();
    return new e(We({}, o, typeof n == "function" ? n(o) : n));
  }, t.substract = function(n) {
    for (var o = this.toObject(), i = Object.entries(n), a = 0, s = i; a < s.length; a++) {
      var l = s[a], c = l[0], f = l[1];
      if (c in Xe) {
        var d = Xe[c];
        o[c] += d.factor(f), o[d.isHorizontal ? "width" : "height"] -= f;
      } else
        o[c] -= f || 0;
    }
    return new e(o);
  }, t.offsetsTo = function(n) {
    return new Dr({
      top: n.top - this.top,
      bottom: this.bottom - n.bottom,
      left: n.left - this.left,
      right: this.right - n.right
    });
  }, t.mapSides = function(n) {
    for (var o = this.toObject(), i = Object.values(Xe), a = 0, s = i; a < s.length; a++) {
      var l = s[a];
      o[l.prop] = n(l, o[l.prop]);
    }
    return new e(o);
  }, rr(e, [{
    key: "surface",
    get: function() {
      return this.width * this.height;
    }
  }]), e;
}(), Ao = /* @__PURE__ */ function() {
  function e(r, n, o, i, a) {
    this.primary = void 0, this.secondary = void 0, this.offsets = void 0, this.subjectsBounds = void 0, this._cachedLayerBounds = null, this._cachedContainerOffsets = null, this.primary = r, this.secondary = n, this.offsets = a, this.setSubjectsBounds(o, i);
  }
  var t = e.prototype;
  return t.setSubjectsBounds = function(n, o) {
    if (!o) {
      this.subjectsBounds = n;
      return;
    }
    var i = (
      // if the user passed a callback, call it with the layerSide corresponding to
      // the placement
      typeof o == "function" ? o(this.primary.prop) : o
    );
    this.subjectsBounds = n.merge({
      layer: We({}, n.layer, i)
    });
  }, t.getLayerBounds = function(n) {
    if (n === void 0 && (n = 0), this._cachedLayerBounds && n === 0)
      return this._cachedLayerBounds;
    var o = this.primary, i = this.secondary, a = this.subjectsBounds, s = a.trigger, l = a.layer, c = a.arrow, f = o.isHorizontal, d = o.oppositeCssProp, g = o.oppositeSizeProp, h = o.prop, v = o.opposite, y = Ze.empty();
    y[v.prop] = s[h] - o.factor(this.offsets.trigger), y[h] = y[v.prop] - o.factor(l[o.sizeProp]);
    var p = this.offsets.arrow * 2, A = s[d] - (l[g] - c[g]) + p, w = s[d] + (s[g] - c[g]) - p;
    if (i.isPush || (A += l[g], w += l[g]), i.isCenter) {
      var X = (f ? Xe.top : Xe.left).prop, T = (f ? Xe.bottom : Xe.right).prop;
      y[X] = qr(s[X] + s[g] / 2 - l[g] / 2 + n, A, w), y[T] = y[X] + l[g];
    } else {
      var m = i, M = s[m.prop], Z = M < A ? A - M : M > w ? w - M : 0;
      y[m.prop] = qr(M + n + Z, A, w), y[m.opposite.prop] = y[m.prop] + i.factor(l[g]);
    }
    y.width = y.right - y.left, y.height = y.bottom - y.top;
    var z = Ze.create(y);
    return n === 0 && (this._cachedLayerBounds = z), z;
  }, t.getLayerCollisionBounds = function() {
    var n = this.offsets.container;
    return this.getLayerBounds().mapSides(function(o, i) {
      return i -= o.factor(n);
    }).merge(function(o) {
      var i = o.width, a = o.height;
      return {
        width: i + n * 2,
        height: a + n * 2
      };
    });
  }, t.getContainerOffsets = function(n) {
    if (this._cachedContainerOffsets && !n)
      return this._cachedContainerOffsets;
    var o = this.subjectsBounds.merge({
      layer: n || this.getLayerCollisionBounds()
    }), i = Dr.mergeSmallestSides(o.layerOffsetsToScrollContainers);
    return n || (this._cachedContainerOffsets = i), i;
  }, rr(e, [{
    key: "type",
    get: function() {
      return this.primary.prop + "-" + (this.secondary.prop === "center" ? "center" : ["bottom", "right"].includes(this.secondary.prop) ? "end" : "start");
    }
  }, {
    key: "triggerIsBigger",
    get: function() {
      var n = this.secondary.isHorizontal, o = this.subjectsBounds, i = o.triggerHasBiggerWidth, a = o.triggerHasBiggerHeight;
      return n && i || !n && a;
    }
    /**
     * Checks whether the placement fits within all it's container (including container-offset)
     */
  }, {
    key: "fitsContainer",
    get: function() {
      return this.getContainerOffsets().allSidesArePositive;
    }
    /**
     * Returns the surface in square pixels of the visible part of the layer
     */
  }, {
    key: "visibleSurface",
    get: function() {
      var n = this.getLayerBounds(), o = this.getContainerOffsets(n), i = o.negativeSides;
      for (var a in i)
        i[a] = -i[a];
      return n.substract(i).surface;
    }
    /**
     * Returns a BoundSide by looking at the most negative offset that is the opposite direction
     */
  }, {
    key: "secondaryOffsetSide",
    get: function() {
      var n, o, i = this, a = this.getContainerOffsets(), s = (n = (o = Object.entries(a.negativeSides).map(function(c) {
        var f = c[0], d = c[1];
        return [Xe[f], d];
      }).filter(function(c) {
        var f = c[0];
        return i.primary.isOppositeDirection(f);
      }).sort(function(c, f) {
        var d = c[1], g = f[1];
        return g - d;
      })) == null ? void 0 : o[0]) != null ? n : [], l = s[0];
      return l || null;
    }
  }]), e;
}(), qn = /* @__PURE__ */ function(e) {
  ws(t, e);
  function t() {
    return e.apply(this, arguments) || this;
  }
  var r = t.prototype;
  return r.getLayerBounds = function() {
    var o = this.subjectsBounds, i = o.trigger, a = o.layer, s = Ze.empty();
    return s.top = i.top + i.height / 2 - a.height / 2, s.bottom = s.top + a.height, s.left = i.left + i.width / 2 - a.width / 2, s.right = s.left + a.width, s.width = s.right - s.left, s.height = s.bottom - s.top, s;
  }, t;
}(Ao);
function Fs(e, t, r) {
  var n = e.layer, o = e.trigger, i = e.arrow, a = t.primary.oppositeSizeProp, s = t.primary.isHorizontal ? ["top", "bottom"] : ["left", "right"], l = s[0], c = s[1], f = n[l] + n[a] / 2 - o[l] - i[a] / 2 - r, d = n[c] - n[a] / 2 - o[c] + i[a] / 2 + r;
  return (f < 0 ? -f : 0) + (d > 0 ? -d : 0);
}
var xn = {
  position: "absolute",
  willChange: "top, left",
  left: null,
  right: null,
  top: null,
  bottom: null
};
function ks(e, t, r) {
  var n;
  if (t.primary.isCenter)
    return xn;
  var o = e.layer, i = e.trigger, a = e.arrow, s = t.primary.oppositeSizeProp, l = i[s] > o[s], c = r + a[s] / 2, f = o[s] - a[s] / 2 - r, d = Fs(e, t, r), g = t.primary.prop, h = t.primary.oppositeCssProp, v = l ? o[s] / 2 + d : i[h] + i[s] / 2 - o[h];
  return We({}, xn, (n = {}, n[g] = "100%", n[h] = qr(v, c, f), n));
}
var En = /* @__PURE__ */ function() {
  function e(r, n, o) {
    this.placements = void 0, this.config = void 0, this.subjectsBounds = void 0, this.placements = r, this.config = n, this.subjectsBounds = o;
  }
  e.getSidesFromPlacementType = function(n) {
    var o = n.split("-"), i = o[0], a = o[1], s = Xe[i], l;
    return a === "center" ? l = Se.center : s.isHorizontal ? l = a === "start" ? Se.top : Se.bottom : l = a === "start" ? Se.left : Se.right, [s, l];
  }, e.create = function(n, o) {
    var i = {
      arrow: o.arrowOffset,
      container: o.containerOffset,
      trigger: o.triggerOffset
    };
    function a(s) {
      s === void 0 && (s = o.placement);
      var l = e.getSidesFromPlacementType(s), c = l[0], f = l[1], d = Xe[c.isHorizontal ? o.preferY : o.preferX], g = !c.isHorizontal && n.triggerHasBiggerWidth || c.isHorizontal && n.triggerHasBiggerHeight;
      function h(y, p) {
        return new Ao(y, p, n, o.layerDimensions, i);
      }
      var v = [];
      return v[0] = h(c, f), v[1] = h(c, f.isCenter ? d : Se.center), v[2] = h(c, Se[(f.opposite.isCenter ? d.opposite : f.opposite).prop]), v[3] = h(d, g ? c : Se[c.opposite.prop]), v[4] = h(d, Se.center), v[5] = h(d, g ? Se[c.opposite.prop] : c), v[6] = h(Xe[d.opposite.prop], g ? c : Se[c.opposite.prop]), v[7] = h(Xe[d.opposite.prop], Se.center), v[8] = h(Xe[d.opposite.prop], g ? Se[c.opposite.prop] : c), v[9] = h(Xe[c.opposite.prop], f), v[10] = h(Xe[c.opposite.prop], f.isCenter ? d : Se.center), v[11] = h(Xe[c.opposite.prop], Se[(f.opposite.isCenter ? d.opposite : f.opposite).prop]), v = v.filter(function(y) {
        return y.type === o.placement || o.possiblePlacements.includes(y.type);
      }), v;
    }
    return o.placement === "center" ? new e([new qn(Se.center, Se.center, n, o.layerDimensions, i)].concat(a(o.preferY + "-" + o.preferX)), o, n) : new e(a(), o, n);
  };
  var t = e.prototype;
  return t.filterPlacementsBySide = function(n) {
    return this.placements.filter(function(o) {
      return o.primary === n;
    });
  }, t.findFirstPlacementThatFits = function() {
    return this.placements.find(function(n) {
      return n.fitsContainer;
    });
  }, t.placementWithBiggestVisibleSurface = function() {
    var n = this.placements.map(function(i) {
      return {
        placement: i,
        surface: i.visibleSurface
      };
    }).sort(function(i, a) {
      return a.surface - i.surface;
    }), o = n[0].placement;
    return o;
  }, t.findSuitablePlacement = function() {
    return this.config.auto ? this.findFirstPlacementThatFits() || this.placementWithBiggestVisibleSurface() : this.placements[0];
  }, t.getSecondaryOffset = function(n) {
    var o = this.config, i = o.auto, a = o.snap;
    if (!i || a || n instanceof qn)
      return 0;
    var s = this.filterPlacementsBySide(n.primary), l = s.indexOf(n) === 0;
    if (l && n.fitsContainer)
      return 0;
    var c = s.find(function(y) {
      return !y.fitsContainer;
    });
    if (!c)
      return 0;
    var f = c.secondaryOffsetSide;
    if (!f)
      return 0;
    var d = n.getContainerOffsets(), g = n.secondary, h;
    n.triggerIsBigger || c === n ? h = f.isPush ? -1 : 1 : h = g === Se.left || [Se.top, Se.center].includes(g) && f.isPush ? -1 : 1;
    var v = d[f.prop];
    return v * h;
  }, t.getStyles = function(n, o, i, a) {
    var s = {
      willChange: "top, left, width, height"
    }, l = ks(this.subjectsBounds.merge({
      layer: n
    }), o, this.config.arrowOffset), c = this.config.overflowContainer ? We({}, s, {
      position: "fixed",
      top: n.top,
      left: n.left
    }) : We({}, s, {
      position: "absolute",
      top: n.top - this.subjectsBounds.parent.top + i.top - a.top,
      left: n.left - this.subjectsBounds.parent.left + i.left - a.left
    });
    return {
      arrow: l,
      layer: c
    };
  }, t.getHasDisappeared = function(n) {
    var o = this.config.overflowContainer ? this.subjectsBounds.trigger : n, i = Dr.mergeSmallestSides(this.subjectsBounds.offsetsToScrollContainers(o, !0)), a = Object.entries(i.negativeSides), s = a.some(function(l) {
      var c = l[0], f = l[1], d = Xe[c];
      return f <= -o[d.sizeProp];
    });
    return s ? "full" : i.allSidesArePositive ? null : "partial";
  }, t.result = function(n, o) {
    var i = this.findSuitablePlacement(), a = this.getSecondaryOffset(i), s = i.getLayerBounds(a), l = this.getStyles(s, i, n, o), c = i.primary.prop;
    return {
      styles: l,
      layerSide: c,
      placement: i,
      layerBounds: s,
      hasDisappeared: this.getHasDisappeared(s)
    };
  }, e;
}(), Ms = /* @__PURE__ */ function() {
  function e(r, n) {
    this.overflowContainer = void 0, this.trigger = void 0, this.layer = void 0, this.arrow = void 0, this.parent = void 0, this.window = void 0, this.scrollContainers = void 0, this.overflowContainer = n, Object.assign(this, r);
  }
  e.create = function(n, o, i, a, s, l, c, f) {
    var d = Ze.fromWindow(n);
    return new e({
      layer: Ze.fromElement(o, {
        environment: n,
        withTransform: !1
      }),
      trigger: f ? Ze.create(xr(f())) : Ze.fromElement(i),
      arrow: s ? Ze.fromElement(s) : Ze.empty(),
      parent: a ? Ze.fromElement(a) : d,
      window: d,
      scrollContainers: [d].concat(l.map(function(g) {
        return Ze.fromElement(g, {
          withScrollbars: !1
        });
      }))
    }, c);
  };
  var t = e.prototype;
  return t.merge = function(n) {
    return new e(We({}, this, n), this.overflowContainer);
  }, t.offsetsToScrollContainers = function(n, o) {
    o === void 0 && (o = !1);
    var i = this.overflowContainer && !o ? [this.window] : this.scrollContainers;
    return i.map(function(a) {
      return a.offsetsTo(n);
    });
  }, rr(e, [{
    key: "layerOffsetsToScrollContainers",
    get: function() {
      return this.offsetsToScrollContainers(this.layer);
    }
  }, {
    key: "triggerHasBiggerWidth",
    get: function() {
      return this.trigger.width > this.layer.width;
    }
  }, {
    key: "triggerHasBiggerHeight",
    get: function() {
      return this.trigger.height > this.layer.height;
    }
  }]), e;
}(), Ln = null, je = {
  auto: !1,
  arrowOffset: 0,
  containerOffset: 10,
  triggerOffset: 0,
  overflowContainer: !0,
  placement: "top-center",
  possiblePlacements: zs,
  preferX: "right",
  preferY: "bottom",
  snap: !1,
  container: void 0,
  trigger: void 0
};
function Rs(e) {
  var t, r = e.isOpen, n = r === void 0 ? !1 : r, o = e.overflowContainer, i = o === void 0 ? je.overflowContainer : o, a = e.environment, s = a === void 0 ? typeof window < "u" ? window : void 0 : a, l = e.ResizeObserver, c = e.placement, f = c === void 0 ? je.placement : c, d = e.possiblePlacements, g = d === void 0 ? je.possiblePlacements : d, h = e.preferX, v = h === void 0 ? je.preferX : h, y = e.preferY, p = y === void 0 ? je.preferY : y, A = e.auto, w = A === void 0 ? je.auto : A, X = e.snap, T = X === void 0 ? je.snap : X, m = e.triggerOffset, M = m === void 0 ? je.triggerOffset : m, Z = e.containerOffset, z = Z === void 0 ? je.containerOffset : Z, W = e.arrowOffset, C = W === void 0 ? je.arrowOffset : W, Y = e.container, j = Y === void 0 ? je.container : Y, F = e.layerDimensions, H = F === void 0 ? null : F, G = e.onDisappear, ne = e.onOutsideClick, re = e.onParentClose, I = e.trigger, ve = _e(function() {
    return {
      layerSide: f === "center" ? "center" : En.getSidesFromPlacementType(f)[0].prop,
      styles: {
        layer: {
          position: i ? "fixed" : "absolute",
          top: 0,
          left: 0
        },
        arrow: {
          position: "absolute",
          top: 0,
          left: 0
        }
      }
    };
  }), he = ve[0], Ne = ve[1], L = ye(null), N = Xs(he, n), q = ye({
    cancelled: !1
  });
  be(function() {
    return function() {
      q.current.cancelled = !0;
    };
  }, []);
  var k = xe(function(ae, _, Ae) {
    var Ce = ae.arrow, we = ae.layer, Ge = ae.scrollContainers, Be = ae.trigger, Ee = Ge[0], R = Ms.create(s, we, Be, Ee, Ce, Ge, i, I == null ? void 0 : I.getBounds), me = {
      placement: f,
      possiblePlacements: g,
      auto: w,
      layerDimensions: H,
      arrowOffset: C,
      containerOffset: z,
      triggerOffset: M,
      preferX: v,
      preferY: p,
      snap: T,
      overflowContainer: i
    }, Ue = En.create(R, me).result(_, Ae), Fe = Ue.hasDisappeared, ke = Ue.layerSide, zt = Ue.styles, it = {
      layerSide: ke,
      styles: zt
    };
    if (!N.current || Ds(N.current, it)) {
      N.current = it, q.current.cancelled = !0;
      var st = {
        cancelled: !1
      };
      q.current = st, Promise.resolve().then(function() {
        st.cancelled || Ne(it);
      });
    }
    Sn(Fe) && Sn(G) && G(Fe);
  }, [C, w, z, s, H, G, i, f, g, v, p, T, M, N, I]), b = qs({
    ResizeObserverPolyfill: l,
    environment: s,
    enabled: n,
    overflowContainer: i,
    onChange: k,
    triggerOption: I
  }), K = b.triggerRef, S = b.layerRef, ie = b.arrowRef, J = b.closestScrollContainer, Q = Ls({
    isOpen: n,
    onOutsideClick: ne,
    onParentClose: re
  }), P = Q.closeOnOutsideClickRefs, de = Q.registrations, x = {
    triggerProps: Boolean(I) ? {} : {
      ref: Xn(K, P.trigger, L)
    },
    layerProps: {
      ref: Xn(S, P.layer),
      style: he.styles.layer
    },
    arrowProps: {
      ref: ie,
      style: he.styles.arrow,
      layerSide: he.layerSide
    },
    layerSide: he.layerSide,
    triggerBounds: n ? I ? I.getBounds() : (t = L.current) == null ? void 0 : t.getBoundingClientRect() : null,
    renderLayer: function(ae) {
      return typeof document < "u" ? Fo(ft(xs, {
        registrations: de,
        children: ae
      }), i || !J ? Bs(j) : J) : null;
    }
  };
  return x;
}
function Ds(e, t) {
  if (e.layerSide !== t.layerSide)
    return !0;
  for (var r = ["position", "top", "left", "right", "bottom"], n = 0, o = r; n < o.length; n++) {
    var i = o[n];
    if (e.styles.layer[i] !== t.styles.layer[i] || e.styles.arrow[i] !== t.styles.arrow[i])
      return !0;
  }
  return !1;
}
var zn = "layers";
function Bs(e) {
  var t;
  if (typeof e == "function") {
    if (t = e(), !t || !(t instanceof HTMLElement))
      throw new Error("react-laag: You've passed a function to the 'container' prop, but it returned no valid HTMLElement");
  } else if (e instanceof HTMLElement)
    t = e;
  else if (typeof e == "string") {
    if (t = document.getElementById(e), !t)
      throw new Error("react-laag: You've passed element with id '" + e + "' to the 'container' prop, but it returned no valid HTMLElement");
  } else {
    if (Ln instanceof HTMLElement)
      return Ln;
    t = document.getElementById(zn), t || (t = document.createElement("div"), t.id = zn, t.style.cssText = `
        position: absolute;
        top: 0px;
        left: 0px;
        right: 0px;
      `, document.body.appendChild(t));
  }
  return t;
}
var js = ["size", "angle", "borderWidth", "borderColor", "roundness", "backgroundColor", "layerSide", "style"], Yt = "left", Zt = "top", Ht = "bottom", Ut = "right";
function Qt(e, t) {
  return Math.tan(e * (Math.PI / 180)) * t;
}
function Ys(e, t, r, n) {
  var o, i = (o = {}, o[Ht] = "0 " + -n + " " + t + " " + e, o[Zt] = "0 0 " + t + " " + (e + n), o[Ut] = -n + " 0 " + e + " " + t, o[Yt] = "0 0 " + (e + n) + " " + t, o);
  return i[r.prop];
}
function Zs(e, t, r, n, o) {
  var i, a, s, l = n / 10 * e * 2, c = (i = {}, i[Ht] = [0, e], i[Zt] = [0, 0], i[Ut] = [e, t], i[Yt] = [0, t], i)[r.prop].join(" "), f = r.isHorizontal ? "V 0" : "H " + t, d = t / 2, g = t / 2 + Qt(o, e / 8), h = e / 8, v = (a = {}, a[Ht] = ["C", g, h, d + l, 0, d, 0], a[Zt] = ["C", g, e - h, d + l, e, d, e], a[Ut] = ["C", h, t - g, 0, d - l, 0, d], a[Yt] = ["C", e - h, t - g, e, d - l, e, d], a)[r.prop].join(" "), y = t / 2 - Qt(o, e / 8), p = e / 8, A = (s = {}, s[Ht] = ["C", d - l, 0, y, p, c], s[Zt] = ["C", d - l, e, y, e - p, c], s[Ut] = ["C", 0, d + l, p, t - y, c], s[Yt] = ["C", e, d + l, e - p, t - y, c], s)[r.prop].join(" ");
  return ["M", c, f, v, A].join(" ");
}
function Hs(e, t, r, n, o) {
  var i = Qt(o, r), a = n.isPush ? [0, r] : [e, e - r], s = a[0], l = a[1];
  return n.isHorizontal ? ["M", s, r, "V", t - r, "L", l, t - r - i, "V", i + r, "Z"].join(" ") : ["M", r, s, "H", t - r, "L", t - r - i, l, "H", i + r, "Z"].join(" ");
}
var Us = /* @__PURE__ */ Cn(function(t, r) {
  var n = t.size, o = n === void 0 ? 8 : n, i = t.angle, a = i === void 0 ? 45 : i, s = t.borderWidth, l = s === void 0 ? 0 : s, c = t.borderColor, f = c === void 0 ? "black" : c, d = t.roundness, g = d === void 0 ? 0 : d, h = t.backgroundColor, v = h === void 0 ? "white" : h, y = t.layerSide, p = y === void 0 ? "top" : y, A = t.style, w = A === void 0 ? {} : A, X = Ps(t, js);
  if (p === "center")
    return null;
  var T = Xe[p], m = o, M = Qt(a, o) * 2, Z = Math.max(m, M);
  return ft("svg", We({
    ref: r
  }, X, {
    style: We({}, w, {
      transform: "translate" + (T.isHorizontal ? "Y" : "X") + "(-50%)"
    }),
    width: Z,
    height: Z,
    preserveAspectRatio: T.isPush ? "xMinYMin" : "xMaxYMax",
    viewBox: Ys(m, M, T, l)
  }), ft("path", {
    fill: v,
    strokeWidth: l,
    stroke: f,
    d: Zs(m, M, T, g, a)
  }), ft("path", {
    fill: v,
    d: Hs(m, M, l, T, a)
  }));
}), Ye;
(function(e) {
  e[e.ENTERING = 0] = "ENTERING", e[e.LEAVING = 1] = "LEAVING", e[e.IDLE = 2] = "IDLE";
})(Ye || (Ye = {}));
function Gs(e) {
  var t = e === void 0 ? {} : e, r = t.delayEnter, n = r === void 0 ? 0 : r, o = t.delayLeave, i = o === void 0 ? 0 : o, a = t.hideOnScroll, s = a === void 0 ? !0 : a, l = _e(!1), c = l[0], f = l[1], d = ye(null), g = ye(Ye.IDLE), h = ye(!1), v = xe(function() {
    clearTimeout(d.current), d.current = null, g.current = Ye.IDLE;
  }, []);
  function y() {
    g.current === Ye.LEAVING && d.current && v(), !c && (g.current = Ye.ENTERING, d.current = window.setTimeout(function() {
      f(!0), d.current = null, g.current = Ye.IDLE;
    }, n));
  }
  function p(w, X) {
    if (g.current === Ye.ENTERING && d.current && v(), !!c) {
      if (X) {
        f(!1), d.current = null, g.current = Ye.IDLE;
        return;
      }
      g.current = Ye.LEAVING, d.current = window.setTimeout(function() {
        f(!1), d.current = null, g.current = Ye.IDLE;
      }, i);
    }
  }
  be(function() {
    function w() {
      c && s && (v(), f(!1));
    }
    return window.addEventListener("scroll", w, !0), function() {
      window.removeEventListener("scroll", w, !0), d.current && clearTimeout(d.current);
    };
  }, [c, s, v]);
  var A = {
    onMouseEnter: y,
    onMouseLeave: p,
    onTouchStart: function() {
      h.current = !1;
    },
    onTouchMove: function() {
      h.current = !0;
    },
    onTouchEnd: function() {
      !h.current && !c && f(!0), h.current = !1;
    }
  };
  return [c, A, function() {
    return p(null, !0);
  }];
}
const Is = Te.div`
	${({ theme: { colors: e, spaces: t } }) => te`
			min-width: 200px;
			max-width: 250px;
			background: ${e.secondary};
			color: ${e.main};
			padding: ${t.small} ${t.medium};
			text-align: center;
			z-index: 9999;
		`}
`, ol = ({
  children: e,
  content: t,
  showOnClick: r,
  isDisabled: n,
  "data-testid": o = "tooltip"
}) => {
  const i = Ma(), [a, s] = Gs(), [l, c] = _e(!1), f = r ? l : a, d = () => {
    c(!1);
  }, { triggerProps: g, layerProps: h, arrowProps: v, renderLayer: y } = Rs({
    isOpen: f,
    placement: "top-center",
    triggerOffset: 8,
    onOutsideClick: d
  });
  return /* @__PURE__ */ ze(Fn, { children: [
    !n && /* @__PURE__ */ U(
      "span",
      {
        "data-testid": o,
        ...g,
        ...r ? { onClick: () => c((A) => !A) } : s,
        children: e
      }
    ),
    y(
      f && /* @__PURE__ */ ze(Is, { ...h, children: [
        t,
        /* @__PURE__ */ U(
          Us,
          {
            ...v,
            backgroundColor: i.colors.secondary,
            size: 6
          }
        )
      ] })
    )
  ] });
}, Js = Te.div`
	${({ theme: { breakpoints: e, spaces: t, colors: r } }) => te`
			width: 100vw;
			height: 100vh;
			position: absolute;
			left: 0;
			top: 0;
			display: grid;
			place-items: center;
			background-color: ${r.secondary_opacity};

			@media (max-width: ${e.mobile}px) {
				padding: 0 ${t.medium};
				overflow: none;
			}
		`}
`, Ks = Te.div`
	${({ theme: { breakpoints: e, colors: t, spaces: r } }) => te`
			padding: ${r.medium};
			width: 700px;
			border-radius: 10px;
			background-color: ${t.main};

			@media (max-width: ${e.mobile}px) {
				width: 100%;
			}
		`}
`, Qs = Te.div`
	${({ theme: { colors: e } }) => te`
			width: 100%;
			display: flex;
			justify-content: space-between;
			align-content: center;

			> .close-icon {
				cursor: pointer;
				width: 30px;
				height: 30px;

				&:hover {
					color: ${e.color5};
				}
			}
		`}
`, _s = Te.div`
	${({ theme: { breakpoints: e, spaces: t } }) => te`
			width: 100%;
			display: flex;
			justify-content: flex-end;
			gap: ${t.medium};

			@media (max-width: ${e.mobile}px) {
				flex-direction: column-reverse;
				justify-content: center;
				flex-wrap: wrap;
			}
		`}
`, $s = Rr.create(
  ({
    id: e,
    title: t,
    subtitle: r = "",
    content: n,
    variant: o = "primary",
    action: i,
    actionName: a,
    cancelName: s,
    "data-testid": l = "modal"
  }) => {
    const c = Mr(), f = () => c.remove();
    return /* @__PURE__ */ U(
      Js,
      {
        id: e,
        "data-testid": l,
        children: /* @__PURE__ */ ze(Ks, { children: [
          /* @__PURE__ */ ze(Qs, { children: [
            /* @__PURE__ */ ze("div", { children: [
              /* @__PURE__ */ U(
                $n,
                {
                  level: 3,
                  align: "left",
                  children: t
                }
              ),
              r && /* @__PURE__ */ ze(Fn, { children: [
                /* @__PURE__ */ U(
                  Bt,
                  {
                    type: "vertical",
                    space: "tiny"
                  }
                ),
                /* @__PURE__ */ U(xt, { children: r })
              ] })
            ] }),
            /* @__PURE__ */ U(
              Oi,
              {
                onClick: f,
                className: "close-icon"
              }
            )
          ] }),
          /* @__PURE__ */ U(Bt, { type: "vertical" }),
          /* @__PURE__ */ U("div", { children: n }),
          /* @__PURE__ */ U(
            Bt,
            {
              type: "vertical",
              space: "large"
            }
          ),
          /* @__PURE__ */ ze(_s, { children: [
            /* @__PURE__ */ U(
              Pr,
              {
                variant: "secondary",
                action: f,
                children: s
              }
            ),
            /* @__PURE__ */ U(
              Pr,
              {
                variant: o,
                action: () => {
                  i(), f();
                },
                children: a
              }
            )
          ] })
        ] })
      }
    );
  }
), al = ({
  id: e,
  title: t,
  subtitle: r = "",
  content: n,
  actionName: o,
  cancelName: i,
  variant: a = "primary",
  action: s
}) => Rr.show($s, {
  id: e,
  title: t,
  subtitle: r,
  content: n,
  actionName: o,
  cancelName: i,
  variant: a,
  action: s
}), il = (e, t) => ge[t](e, {
  position: window.innerWidth <= Sr.MOBILE ? "top-center" : "bottom-left",
  autoClose: 3e3,
  hideProgressBar: !1,
  closeOnClick: !0,
  pauseOnHover: !0,
  closeButton: !1,
  draggable: !0,
  progress: void 0
});
export {
  rl as Break,
  Pr as Button,
  $n as Heading,
  Ja as Image,
  Wi as Input,
  ki as Layout,
  _n as Loader,
  xt as Paragraph,
  Bt as Spacer,
  nl as Theme,
  ol as Tooltip,
  al as modalShow,
  il as toastNotify
};
