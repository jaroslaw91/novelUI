import E, { useRef as Me, useContext as Re, useLayoutEffect as Fr, useMemo as zn, useDebugValue as cr, createElement as Wn, useEffect as Oe, useCallback as Qe, useReducer as Mr, forwardRef as Vn, useState as Mt, isValidElement as $e, cloneElement as Dt } from "react";
var st = {}, En = {
  get exports() {
    return st;
  },
  set exports(e) {
    st = e;
  }
}, ot = {};
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var fr;
function Ln() {
  if (fr)
    return ot;
  fr = 1;
  var e = E, t = Symbol.for("react.element"), r = Symbol.for("react.fragment"), n = Object.prototype.hasOwnProperty, i = e.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, s = { key: !0, ref: !0, __self: !0, __source: !0 };
  function a(l, d, p) {
    var v, b = {}, O = null, w = null;
    p !== void 0 && (O = "" + p), d.key !== void 0 && (O = "" + d.key), d.ref !== void 0 && (w = d.ref);
    for (v in d)
      n.call(d, v) && !s.hasOwnProperty(v) && (b[v] = d[v]);
    if (l && l.defaultProps)
      for (v in d = l.defaultProps, d)
        b[v] === void 0 && (b[v] = d[v]);
    return { $$typeof: t, type: l, key: O, ref: w, props: b, _owner: i.current };
  }
  return ot.Fragment = r, ot.jsx = a, ot.jsxs = a, ot;
}
var at = {};
/**
 * @license React
 * react-jsx-runtime.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var dr;
function xn() {
  return dr || (dr = 1, process.env.NODE_ENV !== "production" && function() {
    var e = E, t = Symbol.for("react.element"), r = Symbol.for("react.portal"), n = Symbol.for("react.fragment"), i = Symbol.for("react.strict_mode"), s = Symbol.for("react.profiler"), a = Symbol.for("react.provider"), l = Symbol.for("react.context"), d = Symbol.for("react.forward_ref"), p = Symbol.for("react.suspense"), v = Symbol.for("react.suspense_list"), b = Symbol.for("react.memo"), O = Symbol.for("react.lazy"), w = Symbol.for("react.offscreen"), y = Symbol.iterator, A = "@@iterator";
    function u(o) {
      if (o === null || typeof o != "object")
        return null;
      var h = y && o[y] || o[A];
      return typeof h == "function" ? h : null;
    }
    var P = e.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    function X(o) {
      {
        for (var h = arguments.length, N = new Array(h > 1 ? h - 1 : 0), F = 1; F < h; F++)
          N[F - 1] = arguments[F];
        V("error", o, N);
      }
    }
    function V(o, h, N) {
      {
        var F = P.ReactDebugCurrentFrame, K = F.getStackAddendum();
        K !== "" && (h += "%s", N = N.concat([K]));
        var ae = N.map(function(U) {
          return String(U);
        });
        ae.unshift("Warning: " + h), Function.prototype.apply.call(console[o], console, ae);
      }
    }
    var q = !1, f = !1, re = !1, de = !1, B = !1, ue;
    ue = Symbol.for("react.module.reference");
    function pe(o) {
      return !!(typeof o == "string" || typeof o == "function" || o === n || o === s || B || o === i || o === p || o === v || de || o === w || q || f || re || typeof o == "object" && o !== null && (o.$$typeof === O || o.$$typeof === b || o.$$typeof === a || o.$$typeof === l || o.$$typeof === d || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      o.$$typeof === ue || o.getModuleId !== void 0));
    }
    function me(o, h, N) {
      var F = o.displayName;
      if (F)
        return F;
      var K = h.displayName || h.name || "";
      return K !== "" ? N + "(" + K + ")" : N;
    }
    function J(o) {
      return o.displayName || "Context";
    }
    function C(o) {
      if (o == null)
        return null;
      if (typeof o.tag == "number" && X("Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."), typeof o == "function")
        return o.displayName || o.name || null;
      if (typeof o == "string")
        return o;
      switch (o) {
        case n:
          return "Fragment";
        case r:
          return "Portal";
        case s:
          return "Profiler";
        case i:
          return "StrictMode";
        case p:
          return "Suspense";
        case v:
          return "SuspenseList";
      }
      if (typeof o == "object")
        switch (o.$$typeof) {
          case l:
            var h = o;
            return J(h) + ".Consumer";
          case a:
            var N = o;
            return J(N._context) + ".Provider";
          case d:
            return me(o, o.render, "ForwardRef");
          case b:
            var F = o.displayName || null;
            return F !== null ? F : C(o.type) || "Memo";
          case O: {
            var K = o, ae = K._payload, U = K._init;
            try {
              return C(U(ae));
            } catch {
              return null;
            }
          }
        }
      return null;
    }
    var H = Object.assign, G = 0, ie, oe, ne, be, Xe, Te, W;
    function S() {
    }
    S.__reactDisabledLog = !0;
    function z() {
      {
        if (G === 0) {
          ie = console.log, oe = console.info, ne = console.warn, be = console.error, Xe = console.group, Te = console.groupCollapsed, W = console.groupEnd;
          var o = {
            configurable: !0,
            enumerable: !0,
            value: S,
            writable: !0
          };
          Object.defineProperties(console, {
            info: o,
            log: o,
            warn: o,
            error: o,
            group: o,
            groupCollapsed: o,
            groupEnd: o
          });
        }
        G++;
      }
    }
    function L() {
      {
        if (G--, G === 0) {
          var o = {
            configurable: !0,
            enumerable: !0,
            writable: !0
          };
          Object.defineProperties(console, {
            log: H({}, o, {
              value: ie
            }),
            info: H({}, o, {
              value: oe
            }),
            warn: H({}, o, {
              value: ne
            }),
            error: H({}, o, {
              value: be
            }),
            group: H({}, o, {
              value: Xe
            }),
            groupCollapsed: H({}, o, {
              value: Te
            }),
            groupEnd: H({}, o, {
              value: W
            })
          });
        }
        G < 0 && X("disabledDepth fell below zero. This is a bug in React. Please file an issue.");
      }
    }
    var c = P.ReactCurrentDispatcher, Z;
    function g(o, h, N) {
      {
        if (Z === void 0)
          try {
            throw Error();
          } catch (K) {
            var F = K.stack.trim().match(/\n( *(at )?)/);
            Z = F && F[1] || "";
          }
        return `
` + Z + o;
      }
    }
    var Q = !1, D;
    {
      var R = typeof WeakMap == "function" ? WeakMap : Map;
      D = new R();
    }
    function m(o, h) {
      if (!o || Q)
        return "";
      {
        var N = D.get(o);
        if (N !== void 0)
          return N;
      }
      var F;
      Q = !0;
      var K = Error.prepareStackTrace;
      Error.prepareStackTrace = void 0;
      var ae;
      ae = c.current, c.current = null, z();
      try {
        if (h) {
          var U = function() {
            throw Error();
          };
          if (Object.defineProperty(U.prototype, "props", {
            set: function() {
              throw Error();
            }
          }), typeof Reflect == "object" && Reflect.construct) {
            try {
              Reflect.construct(U, []);
            } catch (xe) {
              F = xe;
            }
            Reflect.construct(o, [], U);
          } else {
            try {
              U.call();
            } catch (xe) {
              F = xe;
            }
            o.call(U.prototype);
          }
        } else {
          try {
            throw Error();
          } catch (xe) {
            F = xe;
          }
          o();
        }
      } catch (xe) {
        if (xe && F && typeof xe.stack == "string") {
          for (var Y = xe.stack.split(`
`), we = F.stack.split(`
`), ve = Y.length - 1, he = we.length - 1; ve >= 1 && he >= 0 && Y[ve] !== we[he]; )
            he--;
          for (; ve >= 1 && he >= 0; ve--, he--)
            if (Y[ve] !== we[he]) {
              if (ve !== 1 || he !== 1)
                do
                  if (ve--, he--, he < 0 || Y[ve] !== we[he]) {
                    var Se = `
` + Y[ve].replace(" at new ", " at ");
                    return o.displayName && Se.includes("<anonymous>") && (Se = Se.replace("<anonymous>", o.displayName)), typeof o == "function" && D.set(o, Se), Se;
                  }
                while (ve >= 1 && he >= 0);
              break;
            }
        }
      } finally {
        Q = !1, c.current = ae, L(), Error.prepareStackTrace = K;
      }
      var Ie = o ? o.displayName || o.name : "", ur = Ie ? g(Ie) : "";
      return typeof o == "function" && D.set(o, ur), ur;
    }
    function ce(o, h, N) {
      return m(o, !1);
    }
    function T(o) {
      var h = o.prototype;
      return !!(h && h.isReactComponent);
    }
    function M(o, h, N) {
      if (o == null)
        return "";
      if (typeof o == "function")
        return m(o, T(o));
      if (typeof o == "string")
        return g(o);
      switch (o) {
        case p:
          return g("Suspense");
        case v:
          return g("SuspenseList");
      }
      if (typeof o == "object")
        switch (o.$$typeof) {
          case d:
            return ce(o.render);
          case b:
            return M(o.type, h, N);
          case O: {
            var F = o, K = F._payload, ae = F._init;
            try {
              return M(ae(K), h, N);
            } catch {
            }
          }
        }
      return "";
    }
    var se = Object.prototype.hasOwnProperty, k = {}, Ae = P.ReactDebugCurrentFrame;
    function We(o) {
      if (o) {
        var h = o._owner, N = M(o.type, o._source, h ? h.type : null);
        Ae.setExtraStackFrame(N);
      } else
        Ae.setExtraStackFrame(null);
    }
    function ye(o, h, N, F, K) {
      {
        var ae = Function.call.bind(se);
        for (var U in o)
          if (ae(o, U)) {
            var Y = void 0;
            try {
              if (typeof o[U] != "function") {
                var we = Error((F || "React class") + ": " + N + " type `" + U + "` is invalid; it must be a function, usually from the `prop-types` package, but received `" + typeof o[U] + "`.This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.");
                throw we.name = "Invariant Violation", we;
              }
              Y = o[U](h, U, F, N, null, "SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED");
            } catch (ve) {
              Y = ve;
            }
            Y && !(Y instanceof Error) && (We(K), X("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", F || "React class", N, U, typeof Y), We(null)), Y instanceof Error && !(Y.message in k) && (k[Y.message] = !0, We(K), X("Failed %s type: %s", N, Y.message), We(null));
          }
      }
    }
    var Ge = Array.isArray;
    function Ee(o) {
      return Ge(o);
    }
    function qe(o) {
      {
        var h = typeof Symbol == "function" && Symbol.toStringTag, N = h && o[Symbol.toStringTag] || o.constructor.name || "Object";
        return N;
      }
    }
    function x(o) {
      try {
        return fe(o), !1;
      } catch {
        return !0;
      }
    }
    function fe(o) {
      return "" + o;
    }
    function Je(o) {
      if (x(o))
        return X("The provided key is an unsupported type %s. This value must be coerced to a string before before using it here.", qe(o)), fe(o);
    }
    var Le = P.ReactCurrentOwner, Ve = {
      key: !0,
      ref: !0,
      __self: !0,
      __source: !0
    }, tr, rr, Tt;
    Tt = {};
    function vn(o) {
      if (se.call(o, "ref")) {
        var h = Object.getOwnPropertyDescriptor(o, "ref").get;
        if (h && h.isReactWarning)
          return !1;
      }
      return o.ref !== void 0;
    }
    function mn(o) {
      if (se.call(o, "key")) {
        var h = Object.getOwnPropertyDescriptor(o, "key").get;
        if (h && h.isReactWarning)
          return !1;
      }
      return o.key !== void 0;
    }
    function An(o, h) {
      if (typeof o.ref == "string" && Le.current && h && Le.current.stateNode !== h) {
        var N = C(Le.current.type);
        Tt[N] || (X('Component "%s" contains the string ref "%s". Support for string refs will be removed in a future major release. This case cannot be automatically converted to an arrow function. We ask you to manually fix this case by using useRef() or createRef() instead. Learn more about using refs safely here: https://reactjs.org/link/strict-mode-string-ref', C(Le.current.type), o.ref), Tt[N] = !0);
      }
    }
    function hn(o, h) {
      {
        var N = function() {
          tr || (tr = !0, X("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", h));
        };
        N.isReactWarning = !0, Object.defineProperty(o, "key", {
          get: N,
          configurable: !0
        });
      }
    }
    function yn(o, h) {
      {
        var N = function() {
          rr || (rr = !0, X("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", h));
        };
        N.isReactWarning = !0, Object.defineProperty(o, "ref", {
          get: N,
          configurable: !0
        });
      }
    }
    var bn = function(o, h, N, F, K, ae, U) {
      var Y = {
        // This tag allows us to uniquely identify this as a React Element
        $$typeof: t,
        // Built-in properties that belong on the element
        type: o,
        key: h,
        ref: N,
        props: U,
        // Record the component responsible for creating this element.
        _owner: ae
      };
      return Y._store = {}, Object.defineProperty(Y._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: !1
      }), Object.defineProperty(Y, "_self", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: F
      }), Object.defineProperty(Y, "_source", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: K
      }), Object.freeze && (Object.freeze(Y.props), Object.freeze(Y)), Y;
    };
    function gn(o, h, N, F, K) {
      {
        var ae, U = {}, Y = null, we = null;
        N !== void 0 && (Je(N), Y = "" + N), mn(h) && (Je(h.key), Y = "" + h.key), vn(h) && (we = h.ref, An(h, K));
        for (ae in h)
          se.call(h, ae) && !Ve.hasOwnProperty(ae) && (U[ae] = h[ae]);
        if (o && o.defaultProps) {
          var ve = o.defaultProps;
          for (ae in ve)
            U[ae] === void 0 && (U[ae] = ve[ae]);
        }
        if (Y || we) {
          var he = typeof o == "function" ? o.displayName || o.name || "Unknown" : o;
          Y && hn(U, he), we && yn(U, he);
        }
        return bn(o, Y, we, K, F, Le.current, U);
      }
    }
    var St = P.ReactCurrentOwner, nr = P.ReactDebugCurrentFrame;
    function Ke(o) {
      if (o) {
        var h = o._owner, N = M(o.type, o._source, h ? h.type : null);
        nr.setExtraStackFrame(N);
      } else
        nr.setExtraStackFrame(null);
    }
    var Ot;
    Ot = !1;
    function zt(o) {
      return typeof o == "object" && o !== null && o.$$typeof === t;
    }
    function or() {
      {
        if (St.current) {
          var o = C(St.current.type);
          if (o)
            return `

Check the render method of \`` + o + "`.";
        }
        return "";
      }
    }
    function wn(o) {
      {
        if (o !== void 0) {
          var h = o.fileName.replace(/^.*[\\\/]/, ""), N = o.lineNumber;
          return `

Check your code at ` + h + ":" + N + ".";
        }
        return "";
      }
    }
    var ar = {};
    function Pn(o) {
      {
        var h = or();
        if (!h) {
          var N = typeof o == "string" ? o : o.displayName || o.name;
          N && (h = `

Check the top-level render call using <` + N + ">.");
        }
        return h;
      }
    }
    function ir(o, h) {
      {
        if (!o._store || o._store.validated || o.key != null)
          return;
        o._store.validated = !0;
        var N = Pn(h);
        if (ar[N])
          return;
        ar[N] = !0;
        var F = "";
        o && o._owner && o._owner !== St.current && (F = " It was passed a child from " + C(o._owner.type) + "."), Ke(o), X('Each child in a list should have a unique "key" prop.%s%s See https://reactjs.org/link/warning-keys for more information.', N, F), Ke(null);
      }
    }
    function sr(o, h) {
      {
        if (typeof o != "object")
          return;
        if (Ee(o))
          for (var N = 0; N < o.length; N++) {
            var F = o[N];
            zt(F) && ir(F, h);
          }
        else if (zt(o))
          o._store && (o._store.validated = !0);
        else if (o) {
          var K = u(o);
          if (typeof K == "function" && K !== o.entries)
            for (var ae = K.call(o), U; !(U = ae.next()).done; )
              zt(U.value) && ir(U.value, h);
        }
      }
    }
    function Xn(o) {
      {
        var h = o.type;
        if (h == null || typeof h == "string")
          return;
        var N;
        if (typeof h == "function")
          N = h.propTypes;
        else if (typeof h == "object" && (h.$$typeof === d || // Note: Memo only checks outer props here.
        // Inner props are checked in the reconciler.
        h.$$typeof === b))
          N = h.propTypes;
        else
          return;
        if (N) {
          var F = C(h);
          ye(N, o.props, "prop", F, o);
        } else if (h.PropTypes !== void 0 && !Ot) {
          Ot = !0;
          var K = C(h);
          X("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", K || "Unknown");
        }
        typeof h.getDefaultProps == "function" && !h.getDefaultProps.isReactClassApproved && X("getDefaultProps is only used on classic React.createClass definitions. Use a static property named `defaultProps` instead.");
      }
    }
    function qn(o) {
      {
        for (var h = Object.keys(o.props), N = 0; N < h.length; N++) {
          var F = h[N];
          if (F !== "children" && F !== "key") {
            Ke(o), X("Invalid prop `%s` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.", F), Ke(null);
            break;
          }
        }
        o.ref !== null && (Ke(o), X("Invalid attribute `ref` supplied to `React.Fragment`."), Ke(null));
      }
    }
    function lr(o, h, N, F, K, ae) {
      {
        var U = pe(o);
        if (!U) {
          var Y = "";
          (o === void 0 || typeof o == "object" && o !== null && Object.keys(o).length === 0) && (Y += " You likely forgot to export your component from the file it's defined in, or you might have mixed up default and named imports.");
          var we = wn(K);
          we ? Y += we : Y += or();
          var ve;
          o === null ? ve = "null" : Ee(o) ? ve = "array" : o !== void 0 && o.$$typeof === t ? (ve = "<" + (C(o.type) || "Unknown") + " />", Y = " Did you accidentally export a JSX literal instead of a component?") : ve = typeof o, X("React.jsx: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: %s.%s", ve, Y);
        }
        var he = gn(o, h, N, K, ae);
        if (he == null)
          return he;
        if (U) {
          var Se = h.children;
          if (Se !== void 0)
            if (F)
              if (Ee(Se)) {
                for (var Ie = 0; Ie < Se.length; Ie++)
                  sr(Se[Ie], o);
                Object.freeze && Object.freeze(Se);
              } else
                X("React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead.");
            else
              sr(Se, o);
        }
        return o === n ? qn(he) : Xn(he), he;
      }
    }
    function Nn(o, h, N) {
      return lr(o, h, N, !0);
    }
    function Tn(o, h, N) {
      return lr(o, h, N, !1);
    }
    var Sn = Tn, On = Nn;
    at.Fragment = n, at.jsx = Sn, at.jsxs = On;
  }()), at;
}
(function(e) {
  process.env.NODE_ENV === "production" ? e.exports = Ln() : e.exports = xn();
})(En);
const Fn = st.Fragment, le = st.jsx, ke = st.jsxs;
var lt = {}, Mn = {
  get exports() {
    return lt;
  },
  set exports(e) {
    lt = e;
  }
}, _ = {};
/**
 * @license React
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var pr;
function Dn() {
  if (pr)
    return _;
  pr = 1;
  var e = Symbol.for("react.element"), t = Symbol.for("react.portal"), r = Symbol.for("react.fragment"), n = Symbol.for("react.strict_mode"), i = Symbol.for("react.profiler"), s = Symbol.for("react.provider"), a = Symbol.for("react.context"), l = Symbol.for("react.server_context"), d = Symbol.for("react.forward_ref"), p = Symbol.for("react.suspense"), v = Symbol.for("react.suspense_list"), b = Symbol.for("react.memo"), O = Symbol.for("react.lazy"), w = Symbol.for("react.offscreen"), y;
  y = Symbol.for("react.module.reference");
  function A(u) {
    if (typeof u == "object" && u !== null) {
      var P = u.$$typeof;
      switch (P) {
        case e:
          switch (u = u.type, u) {
            case r:
            case i:
            case n:
            case p:
            case v:
              return u;
            default:
              switch (u = u && u.$$typeof, u) {
                case l:
                case a:
                case d:
                case O:
                case b:
                case s:
                  return u;
                default:
                  return P;
              }
          }
        case t:
          return P;
      }
    }
  }
  return _.ContextConsumer = a, _.ContextProvider = s, _.Element = e, _.ForwardRef = d, _.Fragment = r, _.Lazy = O, _.Memo = b, _.Portal = t, _.Profiler = i, _.StrictMode = n, _.Suspense = p, _.SuspenseList = v, _.isAsyncMode = function() {
    return !1;
  }, _.isConcurrentMode = function() {
    return !1;
  }, _.isContextConsumer = function(u) {
    return A(u) === a;
  }, _.isContextProvider = function(u) {
    return A(u) === s;
  }, _.isElement = function(u) {
    return typeof u == "object" && u !== null && u.$$typeof === e;
  }, _.isForwardRef = function(u) {
    return A(u) === d;
  }, _.isFragment = function(u) {
    return A(u) === r;
  }, _.isLazy = function(u) {
    return A(u) === O;
  }, _.isMemo = function(u) {
    return A(u) === b;
  }, _.isPortal = function(u) {
    return A(u) === t;
  }, _.isProfiler = function(u) {
    return A(u) === i;
  }, _.isStrictMode = function(u) {
    return A(u) === n;
  }, _.isSuspense = function(u) {
    return A(u) === p;
  }, _.isSuspenseList = function(u) {
    return A(u) === v;
  }, _.isValidElementType = function(u) {
    return typeof u == "string" || typeof u == "function" || u === r || u === i || u === n || u === p || u === v || u === w || typeof u == "object" && u !== null && (u.$$typeof === O || u.$$typeof === b || u.$$typeof === s || u.$$typeof === a || u.$$typeof === d || u.$$typeof === y || u.getModuleId !== void 0);
  }, _.typeOf = A, _;
}
var $ = {};
/**
 * @license React
 * react-is.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var vr;
function Zn() {
  return vr || (vr = 1, process.env.NODE_ENV !== "production" && function() {
    var e = Symbol.for("react.element"), t = Symbol.for("react.portal"), r = Symbol.for("react.fragment"), n = Symbol.for("react.strict_mode"), i = Symbol.for("react.profiler"), s = Symbol.for("react.provider"), a = Symbol.for("react.context"), l = Symbol.for("react.server_context"), d = Symbol.for("react.forward_ref"), p = Symbol.for("react.suspense"), v = Symbol.for("react.suspense_list"), b = Symbol.for("react.memo"), O = Symbol.for("react.lazy"), w = Symbol.for("react.offscreen"), y = !1, A = !1, u = !1, P = !1, X = !1, V;
    V = Symbol.for("react.module.reference");
    function q(T) {
      return !!(typeof T == "string" || typeof T == "function" || T === r || T === i || X || T === n || T === p || T === v || P || T === w || y || A || u || typeof T == "object" && T !== null && (T.$$typeof === O || T.$$typeof === b || T.$$typeof === s || T.$$typeof === a || T.$$typeof === d || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      T.$$typeof === V || T.getModuleId !== void 0));
    }
    function f(T) {
      if (typeof T == "object" && T !== null) {
        var M = T.$$typeof;
        switch (M) {
          case e:
            var se = T.type;
            switch (se) {
              case r:
              case i:
              case n:
              case p:
              case v:
                return se;
              default:
                var k = se && se.$$typeof;
                switch (k) {
                  case l:
                  case a:
                  case d:
                  case O:
                  case b:
                  case s:
                    return k;
                  default:
                    return M;
                }
            }
          case t:
            return M;
        }
      }
    }
    var re = a, de = s, B = e, ue = d, pe = r, me = O, J = b, C = t, H = i, G = n, ie = p, oe = v, ne = !1, be = !1;
    function Xe(T) {
      return ne || (ne = !0, console.warn("The ReactIs.isAsyncMode() alias has been deprecated, and will be removed in React 18+.")), !1;
    }
    function Te(T) {
      return be || (be = !0, console.warn("The ReactIs.isConcurrentMode() alias has been deprecated, and will be removed in React 18+.")), !1;
    }
    function W(T) {
      return f(T) === a;
    }
    function S(T) {
      return f(T) === s;
    }
    function z(T) {
      return typeof T == "object" && T !== null && T.$$typeof === e;
    }
    function L(T) {
      return f(T) === d;
    }
    function c(T) {
      return f(T) === r;
    }
    function Z(T) {
      return f(T) === O;
    }
    function g(T) {
      return f(T) === b;
    }
    function Q(T) {
      return f(T) === t;
    }
    function D(T) {
      return f(T) === i;
    }
    function R(T) {
      return f(T) === n;
    }
    function m(T) {
      return f(T) === p;
    }
    function ce(T) {
      return f(T) === v;
    }
    $.ContextConsumer = re, $.ContextProvider = de, $.Element = B, $.ForwardRef = ue, $.Fragment = pe, $.Lazy = me, $.Memo = J, $.Portal = C, $.Profiler = H, $.StrictMode = G, $.Suspense = ie, $.SuspenseList = oe, $.isAsyncMode = Xe, $.isConcurrentMode = Te, $.isContextConsumer = W, $.isContextProvider = S, $.isElement = z, $.isForwardRef = L, $.isFragment = c, $.isLazy = Z, $.isMemo = g, $.isPortal = Q, $.isProfiler = D, $.isStrictMode = R, $.isSuspense = m, $.isSuspenseList = ce, $.isValidElementType = q, $.typeOf = f;
  }()), $;
}
(function(e) {
  process.env.NODE_ENV === "production" ? e.exports = Dn() : e.exports = Zn();
})(Mn);
function Rn(e) {
  function t(W, S, z, L, c) {
    for (var Z = 0, g = 0, Q = 0, D = 0, R, m, ce = 0, T = 0, M, se = M = R = 0, k = 0, Ae = 0, We = 0, ye = 0, Ge = z.length, Ee = Ge - 1, qe, x = "", fe = "", Je = "", Le = "", Ve; k < Ge; ) {
      if (m = z.charCodeAt(k), k === Ee && g + D + Q + Z !== 0 && (g !== 0 && (m = g === 47 ? 10 : 47), D = Q = Z = 0, Ge++, Ee++), g + D + Q + Z === 0) {
        if (k === Ee && (0 < Ae && (x = x.replace(O, "")), 0 < x.trim().length)) {
          switch (m) {
            case 32:
            case 9:
            case 59:
            case 13:
            case 10:
              break;
            default:
              x += z.charAt(k);
          }
          m = 59;
        }
        switch (m) {
          case 123:
            for (x = x.trim(), R = x.charCodeAt(0), M = 1, ye = ++k; k < Ge; ) {
              switch (m = z.charCodeAt(k)) {
                case 123:
                  M++;
                  break;
                case 125:
                  M--;
                  break;
                case 47:
                  switch (m = z.charCodeAt(k + 1)) {
                    case 42:
                    case 47:
                      e: {
                        for (se = k + 1; se < Ee; ++se)
                          switch (z.charCodeAt(se)) {
                            case 47:
                              if (m === 42 && z.charCodeAt(se - 1) === 42 && k + 2 !== se) {
                                k = se + 1;
                                break e;
                              }
                              break;
                            case 10:
                              if (m === 47) {
                                k = se + 1;
                                break e;
                              }
                          }
                        k = se;
                      }
                  }
                  break;
                case 91:
                  m++;
                case 40:
                  m++;
                case 34:
                case 39:
                  for (; k++ < Ee && z.charCodeAt(k) !== m; )
                    ;
              }
              if (M === 0)
                break;
              k++;
            }
            switch (M = z.substring(ye, k), R === 0 && (R = (x = x.replace(b, "").trim()).charCodeAt(0)), R) {
              case 64:
                switch (0 < Ae && (x = x.replace(O, "")), m = x.charCodeAt(1), m) {
                  case 100:
                  case 109:
                  case 115:
                  case 45:
                    Ae = S;
                    break;
                  default:
                    Ae = ie;
                }
                if (M = t(S, Ae, M, m, c + 1), ye = M.length, 0 < ne && (Ae = r(ie, x, We), Ve = l(3, M, Ae, S, C, J, ye, m, c, L), x = Ae.join(""), Ve !== void 0 && (ye = (M = Ve.trim()).length) === 0 && (m = 0, M = "")), 0 < ye)
                  switch (m) {
                    case 115:
                      x = x.replace(re, a);
                    case 100:
                    case 109:
                    case 45:
                      M = x + "{" + M + "}";
                      break;
                    case 107:
                      x = x.replace(X, "$1 $2"), M = x + "{" + M + "}", M = G === 1 || G === 2 && s("@" + M, 3) ? "@-webkit-" + M + "@" + M : "@" + M;
                      break;
                    default:
                      M = x + M, L === 112 && (M = (fe += M, ""));
                  }
                else
                  M = "";
                break;
              default:
                M = t(S, r(S, x, We), M, L, c + 1);
            }
            Je += M, M = We = Ae = se = R = 0, x = "", m = z.charCodeAt(++k);
            break;
          case 125:
          case 59:
            if (x = (0 < Ae ? x.replace(O, "") : x).trim(), 1 < (ye = x.length))
              switch (se === 0 && (R = x.charCodeAt(0), R === 45 || 96 < R && 123 > R) && (ye = (x = x.replace(" ", ":")).length), 0 < ne && (Ve = l(1, x, S, W, C, J, fe.length, L, c, L)) !== void 0 && (ye = (x = Ve.trim()).length) === 0 && (x = "\0\0"), R = x.charCodeAt(0), m = x.charCodeAt(1), R) {
                case 0:
                  break;
                case 64:
                  if (m === 105 || m === 99) {
                    Le += x + z.charAt(k);
                    break;
                  }
                default:
                  x.charCodeAt(ye - 1) !== 58 && (fe += i(x, R, m, x.charCodeAt(2)));
              }
            We = Ae = se = R = 0, x = "", m = z.charCodeAt(++k);
        }
      }
      switch (m) {
        case 13:
        case 10:
          g === 47 ? g = 0 : 1 + R === 0 && L !== 107 && 0 < x.length && (Ae = 1, x += "\0"), 0 < ne * Xe && l(0, x, S, W, C, J, fe.length, L, c, L), J = 1, C++;
          break;
        case 59:
        case 125:
          if (g + D + Q + Z === 0) {
            J++;
            break;
          }
        default:
          switch (J++, qe = z.charAt(k), m) {
            case 9:
            case 32:
              if (D + Z + g === 0)
                switch (ce) {
                  case 44:
                  case 58:
                  case 9:
                  case 32:
                    qe = "";
                    break;
                  default:
                    m !== 32 && (qe = " ");
                }
              break;
            case 0:
              qe = "\\0";
              break;
            case 12:
              qe = "\\f";
              break;
            case 11:
              qe = "\\v";
              break;
            case 38:
              D + g + Z === 0 && (Ae = We = 1, qe = "\f" + qe);
              break;
            case 108:
              if (D + g + Z + H === 0 && 0 < se)
                switch (k - se) {
                  case 2:
                    ce === 112 && z.charCodeAt(k - 3) === 58 && (H = ce);
                  case 8:
                    T === 111 && (H = T);
                }
              break;
            case 58:
              D + g + Z === 0 && (se = k);
              break;
            case 44:
              g + Q + D + Z === 0 && (Ae = 1, qe += "\r");
              break;
            case 34:
            case 39:
              g === 0 && (D = D === m ? 0 : D === 0 ? m : D);
              break;
            case 91:
              D + g + Q === 0 && Z++;
              break;
            case 93:
              D + g + Q === 0 && Z--;
              break;
            case 41:
              D + g + Z === 0 && Q--;
              break;
            case 40:
              if (D + g + Z === 0) {
                if (R === 0)
                  switch (2 * ce + 3 * T) {
                    case 533:
                      break;
                    default:
                      R = 1;
                  }
                Q++;
              }
              break;
            case 64:
              g + Q + D + Z + se + M === 0 && (M = 1);
              break;
            case 42:
            case 47:
              if (!(0 < D + Z + Q))
                switch (g) {
                  case 0:
                    switch (2 * m + 3 * z.charCodeAt(k + 1)) {
                      case 235:
                        g = 47;
                        break;
                      case 220:
                        ye = k, g = 42;
                    }
                    break;
                  case 42:
                    m === 47 && ce === 42 && ye + 2 !== k && (z.charCodeAt(ye + 2) === 33 && (fe += z.substring(ye, k + 1)), qe = "", g = 0);
                }
          }
          g === 0 && (x += qe);
      }
      T = ce, ce = m, k++;
    }
    if (ye = fe.length, 0 < ye) {
      if (Ae = S, 0 < ne && (Ve = l(2, fe, Ae, W, C, J, ye, L, c, L), Ve !== void 0 && (fe = Ve).length === 0))
        return Le + fe + Je;
      if (fe = Ae.join(",") + "{" + fe + "}", G * H !== 0) {
        switch (G !== 2 || s(fe, 2) || (H = 0), H) {
          case 111:
            fe = fe.replace(q, ":-moz-$1") + fe;
            break;
          case 112:
            fe = fe.replace(V, "::-webkit-input-$1") + fe.replace(V, "::-moz-$1") + fe.replace(V, ":-ms-input-$1") + fe;
        }
        H = 0;
      }
    }
    return Le + fe + Je;
  }
  function r(W, S, z) {
    var L = S.trim().split(u);
    S = L;
    var c = L.length, Z = W.length;
    switch (Z) {
      case 0:
      case 1:
        var g = 0;
        for (W = Z === 0 ? "" : W[0] + " "; g < c; ++g)
          S[g] = n(W, S[g], z).trim();
        break;
      default:
        var Q = g = 0;
        for (S = []; g < c; ++g)
          for (var D = 0; D < Z; ++D)
            S[Q++] = n(W[D] + " ", L[g], z).trim();
    }
    return S;
  }
  function n(W, S, z) {
    var L = S.charCodeAt(0);
    switch (33 > L && (L = (S = S.trim()).charCodeAt(0)), L) {
      case 38:
        return S.replace(P, "$1" + W.trim());
      case 58:
        return W.trim() + S.replace(P, "$1" + W.trim());
      default:
        if (0 < 1 * z && 0 < S.indexOf("\f"))
          return S.replace(P, (W.charCodeAt(0) === 58 ? "" : "$1") + W.trim());
    }
    return W + S;
  }
  function i(W, S, z, L) {
    var c = W + ";", Z = 2 * S + 3 * z + 4 * L;
    if (Z === 944) {
      W = c.indexOf(":", 9) + 1;
      var g = c.substring(W, c.length - 1).trim();
      return g = c.substring(0, W).trim() + g + ";", G === 1 || G === 2 && s(g, 1) ? "-webkit-" + g + g : g;
    }
    if (G === 0 || G === 2 && !s(c, 1))
      return c;
    switch (Z) {
      case 1015:
        return c.charCodeAt(10) === 97 ? "-webkit-" + c + c : c;
      case 951:
        return c.charCodeAt(3) === 116 ? "-webkit-" + c + c : c;
      case 963:
        return c.charCodeAt(5) === 110 ? "-webkit-" + c + c : c;
      case 1009:
        if (c.charCodeAt(4) !== 100)
          break;
      case 969:
      case 942:
        return "-webkit-" + c + c;
      case 978:
        return "-webkit-" + c + "-moz-" + c + c;
      case 1019:
      case 983:
        return "-webkit-" + c + "-moz-" + c + "-ms-" + c + c;
      case 883:
        if (c.charCodeAt(8) === 45)
          return "-webkit-" + c + c;
        if (0 < c.indexOf("image-set(", 11))
          return c.replace(me, "$1-webkit-$2") + c;
        break;
      case 932:
        if (c.charCodeAt(4) === 45)
          switch (c.charCodeAt(5)) {
            case 103:
              return "-webkit-box-" + c.replace("-grow", "") + "-webkit-" + c + "-ms-" + c.replace("grow", "positive") + c;
            case 115:
              return "-webkit-" + c + "-ms-" + c.replace("shrink", "negative") + c;
            case 98:
              return "-webkit-" + c + "-ms-" + c.replace("basis", "preferred-size") + c;
          }
        return "-webkit-" + c + "-ms-" + c + c;
      case 964:
        return "-webkit-" + c + "-ms-flex-" + c + c;
      case 1023:
        if (c.charCodeAt(8) !== 99)
          break;
        return g = c.substring(c.indexOf(":", 15)).replace("flex-", "").replace("space-between", "justify"), "-webkit-box-pack" + g + "-webkit-" + c + "-ms-flex-pack" + g + c;
      case 1005:
        return y.test(c) ? c.replace(w, ":-webkit-") + c.replace(w, ":-moz-") + c : c;
      case 1e3:
        switch (g = c.substring(13).trim(), S = g.indexOf("-") + 1, g.charCodeAt(0) + g.charCodeAt(S)) {
          case 226:
            g = c.replace(f, "tb");
            break;
          case 232:
            g = c.replace(f, "tb-rl");
            break;
          case 220:
            g = c.replace(f, "lr");
            break;
          default:
            return c;
        }
        return "-webkit-" + c + "-ms-" + g + c;
      case 1017:
        if (c.indexOf("sticky", 9) === -1)
          break;
      case 975:
        switch (S = (c = W).length - 10, g = (c.charCodeAt(S) === 33 ? c.substring(0, S) : c).substring(W.indexOf(":", 7) + 1).trim(), Z = g.charCodeAt(0) + (g.charCodeAt(7) | 0)) {
          case 203:
            if (111 > g.charCodeAt(8))
              break;
          case 115:
            c = c.replace(g, "-webkit-" + g) + ";" + c;
            break;
          case 207:
          case 102:
            c = c.replace(g, "-webkit-" + (102 < Z ? "inline-" : "") + "box") + ";" + c.replace(g, "-webkit-" + g) + ";" + c.replace(g, "-ms-" + g + "box") + ";" + c;
        }
        return c + ";";
      case 938:
        if (c.charCodeAt(5) === 45)
          switch (c.charCodeAt(6)) {
            case 105:
              return g = c.replace("-items", ""), "-webkit-" + c + "-webkit-box-" + g + "-ms-flex-" + g + c;
            case 115:
              return "-webkit-" + c + "-ms-flex-item-" + c.replace(B, "") + c;
            default:
              return "-webkit-" + c + "-ms-flex-line-pack" + c.replace("align-content", "").replace(B, "") + c;
          }
        break;
      case 973:
      case 989:
        if (c.charCodeAt(3) !== 45 || c.charCodeAt(4) === 122)
          break;
      case 931:
      case 953:
        if (pe.test(W) === !0)
          return (g = W.substring(W.indexOf(":") + 1)).charCodeAt(0) === 115 ? i(W.replace("stretch", "fill-available"), S, z, L).replace(":fill-available", ":stretch") : c.replace(g, "-webkit-" + g) + c.replace(g, "-moz-" + g.replace("fill-", "")) + c;
        break;
      case 962:
        if (c = "-webkit-" + c + (c.charCodeAt(5) === 102 ? "-ms-" + c : "") + c, z + L === 211 && c.charCodeAt(13) === 105 && 0 < c.indexOf("transform", 10))
          return c.substring(0, c.indexOf(";", 27) + 1).replace(A, "$1-webkit-$2") + c;
    }
    return c;
  }
  function s(W, S) {
    var z = W.indexOf(S === 1 ? ":" : "{"), L = W.substring(0, S !== 3 ? z : 10);
    return z = W.substring(z + 1, W.length - 1), be(S !== 2 ? L : L.replace(ue, "$1"), z, S);
  }
  function a(W, S) {
    var z = i(S, S.charCodeAt(0), S.charCodeAt(1), S.charCodeAt(2));
    return z !== S + ";" ? z.replace(de, " or ($1)").substring(4) : "(" + S + ")";
  }
  function l(W, S, z, L, c, Z, g, Q, D, R) {
    for (var m = 0, ce = S, T; m < ne; ++m)
      switch (T = oe[m].call(v, W, ce, z, L, c, Z, g, Q, D, R)) {
        case void 0:
        case !1:
        case !0:
        case null:
          break;
        default:
          ce = T;
      }
    if (ce !== S)
      return ce;
  }
  function d(W) {
    switch (W) {
      case void 0:
      case null:
        ne = oe.length = 0;
        break;
      default:
        if (typeof W == "function")
          oe[ne++] = W;
        else if (typeof W == "object")
          for (var S = 0, z = W.length; S < z; ++S)
            d(W[S]);
        else
          Xe = !!W | 0;
    }
    return d;
  }
  function p(W) {
    return W = W.prefix, W !== void 0 && (be = null, W ? typeof W != "function" ? G = 1 : (G = 2, be = W) : G = 0), p;
  }
  function v(W, S) {
    var z = W;
    if (33 > z.charCodeAt(0) && (z = z.trim()), Te = z, z = [Te], 0 < ne) {
      var L = l(-1, S, z, z, C, J, 0, 0, 0, 0);
      L !== void 0 && typeof L == "string" && (S = L);
    }
    var c = t(ie, z, S, 0, 0);
    return 0 < ne && (L = l(-2, c, z, z, C, J, c.length, 0, 0, 0), L !== void 0 && (c = L)), Te = "", H = 0, J = C = 1, c;
  }
  var b = /^\0+/g, O = /[\0\r\f]/g, w = /: */g, y = /zoo|gra/, A = /([,: ])(transform)/g, u = /,\r+?/g, P = /([\t\r\n ])*\f?&/g, X = /@(k\w+)\s*(\S*)\s*/, V = /::(place)/g, q = /:(read-only)/g, f = /[svh]\w+-[tblr]{2}/, re = /\(\s*(.*)\s*\)/g, de = /([\s\S]*?);/g, B = /-self|flex-/g, ue = /[^]*?(:[rp][el]a[\w-]+)[^]*/, pe = /stretch|:\s*\w+\-(?:conte|avail)/, me = /([^-])(image-set\()/, J = 1, C = 1, H = 0, G = 1, ie = [], oe = [], ne = 0, be = null, Xe = 0, Te = "";
  return v.use = d, v.set = p, e !== void 0 && p(e), v;
}
var kn = {
  animationIterationCount: 1,
  borderImageOutset: 1,
  borderImageSlice: 1,
  borderImageWidth: 1,
  boxFlex: 1,
  boxFlexGroup: 1,
  boxOrdinalGroup: 1,
  columnCount: 1,
  columns: 1,
  flex: 1,
  flexGrow: 1,
  flexPositive: 1,
  flexShrink: 1,
  flexNegative: 1,
  flexOrder: 1,
  gridRow: 1,
  gridRowEnd: 1,
  gridRowSpan: 1,
  gridRowStart: 1,
  gridColumn: 1,
  gridColumnEnd: 1,
  gridColumnSpan: 1,
  gridColumnStart: 1,
  msGridRow: 1,
  msGridRowSpan: 1,
  msGridColumn: 1,
  msGridColumnSpan: 1,
  fontWeight: 1,
  lineHeight: 1,
  opacity: 1,
  order: 1,
  orphans: 1,
  tabSize: 1,
  widows: 1,
  zIndex: 1,
  zoom: 1,
  WebkitLineClamp: 1,
  // SVG-related properties
  fillOpacity: 1,
  floodOpacity: 1,
  stopOpacity: 1,
  strokeDasharray: 1,
  strokeDashoffset: 1,
  strokeMiterlimit: 1,
  strokeOpacity: 1,
  strokeWidth: 1
};
function Yn(e) {
  var t = /* @__PURE__ */ Object.create(null);
  return function(r) {
    return t[r] === void 0 && (t[r] = e(r)), t[r];
  };
}
var jn = /^((children|dangerouslySetInnerHTML|key|ref|autoFocus|defaultValue|defaultChecked|innerHTML|suppressContentEditableWarning|suppressHydrationWarning|valueLink|abbr|accept|acceptCharset|accessKey|action|allow|allowUserMedia|allowPaymentRequest|allowFullScreen|allowTransparency|alt|async|autoComplete|autoPlay|capture|cellPadding|cellSpacing|challenge|charSet|checked|cite|classID|className|cols|colSpan|content|contentEditable|contextMenu|controls|controlsList|coords|crossOrigin|data|dateTime|decoding|default|defer|dir|disabled|disablePictureInPicture|download|draggable|encType|enterKeyHint|form|formAction|formEncType|formMethod|formNoValidate|formTarget|frameBorder|headers|height|hidden|high|href|hrefLang|htmlFor|httpEquiv|id|inputMode|integrity|is|keyParams|keyType|kind|label|lang|list|loading|loop|low|marginHeight|marginWidth|max|maxLength|media|mediaGroup|method|min|minLength|multiple|muted|name|nonce|noValidate|open|optimum|pattern|placeholder|playsInline|poster|preload|profile|radioGroup|readOnly|referrerPolicy|rel|required|reversed|role|rows|rowSpan|sandbox|scope|scoped|scrolling|seamless|selected|shape|size|sizes|slot|span|spellCheck|src|srcDoc|srcLang|srcSet|start|step|style|summary|tabIndex|target|title|translate|type|useMap|value|width|wmode|wrap|about|datatype|inlist|prefix|property|resource|typeof|vocab|autoCapitalize|autoCorrect|autoSave|color|incremental|fallback|inert|itemProp|itemScope|itemType|itemID|itemRef|on|option|results|security|unselectable|accentHeight|accumulate|additive|alignmentBaseline|allowReorder|alphabetic|amplitude|arabicForm|ascent|attributeName|attributeType|autoReverse|azimuth|baseFrequency|baselineShift|baseProfile|bbox|begin|bias|by|calcMode|capHeight|clip|clipPathUnits|clipPath|clipRule|colorInterpolation|colorInterpolationFilters|colorProfile|colorRendering|contentScriptType|contentStyleType|cursor|cx|cy|d|decelerate|descent|diffuseConstant|direction|display|divisor|dominantBaseline|dur|dx|dy|edgeMode|elevation|enableBackground|end|exponent|externalResourcesRequired|fill|fillOpacity|fillRule|filter|filterRes|filterUnits|floodColor|floodOpacity|focusable|fontFamily|fontSize|fontSizeAdjust|fontStretch|fontStyle|fontVariant|fontWeight|format|from|fr|fx|fy|g1|g2|glyphName|glyphOrientationHorizontal|glyphOrientationVertical|glyphRef|gradientTransform|gradientUnits|hanging|horizAdvX|horizOriginX|ideographic|imageRendering|in|in2|intercept|k|k1|k2|k3|k4|kernelMatrix|kernelUnitLength|kerning|keyPoints|keySplines|keyTimes|lengthAdjust|letterSpacing|lightingColor|limitingConeAngle|local|markerEnd|markerMid|markerStart|markerHeight|markerUnits|markerWidth|mask|maskContentUnits|maskUnits|mathematical|mode|numOctaves|offset|opacity|operator|order|orient|orientation|origin|overflow|overlinePosition|overlineThickness|panose1|paintOrder|pathLength|patternContentUnits|patternTransform|patternUnits|pointerEvents|points|pointsAtX|pointsAtY|pointsAtZ|preserveAlpha|preserveAspectRatio|primitiveUnits|r|radius|refX|refY|renderingIntent|repeatCount|repeatDur|requiredExtensions|requiredFeatures|restart|result|rotate|rx|ry|scale|seed|shapeRendering|slope|spacing|specularConstant|specularExponent|speed|spreadMethod|startOffset|stdDeviation|stemh|stemv|stitchTiles|stopColor|stopOpacity|strikethroughPosition|strikethroughThickness|string|stroke|strokeDasharray|strokeDashoffset|strokeLinecap|strokeLinejoin|strokeMiterlimit|strokeOpacity|strokeWidth|surfaceScale|systemLanguage|tableValues|targetX|targetY|textAnchor|textDecoration|textRendering|textLength|to|transform|u1|u2|underlinePosition|underlineThickness|unicode|unicodeBidi|unicodeRange|unitsPerEm|vAlphabetic|vHanging|vIdeographic|vMathematical|values|vectorEffect|version|vertAdvY|vertOriginX|vertOriginY|viewBox|viewTarget|visibility|widths|wordSpacing|writingMode|x|xHeight|x1|x2|xChannelSelector|xlinkActuate|xlinkArcrole|xlinkHref|xlinkRole|xlinkShow|xlinkTitle|xlinkType|xmlBase|xmlns|xmlnsXlink|xmlLang|xmlSpace|y|y1|y2|yChannelSelector|z|zoomAndPan|for|class|autofocus)|(([Dd][Aa][Tt][Aa]|[Aa][Rr][Ii][Aa]|x)-.*))$/, mr = /* @__PURE__ */ Yn(
  function(e) {
    return jn.test(e) || e.charCodeAt(0) === 111 && e.charCodeAt(1) === 110 && e.charCodeAt(2) < 91;
  }
  /* Z+1 */
), Zt = {}, Bn = {
  get exports() {
    return Zt;
  },
  set exports(e) {
    Zt = e;
  }
}, ee = {};
/** @license React v16.13.1
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Ar;
function Cn() {
  if (Ar)
    return ee;
  Ar = 1;
  var e = typeof Symbol == "function" && Symbol.for, t = e ? Symbol.for("react.element") : 60103, r = e ? Symbol.for("react.portal") : 60106, n = e ? Symbol.for("react.fragment") : 60107, i = e ? Symbol.for("react.strict_mode") : 60108, s = e ? Symbol.for("react.profiler") : 60114, a = e ? Symbol.for("react.provider") : 60109, l = e ? Symbol.for("react.context") : 60110, d = e ? Symbol.for("react.async_mode") : 60111, p = e ? Symbol.for("react.concurrent_mode") : 60111, v = e ? Symbol.for("react.forward_ref") : 60112, b = e ? Symbol.for("react.suspense") : 60113, O = e ? Symbol.for("react.suspense_list") : 60120, w = e ? Symbol.for("react.memo") : 60115, y = e ? Symbol.for("react.lazy") : 60116, A = e ? Symbol.for("react.block") : 60121, u = e ? Symbol.for("react.fundamental") : 60117, P = e ? Symbol.for("react.responder") : 60118, X = e ? Symbol.for("react.scope") : 60119;
  function V(f) {
    if (typeof f == "object" && f !== null) {
      var re = f.$$typeof;
      switch (re) {
        case t:
          switch (f = f.type, f) {
            case d:
            case p:
            case n:
            case s:
            case i:
            case b:
              return f;
            default:
              switch (f = f && f.$$typeof, f) {
                case l:
                case v:
                case y:
                case w:
                case a:
                  return f;
                default:
                  return re;
              }
          }
        case r:
          return re;
      }
    }
  }
  function q(f) {
    return V(f) === p;
  }
  return ee.AsyncMode = d, ee.ConcurrentMode = p, ee.ContextConsumer = l, ee.ContextProvider = a, ee.Element = t, ee.ForwardRef = v, ee.Fragment = n, ee.Lazy = y, ee.Memo = w, ee.Portal = r, ee.Profiler = s, ee.StrictMode = i, ee.Suspense = b, ee.isAsyncMode = function(f) {
    return q(f) || V(f) === d;
  }, ee.isConcurrentMode = q, ee.isContextConsumer = function(f) {
    return V(f) === l;
  }, ee.isContextProvider = function(f) {
    return V(f) === a;
  }, ee.isElement = function(f) {
    return typeof f == "object" && f !== null && f.$$typeof === t;
  }, ee.isForwardRef = function(f) {
    return V(f) === v;
  }, ee.isFragment = function(f) {
    return V(f) === n;
  }, ee.isLazy = function(f) {
    return V(f) === y;
  }, ee.isMemo = function(f) {
    return V(f) === w;
  }, ee.isPortal = function(f) {
    return V(f) === r;
  }, ee.isProfiler = function(f) {
    return V(f) === s;
  }, ee.isStrictMode = function(f) {
    return V(f) === i;
  }, ee.isSuspense = function(f) {
    return V(f) === b;
  }, ee.isValidElementType = function(f) {
    return typeof f == "string" || typeof f == "function" || f === n || f === p || f === s || f === i || f === b || f === O || typeof f == "object" && f !== null && (f.$$typeof === y || f.$$typeof === w || f.$$typeof === a || f.$$typeof === l || f.$$typeof === v || f.$$typeof === u || f.$$typeof === P || f.$$typeof === X || f.$$typeof === A);
  }, ee.typeOf = V, ee;
}
var te = {};
/** @license React v16.13.1
 * react-is.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var hr;
function Hn() {
  return hr || (hr = 1, process.env.NODE_ENV !== "production" && function() {
    var e = typeof Symbol == "function" && Symbol.for, t = e ? Symbol.for("react.element") : 60103, r = e ? Symbol.for("react.portal") : 60106, n = e ? Symbol.for("react.fragment") : 60107, i = e ? Symbol.for("react.strict_mode") : 60108, s = e ? Symbol.for("react.profiler") : 60114, a = e ? Symbol.for("react.provider") : 60109, l = e ? Symbol.for("react.context") : 60110, d = e ? Symbol.for("react.async_mode") : 60111, p = e ? Symbol.for("react.concurrent_mode") : 60111, v = e ? Symbol.for("react.forward_ref") : 60112, b = e ? Symbol.for("react.suspense") : 60113, O = e ? Symbol.for("react.suspense_list") : 60120, w = e ? Symbol.for("react.memo") : 60115, y = e ? Symbol.for("react.lazy") : 60116, A = e ? Symbol.for("react.block") : 60121, u = e ? Symbol.for("react.fundamental") : 60117, P = e ? Symbol.for("react.responder") : 60118, X = e ? Symbol.for("react.scope") : 60119;
    function V(m) {
      return typeof m == "string" || typeof m == "function" || // Note: its typeof might be other than 'symbol' or 'number' if it's a polyfill.
      m === n || m === p || m === s || m === i || m === b || m === O || typeof m == "object" && m !== null && (m.$$typeof === y || m.$$typeof === w || m.$$typeof === a || m.$$typeof === l || m.$$typeof === v || m.$$typeof === u || m.$$typeof === P || m.$$typeof === X || m.$$typeof === A);
    }
    function q(m) {
      if (typeof m == "object" && m !== null) {
        var ce = m.$$typeof;
        switch (ce) {
          case t:
            var T = m.type;
            switch (T) {
              case d:
              case p:
              case n:
              case s:
              case i:
              case b:
                return T;
              default:
                var M = T && T.$$typeof;
                switch (M) {
                  case l:
                  case v:
                  case y:
                  case w:
                  case a:
                    return M;
                  default:
                    return ce;
                }
            }
          case r:
            return ce;
        }
      }
    }
    var f = d, re = p, de = l, B = a, ue = t, pe = v, me = n, J = y, C = w, H = r, G = s, ie = i, oe = b, ne = !1;
    function be(m) {
      return ne || (ne = !0, console.warn("The ReactIs.isAsyncMode() alias has been deprecated, and will be removed in React 17+. Update your code to use ReactIs.isConcurrentMode() instead. It has the exact same API.")), Xe(m) || q(m) === d;
    }
    function Xe(m) {
      return q(m) === p;
    }
    function Te(m) {
      return q(m) === l;
    }
    function W(m) {
      return q(m) === a;
    }
    function S(m) {
      return typeof m == "object" && m !== null && m.$$typeof === t;
    }
    function z(m) {
      return q(m) === v;
    }
    function L(m) {
      return q(m) === n;
    }
    function c(m) {
      return q(m) === y;
    }
    function Z(m) {
      return q(m) === w;
    }
    function g(m) {
      return q(m) === r;
    }
    function Q(m) {
      return q(m) === s;
    }
    function D(m) {
      return q(m) === i;
    }
    function R(m) {
      return q(m) === b;
    }
    te.AsyncMode = f, te.ConcurrentMode = re, te.ContextConsumer = de, te.ContextProvider = B, te.Element = ue, te.ForwardRef = pe, te.Fragment = me, te.Lazy = J, te.Memo = C, te.Portal = H, te.Profiler = G, te.StrictMode = ie, te.Suspense = oe, te.isAsyncMode = be, te.isConcurrentMode = Xe, te.isContextConsumer = Te, te.isContextProvider = W, te.isElement = S, te.isForwardRef = z, te.isFragment = L, te.isLazy = c, te.isMemo = Z, te.isPortal = g, te.isProfiler = Q, te.isStrictMode = D, te.isSuspense = R, te.isValidElementType = V, te.typeOf = q;
  }()), te;
}
(function(e) {
  process.env.NODE_ENV === "production" ? e.exports = Cn() : e.exports = Hn();
})(Bn);
var Gt = Zt, Un = {
  childContextTypes: !0,
  contextType: !0,
  contextTypes: !0,
  defaultProps: !0,
  displayName: !0,
  getDefaultProps: !0,
  getDerivedStateFromError: !0,
  getDerivedStateFromProps: !0,
  mixins: !0,
  propTypes: !0,
  type: !0
}, Gn = {
  name: !0,
  length: !0,
  prototype: !0,
  caller: !0,
  callee: !0,
  arguments: !0,
  arity: !0
}, Jn = {
  $$typeof: !0,
  render: !0,
  defaultProps: !0,
  displayName: !0,
  propTypes: !0
}, Dr = {
  $$typeof: !0,
  compare: !0,
  defaultProps: !0,
  displayName: !0,
  propTypes: !0,
  type: !0
}, Jt = {};
Jt[Gt.ForwardRef] = Jn;
Jt[Gt.Memo] = Dr;
function yr(e) {
  return Gt.isMemo(e) ? Dr : Jt[e.$$typeof] || Un;
}
var Kn = Object.defineProperty, In = Object.getOwnPropertyNames, br = Object.getOwnPropertySymbols, Qn = Object.getOwnPropertyDescriptor, _n = Object.getPrototypeOf, gr = Object.prototype;
function Zr(e, t, r) {
  if (typeof t != "string") {
    if (gr) {
      var n = _n(t);
      n && n !== gr && Zr(e, n, r);
    }
    var i = In(t);
    br && (i = i.concat(br(t)));
    for (var s = yr(e), a = yr(t), l = 0; l < i.length; ++l) {
      var d = i[l];
      if (!Gn[d] && !(r && r[d]) && !(a && a[d]) && !(s && s[d])) {
        var p = Qn(t, d);
        try {
          Kn(e, d, p);
        } catch {
        }
      }
    }
  }
  return e;
}
var $n = Zr;
function ze() {
  return (ze = Object.assign || function(e) {
    for (var t = 1; t < arguments.length; t++) {
      var r = arguments[t];
      for (var n in r)
        Object.prototype.hasOwnProperty.call(r, n) && (e[n] = r[n]);
    }
    return e;
  }).apply(this, arguments);
}
var wr = function(e, t) {
  for (var r = [e[0]], n = 0, i = t.length; n < i; n += 1)
    r.push(t[n], e[n + 1]);
  return r;
}, Rt = function(e) {
  return e !== null && typeof e == "object" && (e.toString ? e.toString() : Object.prototype.toString.call(e)) === "[object Object]" && !lt.typeOf(e);
}, bt = Object.freeze([]), je = Object.freeze({});
function et(e) {
  return typeof e == "function";
}
function kt(e) {
  return process.env.NODE_ENV !== "production" && typeof e == "string" && e || e.displayName || e.name || "Component";
}
function Kt(e) {
  return e && typeof e.styledComponentId == "string";
}
var tt = typeof process < "u" && (process.env.REACT_APP_SC_ATTR || process.env.SC_ATTR) || "data-styled", It = typeof window < "u" && "HTMLElement" in window, eo = Boolean(typeof SC_DISABLE_SPEEDY == "boolean" ? SC_DISABLE_SPEEDY : typeof process < "u" && process.env.REACT_APP_SC_DISABLE_SPEEDY !== void 0 && process.env.REACT_APP_SC_DISABLE_SPEEDY !== "" ? process.env.REACT_APP_SC_DISABLE_SPEEDY !== "false" && process.env.REACT_APP_SC_DISABLE_SPEEDY : typeof process < "u" && process.env.SC_DISABLE_SPEEDY !== void 0 && process.env.SC_DISABLE_SPEEDY !== "" ? process.env.SC_DISABLE_SPEEDY !== "false" && process.env.SC_DISABLE_SPEEDY : process.env.NODE_ENV !== "production"), to = {}, ro = process.env.NODE_ENV !== "production" ? { 1: `Cannot create styled-component for component: %s.

`, 2: `Can't collect styles once you've consumed a \`ServerStyleSheet\`'s styles! \`ServerStyleSheet\` is a one off instance for each server-side render cycle.

- Are you trying to reuse it across renders?
- Are you accidentally calling collectStyles twice?

`, 3: `Streaming SSR is only supported in a Node.js environment; Please do not try to call this method in the browser.

`, 4: `The \`StyleSheetManager\` expects a valid target or sheet prop!

- Does this error occur on the client and is your target falsy?
- Does this error occur on the server and is the sheet falsy?

`, 5: `The clone method cannot be used on the client!

- Are you running in a client-like environment on the server?
- Are you trying to run SSR on the client?

`, 6: `Trying to insert a new style tag, but the given Node is unmounted!

- Are you using a custom target that isn't mounted?
- Does your document not have a valid head element?
- Have you accidentally removed a style tag manually?

`, 7: 'ThemeProvider: Please return an object from your "theme" prop function, e.g.\n\n```js\ntheme={() => ({})}\n```\n\n', 8: `ThemeProvider: Please make your "theme" prop an object.

`, 9: "Missing document `<head>`\n\n", 10: `Cannot find a StyleSheet instance. Usually this happens if there are multiple copies of styled-components loaded at once. Check out this issue for how to troubleshoot and fix the common cases where this situation can happen: https://github.com/styled-components/styled-components/issues/1941#issuecomment-417862021

`, 11: `_This error was replaced with a dev-time warning, it will be deleted for v4 final._ [createGlobalStyle] received children which will not be rendered. Please use the component without passing children elements.

`, 12: "It seems you are interpolating a keyframe declaration (%s) into an untagged string. This was supported in styled-components v3, but is not longer supported in v4 as keyframes are now injected on-demand. Please wrap your string in the css\\`\\` helper which ensures the styles are injected correctly. See https://www.styled-components.com/docs/api#css\n\n", 13: `%s is not a styled component and cannot be referred to via component selector. See https://www.styled-components.com/docs/advanced#referring-to-other-components for more details.

`, 14: `ThemeProvider: "theme" prop is required.

`, 15: "A stylis plugin has been supplied that is not named. We need a name for each plugin to be able to prevent styling collisions between different stylis configurations within the same app. Before you pass your plugin to `<StyleSheetManager stylisPlugins={[]}>`, please make sure each plugin is uniquely-named, e.g.\n\n```js\nObject.defineProperty(importedPlugin, 'name', { value: 'some-unique-name' });\n```\n\n", 16: `Reached the limit of how many styled components may be created at group %s.
You may only create up to 1,073,741,824 components. If you're creating components dynamically,
as for instance in your render method then you may be running into this limitation.

`, 17: `CSSStyleSheet could not be found on HTMLStyleElement.
Has styled-components' style tag been unmounted or altered by another script?
` } : {};
function no() {
  for (var e = arguments.length <= 0 ? void 0 : arguments[0], t = [], r = 1, n = arguments.length; r < n; r += 1)
    t.push(r < 0 || arguments.length <= r ? void 0 : arguments[r]);
  return t.forEach(function(i) {
    e = e.replace(/%[a-z]/, i);
  }), e;
}
function De(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  throw process.env.NODE_ENV === "production" ? new Error("An error occurred. See https://git.io/JUIaE#" + e + " for more information." + (r.length > 0 ? " Args: " + r.join(", ") : "")) : new Error(no.apply(void 0, [ro[e]].concat(r)).trim());
}
var oo = function() {
  function e(r) {
    this.groupSizes = new Uint32Array(512), this.length = 512, this.tag = r;
  }
  var t = e.prototype;
  return t.indexOfGroup = function(r) {
    for (var n = 0, i = 0; i < r; i++)
      n += this.groupSizes[i];
    return n;
  }, t.insertRules = function(r, n) {
    if (r >= this.groupSizes.length) {
      for (var i = this.groupSizes, s = i.length, a = s; r >= a; )
        (a <<= 1) < 0 && De(16, "" + r);
      this.groupSizes = new Uint32Array(a), this.groupSizes.set(i), this.length = a;
      for (var l = s; l < a; l++)
        this.groupSizes[l] = 0;
    }
    for (var d = this.indexOfGroup(r + 1), p = 0, v = n.length; p < v; p++)
      this.tag.insertRule(d, n[p]) && (this.groupSizes[r]++, d++);
  }, t.clearGroup = function(r) {
    if (r < this.length) {
      var n = this.groupSizes[r], i = this.indexOfGroup(r), s = i + n;
      this.groupSizes[r] = 0;
      for (var a = i; a < s; a++)
        this.tag.deleteRule(i);
    }
  }, t.getGroup = function(r) {
    var n = "";
    if (r >= this.length || this.groupSizes[r] === 0)
      return n;
    for (var i = this.groupSizes[r], s = this.indexOfGroup(r), a = s + i, l = s; l < a; l++)
      n += this.tag.getRule(l) + `/*!sc*/
`;
    return n;
  }, e;
}(), mt = /* @__PURE__ */ new Map(), gt = /* @__PURE__ */ new Map(), it = 1, ft = function(e) {
  if (mt.has(e))
    return mt.get(e);
  for (; gt.has(it); )
    it++;
  var t = it++;
  return process.env.NODE_ENV !== "production" && ((0 | t) < 0 || t > 1 << 30) && De(16, "" + t), mt.set(e, t), gt.set(t, e), t;
}, ao = function(e) {
  return gt.get(e);
}, io = function(e, t) {
  t >= it && (it = t + 1), mt.set(e, t), gt.set(t, e);
}, so = "style[" + tt + '][data-styled-version="5.3.6"]', lo = new RegExp("^" + tt + '\\.g(\\d+)\\[id="([\\w\\d-]+)"\\].*?"([^"]*)'), uo = function(e, t, r) {
  for (var n, i = r.split(","), s = 0, a = i.length; s < a; s++)
    (n = i[s]) && e.registerName(t, n);
}, co = function(e, t) {
  for (var r = (t.textContent || "").split(`/*!sc*/
`), n = [], i = 0, s = r.length; i < s; i++) {
    var a = r[i].trim();
    if (a) {
      var l = a.match(lo);
      if (l) {
        var d = 0 | parseInt(l[1], 10), p = l[2];
        d !== 0 && (io(p, d), uo(e, p, l[3]), e.getTag().insertRules(d, n)), n.length = 0;
      } else
        n.push(a);
    }
  }
}, fo = function() {
  return typeof __webpack_nonce__ < "u" ? __webpack_nonce__ : null;
}, Rr = function(e) {
  var t = document.head, r = e || t, n = document.createElement("style"), i = function(l) {
    for (var d = l.childNodes, p = d.length; p >= 0; p--) {
      var v = d[p];
      if (v && v.nodeType === 1 && v.hasAttribute(tt))
        return v;
    }
  }(r), s = i !== void 0 ? i.nextSibling : null;
  n.setAttribute(tt, "active"), n.setAttribute("data-styled-version", "5.3.6");
  var a = fo();
  return a && n.setAttribute("nonce", a), r.insertBefore(n, s), n;
}, po = function() {
  function e(r) {
    var n = this.element = Rr(r);
    n.appendChild(document.createTextNode("")), this.sheet = function(i) {
      if (i.sheet)
        return i.sheet;
      for (var s = document.styleSheets, a = 0, l = s.length; a < l; a++) {
        var d = s[a];
        if (d.ownerNode === i)
          return d;
      }
      De(17);
    }(n), this.length = 0;
  }
  var t = e.prototype;
  return t.insertRule = function(r, n) {
    try {
      return this.sheet.insertRule(n, r), this.length++, !0;
    } catch {
      return !1;
    }
  }, t.deleteRule = function(r) {
    this.sheet.deleteRule(r), this.length--;
  }, t.getRule = function(r) {
    var n = this.sheet.cssRules[r];
    return n !== void 0 && typeof n.cssText == "string" ? n.cssText : "";
  }, e;
}(), vo = function() {
  function e(r) {
    var n = this.element = Rr(r);
    this.nodes = n.childNodes, this.length = 0;
  }
  var t = e.prototype;
  return t.insertRule = function(r, n) {
    if (r <= this.length && r >= 0) {
      var i = document.createTextNode(n), s = this.nodes[r];
      return this.element.insertBefore(i, s || null), this.length++, !0;
    }
    return !1;
  }, t.deleteRule = function(r) {
    this.element.removeChild(this.nodes[r]), this.length--;
  }, t.getRule = function(r) {
    return r < this.length ? this.nodes[r].textContent : "";
  }, e;
}(), mo = function() {
  function e(r) {
    this.rules = [], this.length = 0;
  }
  var t = e.prototype;
  return t.insertRule = function(r, n) {
    return r <= this.length && (this.rules.splice(r, 0, n), this.length++, !0);
  }, t.deleteRule = function(r) {
    this.rules.splice(r, 1), this.length--;
  }, t.getRule = function(r) {
    return r < this.length ? this.rules[r] : "";
  }, e;
}(), Pr = It, Ao = { isServer: !It, useCSSOMInjection: !eo }, wt = function() {
  function e(r, n, i) {
    r === void 0 && (r = je), n === void 0 && (n = {}), this.options = ze({}, Ao, {}, r), this.gs = n, this.names = new Map(i), this.server = !!r.isServer, !this.server && It && Pr && (Pr = !1, function(s) {
      for (var a = document.querySelectorAll(so), l = 0, d = a.length; l < d; l++) {
        var p = a[l];
        p && p.getAttribute(tt) !== "active" && (co(s, p), p.parentNode && p.parentNode.removeChild(p));
      }
    }(this));
  }
  e.registerId = function(r) {
    return ft(r);
  };
  var t = e.prototype;
  return t.reconstructWithOptions = function(r, n) {
    return n === void 0 && (n = !0), new e(ze({}, this.options, {}, r), this.gs, n && this.names || void 0);
  }, t.allocateGSInstance = function(r) {
    return this.gs[r] = (this.gs[r] || 0) + 1;
  }, t.getTag = function() {
    return this.tag || (this.tag = (i = (n = this.options).isServer, s = n.useCSSOMInjection, a = n.target, r = i ? new mo(a) : s ? new po(a) : new vo(a), new oo(r)));
    var r, n, i, s, a;
  }, t.hasNameForId = function(r, n) {
    return this.names.has(r) && this.names.get(r).has(n);
  }, t.registerName = function(r, n) {
    if (ft(r), this.names.has(r))
      this.names.get(r).add(n);
    else {
      var i = /* @__PURE__ */ new Set();
      i.add(n), this.names.set(r, i);
    }
  }, t.insertRules = function(r, n, i) {
    this.registerName(r, n), this.getTag().insertRules(ft(r), i);
  }, t.clearNames = function(r) {
    this.names.has(r) && this.names.get(r).clear();
  }, t.clearRules = function(r) {
    this.getTag().clearGroup(ft(r)), this.clearNames(r);
  }, t.clearTag = function() {
    this.tag = void 0;
  }, t.toString = function() {
    return function(r) {
      for (var n = r.getTag(), i = n.length, s = "", a = 0; a < i; a++) {
        var l = ao(a);
        if (l !== void 0) {
          var d = r.names.get(l), p = n.getGroup(a);
          if (d && p && d.size) {
            var v = tt + ".g" + a + '[id="' + l + '"]', b = "";
            d !== void 0 && d.forEach(function(O) {
              O.length > 0 && (b += O + ",");
            }), s += "" + p + v + '{content:"' + b + `"}/*!sc*/
`;
          }
        }
      }
      return s;
    }(this);
  }, e;
}(), ho = /(a)(d)/gi, Xr = function(e) {
  return String.fromCharCode(e + (e > 25 ? 39 : 97));
};
function Yt(e) {
  var t, r = "";
  for (t = Math.abs(e); t > 52; t = t / 52 | 0)
    r = Xr(t % 52) + r;
  return (Xr(t % 52) + r).replace(ho, "$1-$2");
}
var Ce = function(e, t) {
  for (var r = t.length; r; )
    e = 33 * e ^ t.charCodeAt(--r);
  return e;
}, kr = function(e) {
  return Ce(5381, e);
};
function Yr(e) {
  for (var t = 0; t < e.length; t += 1) {
    var r = e[t];
    if (et(r) && !Kt(r))
      return !1;
  }
  return !0;
}
var yo = kr("5.3.6"), bo = function() {
  function e(t, r, n) {
    this.rules = t, this.staticRulesId = "", this.isStatic = process.env.NODE_ENV === "production" && (n === void 0 || n.isStatic) && Yr(t), this.componentId = r, this.baseHash = Ce(yo, r), this.baseStyle = n, wt.registerId(r);
  }
  return e.prototype.generateAndInjectStyles = function(t, r, n) {
    var i = this.componentId, s = [];
    if (this.baseStyle && s.push(this.baseStyle.generateAndInjectStyles(t, r, n)), this.isStatic && !n.hash)
      if (this.staticRulesId && r.hasNameForId(i, this.staticRulesId))
        s.push(this.staticRulesId);
      else {
        var a = Ue(this.rules, t, r, n).join(""), l = Yt(Ce(this.baseHash, a) >>> 0);
        if (!r.hasNameForId(i, l)) {
          var d = n(a, "." + l, void 0, i);
          r.insertRules(i, l, d);
        }
        s.push(l), this.staticRulesId = l;
      }
    else {
      for (var p = this.rules.length, v = Ce(this.baseHash, n.hash), b = "", O = 0; O < p; O++) {
        var w = this.rules[O];
        if (typeof w == "string")
          b += w, process.env.NODE_ENV !== "production" && (v = Ce(v, w + O));
        else if (w) {
          var y = Ue(w, t, r, n), A = Array.isArray(y) ? y.join("") : y;
          v = Ce(v, A + O), b += A;
        }
      }
      if (b) {
        var u = Yt(v >>> 0);
        if (!r.hasNameForId(i, u)) {
          var P = n(b, "." + u, void 0, i);
          r.insertRules(i, u, P);
        }
        s.push(u);
      }
    }
    return s.join(" ");
  }, e;
}(), go = /^\s*\/\/.*$/gm, wo = [":", "[", ".", "#"];
function Po(e) {
  var t, r, n, i, s = e === void 0 ? je : e, a = s.options, l = a === void 0 ? je : a, d = s.plugins, p = d === void 0 ? bt : d, v = new Rn(l), b = [], O = function(A) {
    function u(P) {
      if (P)
        try {
          A(P + "}");
        } catch {
        }
    }
    return function(P, X, V, q, f, re, de, B, ue, pe) {
      switch (P) {
        case 1:
          if (ue === 0 && X.charCodeAt(0) === 64)
            return A(X + ";"), "";
          break;
        case 2:
          if (B === 0)
            return X + "/*|*/";
          break;
        case 3:
          switch (B) {
            case 102:
            case 112:
              return A(V[0] + X), "";
            default:
              return X + (pe === 0 ? "/*|*/" : "");
          }
        case -2:
          X.split("/*|*/}").forEach(u);
      }
    };
  }(function(A) {
    b.push(A);
  }), w = function(A, u, P) {
    return u === 0 && wo.indexOf(P[r.length]) !== -1 || P.match(i) ? A : "." + t;
  };
  function y(A, u, P, X) {
    X === void 0 && (X = "&");
    var V = A.replace(go, ""), q = u && P ? P + " " + u + " { " + V + " }" : V;
    return t = X, r = u, n = new RegExp("\\" + r + "\\b", "g"), i = new RegExp("(\\" + r + "\\b){2,}"), v(P || !u ? "" : u, q);
  }
  return v.use([].concat(p, [function(A, u, P) {
    A === 2 && P.length && P[0].lastIndexOf(r) > 0 && (P[0] = P[0].replace(n, w));
  }, O, function(A) {
    if (A === -2) {
      var u = b;
      return b = [], u;
    }
  }])), y.hash = p.length ? p.reduce(function(A, u) {
    return u.name || De(15), Ce(A, u.name);
  }, 5381).toString() : "", y;
}
var jr = E.createContext();
jr.Consumer;
var Br = E.createContext(), Xo = (Br.Consumer, new wt()), jt = Po();
function Cr() {
  return Re(jr) || Xo;
}
function Hr() {
  return Re(Br) || jt;
}
var qo = function() {
  function e(t, r) {
    var n = this;
    this.inject = function(i, s) {
      s === void 0 && (s = jt);
      var a = n.name + s.hash;
      i.hasNameForId(n.id, a) || i.insertRules(n.id, a, s(n.rules, a, "@keyframes"));
    }, this.toString = function() {
      return De(12, String(n.name));
    }, this.name = t, this.id = "sc-keyframes-" + t, this.rules = r;
  }
  return e.prototype.getName = function(t) {
    return t === void 0 && (t = jt), this.name + t.hash;
  }, e;
}(), No = /([A-Z])/, To = /([A-Z])/g, So = /^ms-/, Oo = function(e) {
  return "-" + e.toLowerCase();
};
function qr(e) {
  return No.test(e) ? e.replace(To, Oo).replace(So, "-ms-") : e;
}
var Nr = function(e) {
  return e == null || e === !1 || e === "";
};
function Ue(e, t, r, n) {
  if (Array.isArray(e)) {
    for (var i, s = [], a = 0, l = e.length; a < l; a += 1)
      (i = Ue(e[a], t, r, n)) !== "" && (Array.isArray(i) ? s.push.apply(s, i) : s.push(i));
    return s;
  }
  if (Nr(e))
    return "";
  if (Kt(e))
    return "." + e.styledComponentId;
  if (et(e)) {
    if (typeof (p = e) != "function" || p.prototype && p.prototype.isReactComponent || !t)
      return e;
    var d = e(t);
    return process.env.NODE_ENV !== "production" && lt.isElement(d) && console.warn(kt(e) + " is not a styled component and cannot be referred to via component selector. See https://www.styled-components.com/docs/advanced#referring-to-other-components for more details."), Ue(d, t, r, n);
  }
  var p;
  return e instanceof qo ? r ? (e.inject(r, n), e.getName(n)) : e : Rt(e) ? function v(b, O) {
    var w, y, A = [];
    for (var u in b)
      b.hasOwnProperty(u) && !Nr(b[u]) && (Array.isArray(b[u]) && b[u].isCss || et(b[u]) ? A.push(qr(u) + ":", b[u], ";") : Rt(b[u]) ? A.push.apply(A, v(b[u], u)) : A.push(qr(u) + ": " + (w = u, (y = b[u]) == null || typeof y == "boolean" || y === "" ? "" : typeof y != "number" || y === 0 || w in kn ? String(y).trim() : y + "px") + ";"));
    return O ? [O + " {"].concat(A, ["}"]) : A;
  }(e) : e.toString();
}
var Tr = function(e) {
  return Array.isArray(e) && (e.isCss = !0), e;
};
function I(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  return et(e) || Rt(e) ? Tr(Ue(wr(bt, [e].concat(r)))) : r.length === 0 && e.length === 1 && typeof e[0] == "string" ? e : Tr(Ue(wr(e, r)));
}
var Sr = /invalid hook call/i, dt = /* @__PURE__ */ new Set(), Ur = function(e, t) {
  if (process.env.NODE_ENV !== "production") {
    var r = "The component " + e + (t ? ' with the id of "' + t + '"' : "") + ` has been created dynamically.
You may see this warning because you've called styled inside another component.
To resolve this only create new StyledComponents outside of any render method and function component.`, n = console.error;
    try {
      var i = !0;
      console.error = function(s) {
        if (Sr.test(s))
          i = !1, dt.delete(r);
        else {
          for (var a = arguments.length, l = new Array(a > 1 ? a - 1 : 0), d = 1; d < a; d++)
            l[d - 1] = arguments[d];
          n.apply(void 0, [s].concat(l));
        }
      }, Me(), i && !dt.has(r) && (console.warn(r), dt.add(r));
    } catch (s) {
      Sr.test(s.message) && dt.delete(r);
    } finally {
      console.error = n;
    }
  }
}, Gr = function(e, t, r) {
  return r === void 0 && (r = je), e.theme !== r.theme && e.theme || t || r.theme;
}, zo = /[!"#$%&'()*+,./:;<=>?@[\\\]^`{|}~-]+/g, Wo = /(^-|-$)/g;
function Wt(e) {
  return e.replace(zo, "-").replace(Wo, "");
}
var Jr = function(e) {
  return Yt(kr(e) >>> 0);
};
function pt(e) {
  return typeof e == "string" && (process.env.NODE_ENV === "production" || e.charAt(0) === e.charAt(0).toLowerCase());
}
var Bt = function(e) {
  return typeof e == "function" || typeof e == "object" && e !== null && !Array.isArray(e);
}, Vo = function(e) {
  return e !== "__proto__" && e !== "constructor" && e !== "prototype";
};
function Eo(e, t, r) {
  var n = e[r];
  Bt(t) && Bt(n) ? Kr(n, t) : e[r] = t;
}
function Kr(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  for (var i = 0, s = r; i < s.length; i++) {
    var a = s[i];
    if (Bt(a))
      for (var l in a)
        Vo(l) && Eo(e, a[l], l);
  }
  return e;
}
var ut = E.createContext();
ut.Consumer;
function Lo(e) {
  var t = Re(ut), r = zn(function() {
    return function(n, i) {
      if (!n)
        return De(14);
      if (et(n)) {
        var s = n(i);
        return process.env.NODE_ENV === "production" || s !== null && !Array.isArray(s) && typeof s == "object" ? s : De(7);
      }
      return Array.isArray(n) || typeof n != "object" ? De(8) : i ? ze({}, i, {}, n) : n;
    }(e.theme, t);
  }, [e.theme, t]);
  return e.children ? E.createElement(ut.Provider, { value: r }, e.children) : null;
}
var Vt = {};
function Ir(e, t, r) {
  var n = Kt(e), i = !pt(e), s = t.attrs, a = s === void 0 ? bt : s, l = t.componentId, d = l === void 0 ? function(X, V) {
    var q = typeof X != "string" ? "sc" : Wt(X);
    Vt[q] = (Vt[q] || 0) + 1;
    var f = q + "-" + Jr("5.3.6" + q + Vt[q]);
    return V ? V + "-" + f : f;
  }(t.displayName, t.parentComponentId) : l, p = t.displayName, v = p === void 0 ? function(X) {
    return pt(X) ? "styled." + X : "Styled(" + kt(X) + ")";
  }(e) : p, b = t.displayName && t.componentId ? Wt(t.displayName) + "-" + t.componentId : t.componentId || d, O = n && e.attrs ? Array.prototype.concat(e.attrs, a).filter(Boolean) : a, w = t.shouldForwardProp;
  n && e.shouldForwardProp && (w = t.shouldForwardProp ? function(X, V, q) {
    return e.shouldForwardProp(X, V, q) && t.shouldForwardProp(X, V, q);
  } : e.shouldForwardProp);
  var y, A = new bo(r, b, n ? e.componentStyle : void 0), u = A.isStatic && a.length === 0, P = function(X, V) {
    return function(q, f, re, de) {
      var B = q.attrs, ue = q.componentStyle, pe = q.defaultProps, me = q.foldedComponentIds, J = q.shouldForwardProp, C = q.styledComponentId, H = q.target;
      process.env.NODE_ENV !== "production" && cr(C);
      var G = function(L, c, Z) {
        L === void 0 && (L = je);
        var g = ze({}, c, { theme: L }), Q = {};
        return Z.forEach(function(D) {
          var R, m, ce, T = D;
          for (R in et(T) && (T = T(g)), T)
            g[R] = Q[R] = R === "className" ? (m = Q[R], ce = T[R], m && ce ? m + " " + ce : m || ce) : T[R];
        }), [g, Q];
      }(Gr(f, Re(ut), pe) || je, f, B), ie = G[0], oe = G[1], ne = function(L, c, Z, g) {
        var Q = Cr(), D = Hr(), R = c ? L.generateAndInjectStyles(je, Q, D) : L.generateAndInjectStyles(Z, Q, D);
        return process.env.NODE_ENV !== "production" && cr(R), process.env.NODE_ENV !== "production" && !c && g && g(R), R;
      }(ue, de, ie, process.env.NODE_ENV !== "production" ? q.warnTooManyClasses : void 0), be = re, Xe = oe.$as || f.$as || oe.as || f.as || H, Te = pt(Xe), W = oe !== f ? ze({}, f, {}, oe) : f, S = {};
      for (var z in W)
        z[0] !== "$" && z !== "as" && (z === "forwardedAs" ? S.as = W[z] : (J ? J(z, mr, Xe) : !Te || mr(z)) && (S[z] = W[z]));
      return f.style && oe.style !== f.style && (S.style = ze({}, f.style, {}, oe.style)), S.className = Array.prototype.concat(me, C, ne !== C ? ne : null, f.className, oe.className).filter(Boolean).join(" "), S.ref = be, Wn(Xe, S);
    }(y, X, V, u);
  };
  return P.displayName = v, (y = E.forwardRef(P)).attrs = O, y.componentStyle = A, y.displayName = v, y.shouldForwardProp = w, y.foldedComponentIds = n ? Array.prototype.concat(e.foldedComponentIds, e.styledComponentId) : bt, y.styledComponentId = b, y.target = n ? e.target : e, y.withComponent = function(X) {
    var V = t.componentId, q = function(re, de) {
      if (re == null)
        return {};
      var B, ue, pe = {}, me = Object.keys(re);
      for (ue = 0; ue < me.length; ue++)
        B = me[ue], de.indexOf(B) >= 0 || (pe[B] = re[B]);
      return pe;
    }(t, ["componentId"]), f = V && V + "-" + (pt(X) ? X : Wt(kt(X)));
    return Ir(X, ze({}, q, { attrs: O, componentId: f }), r);
  }, Object.defineProperty(y, "defaultProps", { get: function() {
    return this._foldedDefaultProps;
  }, set: function(X) {
    this._foldedDefaultProps = n ? Kr({}, e.defaultProps, X) : X;
  } }), process.env.NODE_ENV !== "production" && (Ur(v, b), y.warnTooManyClasses = function(X, V) {
    var q = {}, f = !1;
    return function(re) {
      if (!f && (q[re] = !0, Object.keys(q).length >= 200)) {
        var de = V ? ' with the id of "' + V + '"' : "";
        console.warn("Over 200 classes were generated for component " + X + de + `.
Consider using the attrs method, together with a style object for frequently changed styles.
Example:
  const Component = styled.div.attrs(props => ({
    style: {
      background: props.background,
    },
  }))\`width: 100%;\`

  <Component />`), f = !0, q = {};
      }
    };
  }(v, b)), y.toString = function() {
    return "." + y.styledComponentId;
  }, i && $n(y, e, { attrs: !0, componentStyle: !0, displayName: !0, foldedComponentIds: !0, shouldForwardProp: !0, styledComponentId: !0, target: !0, withComponent: !0 }), y;
}
var Ct = function(e) {
  return function t(r, n, i) {
    if (i === void 0 && (i = je), !lt.isValidElementType(n))
      return De(1, String(n));
    var s = function() {
      return r(n, i, I.apply(void 0, arguments));
    };
    return s.withConfig = function(a) {
      return t(r, n, ze({}, i, {}, a));
    }, s.attrs = function(a) {
      return t(r, n, ze({}, i, { attrs: Array.prototype.concat(i.attrs, a).filter(Boolean) }));
    }, s;
  }(Ir, e);
};
["a", "abbr", "address", "area", "article", "aside", "audio", "b", "base", "bdi", "bdo", "big", "blockquote", "body", "br", "button", "canvas", "caption", "cite", "code", "col", "colgroup", "data", "datalist", "dd", "del", "details", "dfn", "dialog", "div", "dl", "dt", "em", "embed", "fieldset", "figcaption", "figure", "footer", "form", "h1", "h2", "h3", "h4", "h5", "h6", "head", "header", "hgroup", "hr", "html", "i", "iframe", "img", "input", "ins", "kbd", "keygen", "label", "legend", "li", "link", "main", "map", "mark", "marquee", "menu", "menuitem", "meta", "meter", "nav", "noscript", "object", "ol", "optgroup", "option", "output", "p", "param", "picture", "pre", "progress", "q", "rp", "rt", "ruby", "s", "samp", "script", "section", "select", "small", "source", "span", "strong", "style", "sub", "summary", "sup", "table", "tbody", "td", "textarea", "tfoot", "th", "thead", "time", "title", "tr", "track", "u", "ul", "var", "video", "wbr", "circle", "clipPath", "defs", "ellipse", "foreignObject", "g", "image", "line", "linearGradient", "marker", "mask", "path", "pattern", "polygon", "polyline", "radialGradient", "rect", "stop", "svg", "text", "textPath", "tspan"].forEach(function(e) {
  Ct[e] = Ct(e);
});
var xo = function() {
  function e(r, n) {
    this.rules = r, this.componentId = n, this.isStatic = Yr(r), wt.registerId(this.componentId + 1);
  }
  var t = e.prototype;
  return t.createStyles = function(r, n, i, s) {
    var a = s(Ue(this.rules, n, i, s).join(""), ""), l = this.componentId + r;
    i.insertRules(l, l, a);
  }, t.removeStyles = function(r, n) {
    n.clearRules(this.componentId + r);
  }, t.renderStyles = function(r, n, i, s) {
    r > 2 && wt.registerId(this.componentId + r), this.removeStyles(r, i), this.createStyles(r, n, i, s);
  }, e;
}();
function Fo(e) {
  for (var t = arguments.length, r = new Array(t > 1 ? t - 1 : 0), n = 1; n < t; n++)
    r[n - 1] = arguments[n];
  var i = I.apply(void 0, [e].concat(r)), s = "sc-global-" + Jr(JSON.stringify(i)), a = new xo(i, s);
  function l(p) {
    var v = Cr(), b = Hr(), O = Re(ut), w = Me(v.allocateGSInstance(s)).current;
    return process.env.NODE_ENV !== "production" && E.Children.count(p.children) && console.warn("The global style component " + s + " was given child JSX. createGlobalStyle does not render children."), process.env.NODE_ENV !== "production" && i.some(function(y) {
      return typeof y == "string" && y.indexOf("@import") !== -1;
    }) && console.warn("Please do not use @import CSS syntax in createGlobalStyle at this time, as the CSSOM APIs we use in production do not handle it well. Instead, we recommend using a library such as react-helmet to inject a typical <link> meta tag to the stylesheet, or simply embedding it manually in your index.html <head> section for a simpler app."), v.server && d(w, p, v, O, b), Fr(function() {
      if (!v.server)
        return d(w, p, v, O, b), function() {
          return a.removeStyles(w, v);
        };
    }, [w, p, v, O, b]), null;
  }
  function d(p, v, b, O, w) {
    if (a.isStatic)
      a.renderStyles(p, to, b, w);
    else {
      var y = ze({}, v, { theme: Gr(v, O, l.defaultProps) });
      a.renderStyles(p, y, b, w);
    }
  }
  return process.env.NODE_ENV !== "production" && Ur(s), E.memo(l);
}
process.env.NODE_ENV !== "production" && typeof navigator < "u" && navigator.product === "ReactNative" && console.warn(`It looks like you've imported 'styled-components' on React Native.
Perhaps you're looking to import 'styled-components/native'?
Read more about this at https://www.styled-components.com/docs/basics#react-native`), process.env.NODE_ENV !== "production" && process.env.NODE_ENV !== "test" && typeof window < "u" && (window["__styled-components-init__"] = window["__styled-components-init__"] || 0, window["__styled-components-init__"] === 1 && console.warn(`It looks like there are several instances of 'styled-components' initialized in this application. This may cause dynamic styles to not render properly, errors during the rehydration process, a missing theme prop, and makes your application bigger without good reason.

See https://s-c.sh/2BAXzed for more info.`), window["__styled-components-init__"] += 1);
const Pe = Ct, Mo = Pe.hr`
	width: 100%;

	${({ theme: { colors: e }, width: t }) => I`
			width: ${t} || 100%;
			border: 0.5px solid ${e.color5};
		`}
`, Ba = ({ width: e = "", "data-testid": t = "break" }) => /* @__PURE__ */ le(
  Mo,
  {
    width: e,
    "data-testid": t
  }
), Do = Pe.div`
	text-align: center;
	display: flex;

	> div {
		width: 10px;
		height: 10px;
		background-color: ${({ theme: e }) => e.colors.main};
		border-radius: 100%;
		display: inline-block;
		animation: animation-loader-button 1.4s infinite ease-in-out both;
	}

	.load1 {
		animation-delay: -0.32s;
	}

	.load2 {
		animation-delay: -0.16s;
	}

	@keyframes animation-loader-button {
		0%,
		80%,
		100% {
			-webkit-transform: scale(0);
			transform: scale(0);
		}
		40% {
			-webkit-transform: scale(1);
			transform: scale(1);
		}
	}
`, Zo = Pe.div`
	display: inline-block;
	width: 100%;
	height: 100vh;
	display: flex;
	justify-content: center;
	align-items: center;

	&:after {
		content: ' ';
		display: block;
		width: 64px;
		height: 64px;
		border-radius: 50%;
		border: ${({ theme: e }) => `6px solid ${e.colors.secondary}`};
		border-color: ${({ theme: e }) => `${e.colors.secondary} transparent ${e.colors.secondary} transparent`};
		animation: animation-loader 1.2s linear infinite;
	}

	@keyframes animation-loader {
		0% {
			transform: rotate(0deg);
		}
		100% {
			transform: rotate(360deg);
		}
	}
`, Qr = ({
  type: e = "button",
  "data-testid": t = "loader"
}) => e === "button" ? /* @__PURE__ */ ke(Do, { "data-testid": t, children: [
  /* @__PURE__ */ le("div", { className: "load1" }),
  /* @__PURE__ */ le("div", { className: "load2" }),
  /* @__PURE__ */ le("div", { className: "load3" })
] }) : /* @__PURE__ */ le(Zo, { "data-testid": t });
Qr.displayName = "Loader";
const Ro = ({ colors: e }, t = "primary") => ({
  primary: I`
			color: ${e.main};
			background: ${e.secondary};

			&:hover {
				background: ${e.color1};
			}

			&:disabled,
			&[disabled] {
				background: ${e.color5};
				color: ${e.main};
				cursor: not-allowed;
			}
		`,
  secondary: I`
			color: ${e.secondary};
			background: ${e.main};
			border: 2px solid ${e.secondary};

			&:hover {
				background: ${e.secondary};
				color: ${e.main};
			}
		`,
  alert: I`
			color: ${e.secondary};
			background: ${e.error};

			&:hover {
				background: ${e.color5};
			}
		`
})[t], ko = (e = "center") => ({
  center: I`
			justify-content: center;
		`,
  left: I`
			justify-content: flex-start;
		`
})[e], Yo = Pe.button`
	cursor: pointer;
	min-width: fit-content;
	min-height: 44px;
	max-height: 44px;
	border: 0;
	border-radius: 15px;
	display: flex;
	align-items: center;
	gap: 8px;
	transition: 0.2s;

	${({ theme: { spaces: e, font: t }, hasOnlyIcon: r, hasFullWidth: n, isDropdown: i }) => I`
			width: ${n && "100%"};
			font-weight: ${t.weights.bold};
			padding: ${r ? `${e.tiny} ${e.small}` : `${e.tiny} ${e.big}`};
			border-radius: ${i && "15px 15px 0 0"};
		`}

	${({ theme: e, variant: t }) => Ro(e, t)}
	${({ align: e }) => ko(e)}
`, Ht = ({
  variant: e = "primary",
  type: t = "button",
  children: r,
  action: n,
  hasFullWidth: i = !1,
  hasOnlyIcon: s = !1,
  align: a = "center",
  isLoading: l = !1,
  isDisabled: d = !1,
  isDropdown: p = !1,
  "data-testid": v = "button"
}) => /* @__PURE__ */ le(
  Yo,
  {
    variant: e,
    type: t,
    onClick: n,
    hasFullWidth: i,
    hasOnlyIcon: s,
    align: a,
    disabled: d,
    isDropdown: p,
    "data-testid": v,
    children: l ? /* @__PURE__ */ le(Qr, { "data-testid": "button-loader" }) : r
  }
);
Ht.displayName = "Button";
const jo = ({ font: e, colors: t }, r = "h1") => ({
  h1: I`
			font-weight: ${e.weights.medium};
			font-size: ${e.sizes.h1};
			line-height: ${e.lineHeight.h1};
			letter-spacing: ${e.letterSpacing.h1};
			color: ${t.color1};
			text-transform: uppercase;
		`,
  h2: I`
			font-weight: ${e.weights.regular};
			font-size: ${e.sizes.h2};
			line-height: ${e.lineHeight.h2};
			letter-spacing: ${e.letterSpacing.h2};
			color: ${t.color2};
		`,
  h3: I`
			font-weight: ${e.weights.regular};
			font-size: ${e.sizes.h1};
			line-height: ${e.lineHeight.h2};
			letter-spacing: ${e.letterSpacing.h2};
			color: ${t.color2};
		`
})[r], Bo = Pe.h1`
	text-align: ${({ align: e }) => e};

	${({ theme: e, level: t }) => jo(e, `h${t}`)};
`, _r = ({
  level: e = 1,
  children: t,
  align: r = "center",
  "data-testid": n = "heading"
}) => /* @__PURE__ */ le(
  Bo,
  {
    as: `h${e}`,
    level: e,
    align: r,
    "data-testid": n,
    children: t
  }
);
_r.displayName = "Heading";
const Co = ({ colors: e }, t = "avatar") => ({
  avatar: I`
			cursor: pointer;
			width: 120px;
			height: 120px;
			background: ${e.main};
			border: 2px solid ${e.secondary};
			border-radius: 100%;
		`
})[t], Ho = Pe.img`
	${({ onClick: e }) => I`
			cursor: ${e && "pointer"};
		`}

	${({ theme: e, variant: t }) => Co(e, t)}
`, Uo = ({
  src: e,
  width: t,
  height: r,
  alt: n,
  variant: i = "default",
  onClick: s,
  "data-testid": a = "image"
}) => /* @__PURE__ */ le(
  Ho,
  {
    src: e,
    width: t,
    height: r,
    alt: n,
    variant: i,
    onClick: s,
    "data-testid": a
  }
);
Uo.displayName = "Image";
const Go = Pe.div`
	${({ theme: { breakpoints: e } }) => I`
			width: 100%;
			display: flex;
			justify-content: center;
			align-items: flex-start;
			overflow-y: auto;

			@media (max-width: ${e.mobile}px) {
				width: 100%;
				height: calc(100vh - 112px);
				overflow: none;
			}
		`}
`, Jo = Pe.div`
	${({ theme: { spaces: e, colors: t, breakpoints: r }, image: n }) => I`
			width: calc(100% - 600px);
			height: calc(100vh - 84px);
			padding: calc(${e.huge} + ${e.huge}) ${e.huge};
			background: ${t.main};
			background-image: url(${n});
			background-position: center;
			background-repeat: no-repeat;
			background-size: cover;
			opacity: 0.5;
			display: ${!n && "none"};

			@media (max-width: ${r.mobile}px) {
				display: none;
			}

			@media (max-width: ${r.tablet}px) {
				display: none;
			}
		`}
`, Ko = Pe.div`
	${({ theme: { spaces: e, breakpoints: t } }) => I`
			width: 400px;
			margin: 0 auto;
			padding: ${e.huge} ${e.big};
			display: flex;
			flex-direction: column;
			align-items: center;

			@media (max-width: ${t.mobile}px) {
				width: 100%;
				height: calc(100vh - 172px);
			}
		`}
`, Io = ({
  children: e,
  image: t,
  "data-testid": r = "layout"
}) => /* @__PURE__ */ ke(Go, { "data-testid": r, children: [
  /* @__PURE__ */ le(Jo, { image: t }),
  /* @__PURE__ */ le(Ko, { children: e })
] });
Io.displayName = "Layout";
const Qo = ({ colors: e }, t = "label") => ({
  default: I`
			color: ${e.secondary};
		`,
  label: I`
			color: ${e.color5};
		`,
  error: I`
			color: ${e.error};
		`
})[t], _o = ({ font: e }, t = "medium") => ({
  small: I`
			font-size: ${e.sizes.error};
		`,
  medium: I`
			font-size: ${e.sizes.default};
		`,
  big: I`
			font-size: ${e.sizes.info};
		`,
  large: I`
			font-size: ${e.sizes.icon};
		`,
  huge: I`
			font-size: ${e.sizes.notFound};
		`
})[t], $o = Pe.p`
	font-size: ${({ size: e }) => e};
	text-align: ${({ align: e }) => e};
	min-height: 24px;
	display: flex;
	align-items: center;
	gap: 5px;

	${({ theme: e, type: t }) => Qo(e, t)};
	${({ theme: e, size: t }) => _o(e, t)};
`, $r = ({
  type: e = "default",
  children: t,
  size: r = "medium",
  align: n = "center",
  "data-testid": i = "paragraph"
}) => /* @__PURE__ */ le(
  $o,
  {
    type: e,
    size: r,
    align: n,
    "data-testid": i,
    children: t
  }
);
$r.displayName = "Paragraph";
const ea = Pe.div`
	${({ theme: { spaces: e }, type: t, space: r = "medium" }) => I`
			width: ${t === "horizontal" && e[r]};
			height: ${t === "vertical" && e[r]};
		`}
`, At = ({
  type: e = "horizontal",
  space: t = "medium",
  "data-testid": r = "spacer"
}) => /* @__PURE__ */ le(
  ea,
  {
    type: e,
    space: t,
    "data-testid": r
  }
);
At.displayName = "Spacer";
var ge = globalThis && globalThis.__assign || function() {
  return ge = Object.assign || function(e) {
    for (var t, r = 1, n = arguments.length; r < n; r++) {
      t = arguments[r];
      for (var i in t)
        Object.prototype.hasOwnProperty.call(t, i) && (e[i] = t[i]);
    }
    return e;
  }, ge.apply(this, arguments);
}, ta = globalThis && globalThis.__rest || function(e, t) {
  var r = {};
  for (var n in e)
    Object.prototype.hasOwnProperty.call(e, n) && t.indexOf(n) < 0 && (r[n] = e[n]);
  if (e != null && typeof Object.getOwnPropertySymbols == "function")
    for (var i = 0, n = Object.getOwnPropertySymbols(e); i < n.length; i++)
      t.indexOf(n[i]) < 0 && Object.prototype.propertyIsEnumerable.call(e, n[i]) && (r[n[i]] = e[n[i]]);
  return r;
}, Et = Symbol("NiceModalId"), Qt = {}, rt = E.createContext(Qt), en = E.createContext(null), Ze = {}, ct = {}, ra = 0, nt = function() {
  throw new Error("No dispatch method detected, did you embed your app with NiceModal.Provider?");
}, na = function() {
  return "_nice_modal_" + ra++;
}, tn = function(e, t) {
  var r, n, i;
  switch (e === void 0 && (e = Qt), t.type) {
    case "nice-modal/show": {
      var s = t.payload, a = s.modalId, l = s.args;
      return ge(ge({}, e), (r = {}, r[a] = ge(ge({}, e[a]), {
        id: a,
        args: l,
        // If modal is not mounted, mount it first then make it visible.
        // There is logic inside HOC wrapper to make it visible after its first mount.
        // This mechanism ensures the entering transition.
        visible: !!ct[a],
        delayVisible: !ct[a]
      }), r));
    }
    case "nice-modal/hide": {
      var a = t.payload.modalId;
      return e[a] ? ge(ge({}, e), (n = {}, n[a] = ge(ge({}, e[a]), { visible: !1 }), n)) : e;
    }
    case "nice-modal/remove": {
      var a = t.payload.modalId, d = ge({}, e);
      return delete d[a], d;
    }
    case "nice-modal/set-flags": {
      var p = t.payload, a = p.modalId, v = p.flags;
      return ge(ge({}, e), (i = {}, i[a] = ge(ge({}, e[a]), v), i));
    }
    default:
      return e;
  }
};
function oa(e, t) {
  return {
    type: "nice-modal/show",
    payload: {
      modalId: e,
      args: t
    }
  };
}
function aa(e, t) {
  return {
    type: "nice-modal/set-flags",
    payload: {
      modalId: e,
      flags: t
    }
  };
}
function ia(e) {
  return {
    type: "nice-modal/hide",
    payload: {
      modalId: e
    }
  };
}
function sa(e) {
  return {
    type: "nice-modal/remove",
    payload: {
      modalId: e
    }
  };
}
var Fe = {}, _e = {}, _t = function(e) {
  return typeof e == "string" ? e : (e[Et] || (e[Et] = na()), e[Et]);
};
function rn(e, t) {
  var r = _t(e);
  if (typeof e != "string" && !Ze[r] && Xt(r, e), nt(oa(r, t)), !Fe[r]) {
    var n, i, s = new Promise(function(a, l) {
      n = a, i = l;
    });
    Fe[r] = {
      resolve: n,
      reject: i,
      promise: s
    };
  }
  return Fe[r].promise;
}
function nn(e) {
  var t = _t(e);
  if (nt(ia(t)), delete Fe[t], !_e[t]) {
    var r, n, i = new Promise(function(s, a) {
      r = s, n = a;
    });
    _e[t] = {
      resolve: r,
      reject: n,
      promise: i
    };
  }
  return _e[t].promise;
}
var on = function(e) {
  nt(sa(e)), delete Fe[e], delete _e[e];
}, la = function(e, t) {
  nt(aa(e, t));
};
function $t(e, t) {
  var r = Re(rt), n = Re(en), i = null, s = e && typeof e != "string";
  if (e ? i = _t(e) : i = n, !i)
    throw new Error("No modal id found in NiceModal.useModal.");
  var a = i;
  Oe(function() {
    s && !Ze[a] && Xt(a, e, t);
  }, [s, a, e, t]);
  var l = r[a], d = Qe(function(y) {
    return rn(a, y);
  }, [a]), p = Qe(function() {
    return nn(a);
  }, [a]), v = Qe(function() {
    return on(a);
  }, [a]), b = Qe(function(y) {
    var A;
    (A = Fe[a]) === null || A === void 0 || A.resolve(y), delete Fe[a];
  }, [a]), O = Qe(function(y) {
    var A;
    (A = Fe[a]) === null || A === void 0 || A.reject(y), delete Fe[a];
  }, [a]), w = Qe(function(y) {
    var A;
    (A = _e[a]) === null || A === void 0 || A.resolve(y), delete _e[a];
  }, [a]);
  return {
    id: a,
    args: l == null ? void 0 : l.args,
    visible: !!(l != null && l.visible),
    keepMounted: !!(l != null && l.keepMounted),
    show: d,
    hide: p,
    remove: v,
    resolve: b,
    reject: O,
    resolveHide: w
  };
}
var ua = function(e) {
  return function(t) {
    var r, n = t.defaultVisible, i = t.keepMounted, s = t.id, a = ta(t, ["defaultVisible", "keepMounted", "id"]), l = $t(s), d = l.args, p = l.show, v = Re(rt), b = !!v[s];
    Oe(function() {
      return n && p(), ct[s] = !0, function() {
        delete ct[s];
      };
    }, [s, p, n]), Oe(function() {
      i && la(s, { keepMounted: !0 });
    }, [s, i]);
    var O = (r = v[s]) === null || r === void 0 ? void 0 : r.delayVisible;
    return Oe(function() {
      O && p(d);
    }, [O, d, p]), b ? E.createElement(
      en.Provider,
      { value: s },
      E.createElement(e, ge({}, a, d))
    ) : null;
  };
}, Xt = function(e, t, r) {
  Ze[e] ? Ze[e].props = r : Ze[e] = { comp: t, props: r };
}, ca = function(e) {
  delete Ze[e];
}, an = function() {
  var e = Re(rt), t = Object.keys(e).filter(function(n) {
    return !!e[n];
  });
  t.forEach(function(n) {
    if (!Ze[n] && !ct[n]) {
      console.warn("No modal found for id: " + n + ". Please check the id or if it is registered or declared via JSX.");
      return;
    }
  });
  var r = t.filter(function(n) {
    return Ze[n];
  }).map(function(n) {
    return ge({ id: n }, Ze[n]);
  });
  return E.createElement(E.Fragment, null, r.map(function(n) {
    return E.createElement(n.comp, ge({ key: n.id, id: n.id }, n.props));
  }));
}, fa = function(e) {
  var t = e.children, r = Mr(tn, Qt), n = r[0];
  return nt = r[1], E.createElement(
    rt.Provider,
    { value: n },
    t,
    E.createElement(an, null)
  );
}, da = function(e) {
  var t = e.children, r = e.dispatch, n = e.modals;
  return !r || !n ? E.createElement(fa, null, t) : (nt = r, E.createElement(
    rt.Provider,
    { value: n },
    t,
    E.createElement(an, null)
  ));
}, pa = function(e) {
  var t = e.id, r = e.component;
  return Oe(function() {
    return Xt(t, r), function() {
      ca(t);
    };
  }, [t, r]), null;
}, va = function(e) {
  return {
    visible: e.visible,
    onOk: function() {
      return e.hide();
    },
    onCancel: function() {
      return e.hide();
    },
    afterClose: function() {
      e.resolveHide(), e.keepMounted || e.remove();
    }
  };
}, ma = function(e) {
  return {
    visible: e.visible,
    onClose: function() {
      return e.hide();
    },
    afterVisibleChange: function(t) {
      t || e.resolveHide(), !t && !e.keepMounted && e.remove();
    }
  };
}, Aa = function(e) {
  return {
    open: e.visible,
    onClose: function() {
      return e.hide();
    },
    onExited: function() {
      e.resolveHide(), !e.keepMounted && e.remove();
    }
  };
}, ha = function(e) {
  return {
    show: e.visible,
    onHide: function() {
      return e.hide();
    },
    onExited: function() {
      e.resolveHide(), !e.keepMounted && e.remove();
    }
  };
}, er = {
  Provider: da,
  ModalDef: pa,
  NiceModalContext: rt,
  create: ua,
  register: Xt,
  show: rn,
  hide: nn,
  remove: on,
  useModal: $t,
  reducer: tn,
  antdModal: va,
  antdDrawer: ma,
  muiDialog: Aa,
  bootstrapDialog: ha
};
function sn(e) {
  var t, r, n = "";
  if (typeof e == "string" || typeof e == "number")
    n += e;
  else if (typeof e == "object")
    if (Array.isArray(e))
      for (t = 0; t < e.length; t++)
        e[t] && (r = sn(e[t])) && (n && (n += " "), n += r);
    else
      for (t in e)
        e[t] && (n && (n += " "), n += t);
  return n;
}
function Ye() {
  for (var e, t, r = 0, n = ""; r < arguments.length; )
    (e = arguments[r++]) && (t = sn(e)) && (n && (n += " "), n += t);
  return n;
}
const ht = (e) => typeof e == "number" && !isNaN(e), Pt = (e) => typeof e == "string", Ne = (e) => typeof e == "function", yt = (e) => Pt(e) || Ne(e) ? e : null, Lt = (e) => $e(e) || Pt(e) || Ne(e) || ht(e);
function ya(e, t, r) {
  r === void 0 && (r = 300);
  const { scrollHeight: n, style: i } = e;
  requestAnimationFrame(() => {
    i.minHeight = "initial", i.height = n + "px", i.transition = `all ${r}ms`, requestAnimationFrame(() => {
      i.height = "0", i.padding = "0", i.margin = "0", setTimeout(t, r);
    });
  });
}
function qt(e) {
  let { enter: t, exit: r, appendPosition: n = !1, collapse: i = !0, collapseDuration: s = 300 } = e;
  return function(a) {
    let { children: l, position: d, preventExitTransition: p, done: v, nodeRef: b, isIn: O } = a;
    const w = n ? `${t}--${d}` : t, y = n ? `${r}--${d}` : r, A = Me(0);
    return Fr(() => {
      const u = b.current, P = w.split(" "), X = (V) => {
        V.target === b.current && (u.dispatchEvent(new Event("d")), u.removeEventListener("animationend", X), u.removeEventListener("animationcancel", X), A.current === 0 && V.type !== "animationcancel" && u.classList.remove(...P));
      };
      u.classList.add(...P), u.addEventListener("animationend", X), u.addEventListener("animationcancel", X);
    }, []), Oe(() => {
      const u = b.current, P = () => {
        u.removeEventListener("animationend", P), i ? ya(u, v, s) : v();
      };
      O || (p ? P() : (A.current = 1, u.className += ` ${y}`, u.addEventListener("animationend", P)));
    }, [O]), E.createElement(E.Fragment, null, l);
  };
}
function Or(e, t) {
  return { content: e.content, containerId: e.props.containerId, id: e.props.toastId, theme: e.props.theme, type: e.props.type, data: e.props.data || {}, isLoading: e.props.isLoading, icon: e.props.icon, status: t };
}
const He = { list: /* @__PURE__ */ new Map(), emitQueue: /* @__PURE__ */ new Map(), on(e, t) {
  return this.list.has(e) || this.list.set(e, []), this.list.get(e).push(t), this;
}, off(e, t) {
  if (t) {
    const r = this.list.get(e).filter((n) => n !== t);
    return this.list.set(e, r), this;
  }
  return this.list.delete(e), this;
}, cancelEmit(e) {
  const t = this.emitQueue.get(e);
  return t && (t.forEach(clearTimeout), this.emitQueue.delete(e)), this;
}, emit(e) {
  this.list.has(e) && this.list.get(e).forEach((t) => {
    const r = setTimeout(() => {
      t(...[].slice.call(arguments, 1));
    }, 0);
    this.emitQueue.has(e) || this.emitQueue.set(e, []), this.emitQueue.get(e).push(r);
  });
} }, vt = (e) => {
  let { theme: t, type: r, ...n } = e;
  return E.createElement("svg", { viewBox: "0 0 24 24", width: "100%", height: "100%", fill: t === "colored" ? "currentColor" : `var(--toastify-icon-color-${r})`, ...n });
}, xt = { info: function(e) {
  return E.createElement(vt, { ...e }, E.createElement("path", { d: "M12 0a12 12 0 1012 12A12.013 12.013 0 0012 0zm.25 5a1.5 1.5 0 11-1.5 1.5 1.5 1.5 0 011.5-1.5zm2.25 13.5h-4a1 1 0 010-2h.75a.25.25 0 00.25-.25v-4.5a.25.25 0 00-.25-.25h-.75a1 1 0 010-2h1a2 2 0 012 2v4.75a.25.25 0 00.25.25h.75a1 1 0 110 2z" }));
}, warning: function(e) {
  return E.createElement(vt, { ...e }, E.createElement("path", { d: "M23.32 17.191L15.438 2.184C14.728.833 13.416 0 11.996 0c-1.42 0-2.733.833-3.443 2.184L.533 17.448a4.744 4.744 0 000 4.368C1.243 23.167 2.555 24 3.975 24h16.05C22.22 24 24 22.044 24 19.632c0-.904-.251-1.746-.68-2.44zm-9.622 1.46c0 1.033-.724 1.823-1.698 1.823s-1.698-.79-1.698-1.822v-.043c0-1.028.724-1.822 1.698-1.822s1.698.79 1.698 1.822v.043zm.039-12.285l-.84 8.06c-.057.581-.408.943-.897.943-.49 0-.84-.367-.896-.942l-.84-8.065c-.057-.624.25-1.095.779-1.095h1.91c.528.005.84.476.784 1.1z" }));
}, success: function(e) {
  return E.createElement(vt, { ...e }, E.createElement("path", { d: "M12 0a12 12 0 1012 12A12.014 12.014 0 0012 0zm6.927 8.2l-6.845 9.289a1.011 1.011 0 01-1.43.188l-4.888-3.908a1 1 0 111.25-1.562l4.076 3.261 6.227-8.451a1 1 0 111.61 1.183z" }));
}, error: function(e) {
  return E.createElement(vt, { ...e }, E.createElement("path", { d: "M11.983 0a12.206 12.206 0 00-8.51 3.653A11.8 11.8 0 000 12.207 11.779 11.779 0 0011.8 24h.214A12.111 12.111 0 0024 11.791 11.766 11.766 0 0011.983 0zM10.5 16.542a1.476 1.476 0 011.449-1.53h.027a1.527 1.527 0 011.523 1.47 1.475 1.475 0 01-1.449 1.53h-.027a1.529 1.529 0 01-1.523-1.47zM11 12.5v-6a1 1 0 012 0v6a1 1 0 11-2 0z" }));
}, spinner: function() {
  return E.createElement("div", { className: "Toastify__spinner" });
} };
function ba(e) {
  const [, t] = Mr((w) => w + 1, 0), [r, n] = Mt([]), i = Me(null), s = Me(/* @__PURE__ */ new Map()).current, a = (w) => r.indexOf(w) !== -1, l = Me({ toastKey: 1, displayedToast: 0, count: 0, queue: [], props: e, containerId: null, isToastActive: a, getToast: (w) => s.get(w) }).current;
  function d(w) {
    let { containerId: y } = w;
    const { limit: A } = l.props;
    !A || y && l.containerId !== y || (l.count -= l.queue.length, l.queue = []);
  }
  function p(w) {
    n((y) => w == null ? [] : y.filter((A) => A !== w));
  }
  function v() {
    const { toastContent: w, toastProps: y, staleId: A } = l.queue.shift();
    O(w, y, A);
  }
  function b(w, y) {
    let { delay: A, staleId: u, ...P } = y;
    if (!Lt(w) || function(J) {
      return !i.current || l.props.enableMultiContainer && J.containerId !== l.props.containerId || s.has(J.toastId) && J.updateId == null;
    }(P))
      return;
    const { toastId: X, updateId: V, data: q } = P, { props: f } = l, re = () => p(X), de = V == null;
    de && l.count++;
    const B = { ...f, style: f.toastStyle, key: l.toastKey++, ...P, toastId: X, updateId: V, data: q, closeToast: re, isIn: !1, className: yt(P.className || f.toastClassName), bodyClassName: yt(P.bodyClassName || f.bodyClassName), progressClassName: yt(P.progressClassName || f.progressClassName), autoClose: !P.isLoading && (ue = P.autoClose, pe = f.autoClose, ue === !1 || ht(ue) && ue > 0 ? ue : pe), deleteToast() {
      const J = Or(s.get(X), "removed");
      s.delete(X), He.emit(4, J);
      const C = l.queue.length;
      if (l.count = X == null ? l.count - l.displayedToast : l.count - 1, l.count < 0 && (l.count = 0), C > 0) {
        const H = X == null ? l.props.limit : 1;
        if (C === 1 || H === 1)
          l.displayedToast++, v();
        else {
          const G = H > C ? C : H;
          l.displayedToast = G;
          for (let ie = 0; ie < G; ie++)
            v();
        }
      } else
        t();
    } };
    var ue, pe;
    B.iconOut = function(J) {
      let { theme: C, type: H, isLoading: G, icon: ie } = J, oe = null;
      const ne = { theme: C, type: H };
      return ie === !1 || (Ne(ie) ? oe = ie(ne) : $e(ie) ? oe = Dt(ie, ne) : Pt(ie) || ht(ie) ? oe = ie : G ? oe = xt.spinner() : ((be) => be in xt)(H) && (oe = xt[H](ne))), oe;
    }(B), Ne(P.onOpen) && (B.onOpen = P.onOpen), Ne(P.onClose) && (B.onClose = P.onClose), B.closeButton = f.closeButton, P.closeButton === !1 || Lt(P.closeButton) ? B.closeButton = P.closeButton : P.closeButton === !0 && (B.closeButton = !Lt(f.closeButton) || f.closeButton);
    let me = w;
    $e(w) && !Pt(w.type) ? me = Dt(w, { closeToast: re, toastProps: B, data: q }) : Ne(w) && (me = w({ closeToast: re, toastProps: B, data: q })), f.limit && f.limit > 0 && l.count > f.limit && de ? l.queue.push({ toastContent: me, toastProps: B, staleId: u }) : ht(A) ? setTimeout(() => {
      O(me, B, u);
    }, A) : O(me, B, u);
  }
  function O(w, y, A) {
    const { toastId: u } = y;
    A && s.delete(A);
    const P = { content: w, props: y };
    s.set(u, P), n((X) => [...X, u].filter((V) => V !== A)), He.emit(4, Or(P, P.props.updateId == null ? "added" : "updated"));
  }
  return Oe(() => (l.containerId = e.containerId, He.cancelEmit(3).on(0, b).on(1, (w) => i.current && p(w)).on(5, d).emit(2, l), () => {
    s.clear(), He.emit(3, l);
  }), []), Oe(() => {
    l.props = e, l.isToastActive = a, l.displayedToast = r.length;
  }), { getToastToRender: function(w) {
    const y = /* @__PURE__ */ new Map(), A = Array.from(s.values());
    return e.newestOnTop && A.reverse(), A.forEach((u) => {
      const { position: P } = u.props;
      y.has(P) || y.set(P, []), y.get(P).push(u);
    }), Array.from(y, (u) => w(u[0], u[1]));
  }, containerRef: i, isToastActive: a };
}
function zr(e) {
  return e.targetTouches && e.targetTouches.length >= 1 ? e.targetTouches[0].clientX : e.clientX;
}
function Wr(e) {
  return e.targetTouches && e.targetTouches.length >= 1 ? e.targetTouches[0].clientY : e.clientY;
}
function ga(e) {
  const [t, r] = Mt(!1), [n, i] = Mt(!1), s = Me(null), a = Me({ start: 0, x: 0, y: 0, delta: 0, removalDistance: 0, canCloseOnClick: !0, canDrag: !1, boundingRect: null, didMove: !1 }).current, l = Me(e), { autoClose: d, pauseOnHover: p, closeToast: v, onClick: b, closeOnClick: O } = e;
  function w(q) {
    if (e.draggable) {
      q.nativeEvent.type === "touchstart" && q.nativeEvent.preventDefault(), a.didMove = !1, document.addEventListener("mousemove", P), document.addEventListener("mouseup", X), document.addEventListener("touchmove", P), document.addEventListener("touchend", X);
      const f = s.current;
      a.canCloseOnClick = !0, a.canDrag = !0, a.boundingRect = f.getBoundingClientRect(), f.style.transition = "", a.x = zr(q.nativeEvent), a.y = Wr(q.nativeEvent), e.draggableDirection === "x" ? (a.start = a.x, a.removalDistance = f.offsetWidth * (e.draggablePercent / 100)) : (a.start = a.y, a.removalDistance = f.offsetHeight * (e.draggablePercent === 80 ? 1.5 * e.draggablePercent : e.draggablePercent / 100));
    }
  }
  function y(q) {
    if (a.boundingRect) {
      const { top: f, bottom: re, left: de, right: B } = a.boundingRect;
      q.nativeEvent.type !== "touchend" && e.pauseOnHover && a.x >= de && a.x <= B && a.y >= f && a.y <= re ? u() : A();
    }
  }
  function A() {
    r(!0);
  }
  function u() {
    r(!1);
  }
  function P(q) {
    const f = s.current;
    a.canDrag && f && (a.didMove = !0, t && u(), a.x = zr(q), a.y = Wr(q), a.delta = e.draggableDirection === "x" ? a.x - a.start : a.y - a.start, a.start !== a.x && (a.canCloseOnClick = !1), f.style.transform = `translate${e.draggableDirection}(${a.delta}px)`, f.style.opacity = "" + (1 - Math.abs(a.delta / a.removalDistance)));
  }
  function X() {
    document.removeEventListener("mousemove", P), document.removeEventListener("mouseup", X), document.removeEventListener("touchmove", P), document.removeEventListener("touchend", X);
    const q = s.current;
    if (a.canDrag && a.didMove && q) {
      if (a.canDrag = !1, Math.abs(a.delta) > a.removalDistance)
        return i(!0), void e.closeToast();
      q.style.transition = "transform 0.2s, opacity 0.2s", q.style.transform = `translate${e.draggableDirection}(0)`, q.style.opacity = "1";
    }
  }
  Oe(() => {
    l.current = e;
  }), Oe(() => (s.current && s.current.addEventListener("d", A, { once: !0 }), Ne(e.onOpen) && e.onOpen($e(e.children) && e.children.props), () => {
    const q = l.current;
    Ne(q.onClose) && q.onClose($e(q.children) && q.children.props);
  }), []), Oe(() => (e.pauseOnFocusLoss && (document.hasFocus() || u(), window.addEventListener("focus", A), window.addEventListener("blur", u)), () => {
    e.pauseOnFocusLoss && (window.removeEventListener("focus", A), window.removeEventListener("blur", u));
  }), [e.pauseOnFocusLoss]);
  const V = { onMouseDown: w, onTouchStart: w, onMouseUp: y, onTouchEnd: y };
  return d && p && (V.onMouseEnter = u, V.onMouseLeave = A), O && (V.onClick = (q) => {
    b && b(q), a.canCloseOnClick && v();
  }), { playToast: A, pauseToast: u, isRunning: t, preventExitTransition: n, toastRef: s, eventHandlers: V };
}
function ln(e) {
  let { closeToast: t, theme: r, ariaLabel: n = "close" } = e;
  return E.createElement("button", { className: `Toastify__close-button Toastify__close-button--${r}`, type: "button", onClick: (i) => {
    i.stopPropagation(), t(i);
  }, "aria-label": n }, E.createElement("svg", { "aria-hidden": "true", viewBox: "0 0 14 16" }, E.createElement("path", { fillRule: "evenodd", d: "M7.71 8.23l3.75 3.75-1.48 1.48-3.75-3.75-3.75 3.75L1 11.98l3.75-3.75L1 4.48 2.48 3l3.75 3.75L9.98 3l1.48 1.48-3.75 3.75z" })));
}
function wa(e) {
  let { delay: t, isRunning: r, closeToast: n, type: i = "default", hide: s, className: a, style: l, controlledProgress: d, progress: p, rtl: v, isIn: b, theme: O } = e;
  const w = s || d && p === 0, y = { ...l, animationDuration: `${t}ms`, animationPlayState: r ? "running" : "paused", opacity: w ? 0 : 1 };
  d && (y.transform = `scaleX(${p})`);
  const A = Ye("Toastify__progress-bar", d ? "Toastify__progress-bar--controlled" : "Toastify__progress-bar--animated", `Toastify__progress-bar-theme--${O}`, `Toastify__progress-bar--${i}`, { "Toastify__progress-bar--rtl": v }), u = Ne(a) ? a({ rtl: v, type: i, defaultClassName: A }) : Ye(A, a);
  return E.createElement("div", { role: "progressbar", "aria-hidden": w ? "true" : "false", "aria-label": "notification timer", className: u, style: y, [d && p >= 1 ? "onTransitionEnd" : "onAnimationEnd"]: d && p < 1 ? null : () => {
    b && n();
  } });
}
const Pa = (e) => {
  const { isRunning: t, preventExitTransition: r, toastRef: n, eventHandlers: i } = ga(e), { closeButton: s, children: a, autoClose: l, onClick: d, type: p, hideProgressBar: v, closeToast: b, transition: O, position: w, className: y, style: A, bodyClassName: u, bodyStyle: P, progressClassName: X, progressStyle: V, updateId: q, role: f, progress: re, rtl: de, toastId: B, deleteToast: ue, isIn: pe, isLoading: me, iconOut: J, closeOnClick: C, theme: H } = e, G = Ye("Toastify__toast", `Toastify__toast-theme--${H}`, `Toastify__toast--${p}`, { "Toastify__toast--rtl": de }, { "Toastify__toast--close-on-click": C }), ie = Ne(y) ? y({ rtl: de, position: w, type: p, defaultClassName: G }) : Ye(G, y), oe = !!re || !l, ne = { closeToast: b, type: p, theme: H };
  let be = null;
  return s === !1 || (be = Ne(s) ? s(ne) : $e(s) ? Dt(s, ne) : ln(ne)), E.createElement(O, { isIn: pe, done: ue, position: w, preventExitTransition: r, nodeRef: n }, E.createElement("div", { id: B, onClick: d, className: ie, ...i, style: A, ref: n }, E.createElement("div", { ...pe && { role: f }, className: Ne(u) ? u({ type: p }) : Ye("Toastify__toast-body", u), style: P }, J != null && E.createElement("div", { className: Ye("Toastify__toast-icon", { "Toastify--animate-icon Toastify__zoom-enter": !me }) }, J), E.createElement("div", null, a)), be, E.createElement(wa, { ...q && !oe ? { key: `pb-${q}` } : {}, rtl: de, theme: H, delay: l, isRunning: t, isIn: pe, closeToast: b, hide: v, type: p, style: V, className: X, controlledProgress: oe, progress: re || 0 })));
}, Nt = function(e, t) {
  return t === void 0 && (t = !1), { enter: `Toastify--animate Toastify__${e}-enter`, exit: `Toastify--animate Toastify__${e}-exit`, appendPosition: t };
}, Xa = qt(Nt("bounce", !0));
qt(Nt("slide", !0));
qt(Nt("zoom"));
qt(Nt("flip"));
const Ut = Vn((e, t) => {
  const { getToastToRender: r, containerRef: n, isToastActive: i } = ba(e), { className: s, style: a, rtl: l, containerId: d } = e;
  function p(v) {
    const b = Ye("Toastify__toast-container", `Toastify__toast-container--${v}`, { "Toastify__toast-container--rtl": l });
    return Ne(s) ? s({ position: v, rtl: l, defaultClassName: b }) : Ye(b, yt(s));
  }
  return Oe(() => {
    t && (t.current = n.current);
  }, []), E.createElement("div", { ref: n, className: "Toastify", id: d }, r((v, b) => {
    const O = b.length ? { ...a } : { ...a, pointerEvents: "none" };
    return E.createElement("div", { className: p(v), style: O, key: `container-${v}` }, b.map((w, y) => {
      let { content: A, props: u } = w;
      return E.createElement(Pa, { ...u, isIn: i(u.toastId), style: { ...u.style, "--nth": y + 1, "--len": b.length }, key: `toast-${u.key}` }, A);
    }));
  }));
});
Ut.displayName = "ToastContainer", Ut.defaultProps = { position: "top-right", transition: Xa, autoClose: 5e3, closeButton: ln, pauseOnHover: !0, pauseOnFocusLoss: !0, closeOnClick: !0, draggable: !0, draggablePercent: 80, draggableDirection: "x", role: "alert", theme: "light" };
let Vr, Ft = /* @__PURE__ */ new Map(), Er = [];
He.on(2, (e) => {
  Vr = e.containerId || e, Ft.set(Vr, e), Er.forEach((t) => {
    He.emit(0, t.content, t.options);
  }), Er = [];
}).on(3, (e) => {
  Ft.delete(e.containerId || e), Ft.size === 0 && He.off(0).off(1).off(5);
});
const qa = "data:font/woff;base64,", Na = "data:font/woff2;base64,", Ta = "data:font/woff;base64,", Sa = "data:font/woff2;base64,", Oa = "data:font/woff;base64,", za = "data:font/woff2;base64,", Wa = Fo`
	${({ theme: { font: e, colors: t } }) => I`
		@font-face {
			font-family: Poppins;
			src: url(${Sa}) format('woff2'),
				url(${Ta}) format('woff');
			font-weight: 400;
		}
	
		@font-face {
			font-family: Poppins;
			src: url(${Na}) format('woff2'),
				url(${qa}) format('woff');
			font-weight: 500;
		}
	
		@font-face {
			font-family: Poppins;
			src: url(${za}) format('woff2'),
				url(${Oa}) format('woff');
			font-weight: 600;
		}
	
		:root {
			font-family: 'Poppins', sans-serif;
			font-weight: ${e.weights.regular};
	
			--toastify-color-light: ${t.secondary};
			--toastify-color-dark: ${t.secondary};
			--toastify-color-info: ${t.toastInfo};;
			--toastify-color-success: ${t.toastSuccess};
			--toastify-color-warning: ${t.toastWarning};
			--toastify-color-error: ${t.toastError};
			
			--toastify-icon-color-info: var(--toastify-color-info);
			--toastify-icon-color-success: var(--toastify-color-success);
			--toastify-icon-color-warning: var(--toastify-color-warning);
			--toastify-icon-color-error: var(--toastify-color-error);
			
			--toastify-toast-width: 320px;
			--toastify-toast-min-height: 64px;
			--toastify-toast-max-height: 800px;
			--toastify-font-family: 'Poppins', sans-serif;
			--toastify-z-index: 9999;
			
			--toastify-text-color-light: ${t.main};
			--toastify-text-color-dark: ${t.main};
		}
	
		*,
		*::before,
		*::after {
			box-sizing: border-box;
		}
	
		* {
			margin: 0;
		}
	
		main {
			display: block;
		}
	
		a {
			background-color: transparent;
		}
	
		img {
			border-style: none;
		}
	
		html,
		body {
			height: 100vh;
			font-size: ${e.sizes.default}
			line-height: ${e.lineHeight.default};
			color: ${t.secondary};
			background-color: ${({ theme: r }) => r.colors.main};
		}

		body {
			transition: background .5s, color .7s;
		}

		img,
		picture,
		video,
		canvas,
		svg {
			display: block;
			max-width: 100%;
		}

		input,
		button,
		textarea,
		select {
			font: inherit;
		}

		p,
		h1,
		h2,
		h3,
		h4,
		h5,
		h6 {
			overflow-wrap: break-word;
		}

		ul {
			list-style: none;
			padding: 0;
		}
	`}	
`, Lr = {
  MOBILE: 428,
  TABLET: 1200
}, j = {
  Grey100: "#090909",
  Grey100_50: "#09090980",
  Grey200: "#111111",
  Grey300: "#3C3C3C",
  Grey400: "#797979",
  White100: "#EAEAEA",
  White200: "#EEEEEE",
  White300: "#DDDDDD",
  White300_50: "#DDDDDD80",
  Red100: "#F97777",
  Red200: "#F86565",
  Orange100: "#EFA360",
  Orange200: "#F18F5D",
  Yellow100: "#DECC56",
  Yellow200: "#E4CF49",
  Green100: "#D7E66D",
  Green200: "#D6E560",
  Green300: "#8EE296",
  Green400: "#83DF8C",
  Blue100: "#83CDE8",
  Blue200: "#77C8E6"
}, un = {
  mini: "4px",
  tiny: "8px",
  small: "12px",
  medium: "20px",
  big: "40px",
  large: "60px",
  huge: "80px"
}, cn = {
  weights: {
    regular: 400,
    medium: 500,
    bold: 600
  },
  sizes: {
    error: "12px",
    default: "16px",
    info: "18px",
    h1: "28px",
    h2: "18px",
    icon: "64px",
    notFound: "128px"
  },
  lineHeight: {
    default: "24px",
    h1: "44px",
    h2: "28px"
  },
  letterSpacing: {
    default: "normal",
    h1: "0.1em",
    h2: "0.02em"
  }
}, fn = {
  mobile: Lr.MOBILE,
  tablet: Lr.TABLET
}, Va = {
  colors: {
    main: j.White100,
    secondary: j.Grey100,
    secondary_opacity: j.Grey100_50,
    color1: j.Grey300,
    color2: j.Grey200,
    color3: j.White200,
    color4: j.Grey300,
    color5: j.Grey400,
    weak: j.Orange100,
    good: j.Green100,
    error: j.Red100,
    warning: j.Yellow100,
    success: j.Green300,
    info: j.Blue100,
    toastError: j.Red200,
    toastWarning: j.Yellow200,
    toastSuccess: j.Green400,
    toastInfo: j.Blue200
  },
  font: cn,
  spaces: un,
  breakpoints: fn
}, Ea = {
  colors: {
    main: j.Grey100,
    secondary: j.White300,
    secondary_opacity: j.White300_50,
    color1: j.White100,
    color2: j.White200,
    color3: j.Grey200,
    color4: j.Grey300,
    color5: j.Grey400,
    weak: j.Orange200,
    good: j.Green200,
    error: j.Red200,
    warning: j.Yellow200,
    success: j.Green400,
    info: j.Blue200,
    toastError: j.Red100,
    toastWarning: j.Yellow100,
    toastSuccess: j.Green300,
    toastInfo: j.Blue100
  },
  font: cn,
  spaces: un,
  breakpoints: fn
}, Ca = ({ theme: e = "dark", children: t }) => /* @__PURE__ */ ke(Lo, { theme: e === "dark" ? Ea : Va, children: [
  /* @__PURE__ */ le(Wa, {}),
  /* @__PURE__ */ le(Ut, { theme: e }),
  /* @__PURE__ */ le(er.Provider, { children: t })
] });
var dn = {
  color: void 0,
  size: void 0,
  className: void 0,
  style: void 0,
  attr: void 0
}, xr = E.createContext && E.createContext(dn), Be = globalThis && globalThis.__assign || function() {
  return Be = Object.assign || function(e) {
    for (var t, r = 1, n = arguments.length; r < n; r++) {
      t = arguments[r];
      for (var i in t)
        Object.prototype.hasOwnProperty.call(t, i) && (e[i] = t[i]);
    }
    return e;
  }, Be.apply(this, arguments);
}, La = globalThis && globalThis.__rest || function(e, t) {
  var r = {};
  for (var n in e)
    Object.prototype.hasOwnProperty.call(e, n) && t.indexOf(n) < 0 && (r[n] = e[n]);
  if (e != null && typeof Object.getOwnPropertySymbols == "function")
    for (var i = 0, n = Object.getOwnPropertySymbols(e); i < n.length; i++)
      t.indexOf(n[i]) < 0 && Object.prototype.propertyIsEnumerable.call(e, n[i]) && (r[n[i]] = e[n[i]]);
  return r;
};
function pn(e) {
  return e && e.map(function(t, r) {
    return E.createElement(t.tag, Be({
      key: r
    }, t.attr), pn(t.child));
  });
}
function xa(e) {
  return function(t) {
    return E.createElement(Fa, Be({
      attr: Be({}, e.attr)
    }, t), pn(e.child));
  };
}
function Fa(e) {
  var t = function(r) {
    var n = e.attr, i = e.size, s = e.title, a = La(e, ["attr", "size", "title"]), l = i || r.size || "1em", d;
    return r.className && (d = r.className), e.className && (d = (d ? d + " " : "") + e.className), E.createElement("svg", Be({
      stroke: "currentColor",
      fill: "currentColor",
      strokeWidth: "0"
    }, r.attr, n, a, {
      className: d,
      style: Be(Be({
        color: e.color || r.color
      }, r.style), e.style),
      height: l,
      width: l,
      xmlns: "http://www.w3.org/2000/svg"
    }), s && E.createElement("title", null, s), e.children);
  };
  return xr !== void 0 ? E.createElement(xr.Consumer, null, function(r) {
    return t(r);
  }) : t(dn);
}
function Ma(e) {
  return xa({ tag: "svg", attr: { viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true" }, child: [{ tag: "path", attr: { fillRule: "evenodd", d: "M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zm-1.72 6.97a.75.75 0 10-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 101.06 1.06L12 13.06l1.72 1.72a.75.75 0 101.06-1.06L13.06 12l1.72-1.72a.75.75 0 10-1.06-1.06L12 10.94l-1.72-1.72z", clipRule: "evenodd" } }] })(e);
}
const Da = Pe.div`
	${({ theme: { breakpoints: e, spaces: t, colors: r } }) => I`
			width: 100vw;
			height: 100vh;
			position: absolute;
			left: 0;
			top: 0;
			display: grid;
			place-items: center;
			background-color: ${r.secondary_opacity};

			@media (max-width: ${e.mobile}px) {
				width: 100%;
				padding: ${t.medium};
				overflow: none;
			}
		`}
`, Za = Pe.div`
	${({ theme: { breakpoints: e, colors: t, spaces: r } }) => I`
			padding: ${r.medium};
			width: 700px;
			border-radius: 10px;
			background-color: ${t.main};

			@media (max-width: ${e.mobile}px) {
				width: 100%;
				overflow: none;
			}
		`}
`, Ra = Pe.div`
	${({ theme: { spaces: e } }) => I`
			width: 100%;
			display: flex;
			justify-content: flex-end;
			gap: ${e.medium};
		`}
`, ka = Pe.div`
	${({ theme: { colors: e } }) => I`
			width: 100%;
			display: flex;
			justify-content: space-between;
			align-content: center;

			> .close-icon {
				cursor: pointer;
				width: 30px;
				height: 30px;

				&:hover {
					color: ${e.color5};
				}
			}
		`}
`, Ya = er.create(
  ({
    id: e,
    title: t,
    subtitle: r = "",
    content: n,
    variant: i = "primary",
    action: s,
    actionName: a,
    cancelName: l,
    "data-testid": d = "modal"
  }) => {
    const p = $t(), v = () => p.remove();
    return /* @__PURE__ */ le(
      Da,
      {
        id: e,
        "data-testid": d,
        children: /* @__PURE__ */ ke(Za, { children: [
          /* @__PURE__ */ ke(ka, { children: [
            /* @__PURE__ */ ke("div", { children: [
              /* @__PURE__ */ le(
                _r,
                {
                  level: 3,
                  align: "left",
                  children: t
                }
              ),
              r && /* @__PURE__ */ ke(Fn, { children: [
                /* @__PURE__ */ le(
                  At,
                  {
                    type: "vertical",
                    space: "tiny"
                  }
                ),
                /* @__PURE__ */ le($r, { children: r })
              ] })
            ] }),
            /* @__PURE__ */ le(
              Ma,
              {
                onClick: v,
                className: "close-icon"
              }
            )
          ] }),
          /* @__PURE__ */ le(At, { type: "vertical" }),
          /* @__PURE__ */ le("div", { children: n }),
          /* @__PURE__ */ le(
            At,
            {
              type: "vertical",
              space: "large"
            }
          ),
          /* @__PURE__ */ ke(Ra, { children: [
            /* @__PURE__ */ le(
              Ht,
              {
                variant: "secondary",
                action: v,
                children: l
              }
            ),
            /* @__PURE__ */ le(
              Ht,
              {
                variant: i,
                action: s,
                children: a
              }
            )
          ] })
        ] })
      }
    );
  }
), Ha = ({
  id: e,
  title: t,
  subtitle: r = "",
  content: n,
  actionName: i,
  cancelName: s,
  variant: a = "primary",
  action: l
}) => er.show(Ya, {
  id: e,
  title: t,
  subtitle: r,
  content: n,
  actionName: i,
  cancelName: s,
  variant: a,
  action: l
});
export {
  Ba as Break,
  Ht as Button,
  _r as Heading,
  Uo as Image,
  Io as Layout,
  Qr as Loader,
  $r as Paragraph,
  At as Spacer,
  Ca as Theme,
  Ha as modalShow
};
